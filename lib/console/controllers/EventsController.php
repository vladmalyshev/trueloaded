...
class EventsController extends Controller {
   ...
   /* public function options($actionID)
    ...
    /**
     * cron newsletters sync
     */
    public function actionNewslettersSubscription() {
    ...
    public function actionProductPricesindex(){
        if($ext = \common\helpers\Acl::checkExtension('ProductPriceIndex', 'reindex')){
            if ($ext::isEnabled()){
                //$ext::reindex();
    ...
    public function actionProductPricescleanup(){
        if($ext = \common\helpers\Acl::checkExtension('ProductPriceIndex', 'cleanup')){
            if ($ext::isEnabled()){
                $ext::cleanup();
            }
        } else {
    ...
    public function actionPlainProductTable($count=0, $ignoreLifeTime=0){
      /* @var $ext \common\extensions\PlainProductsDescription\PlainProductsDescription */
        if($ext = \common\helpers\Acl::checkExtension('PlainProductsDescription', 'reindex')){
            if ($ext::isEnabled()){
 ...
 */
    public function actionPlainProductTablecleanup($force = 0){
      /* @var $ext \common\extensions\PlainProductsDescription\PlainProductsDescription */
        if($ext = \common\helpers\Acl::checkExtension('PlainProductsDescription', 'cleanup')){
            if ($ext::isEnabled()){
                $ext::cleanup($force);
            }
        } else {
