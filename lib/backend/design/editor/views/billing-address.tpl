    ...
    <div class="widget-header widget-header-billing">
        <h4 {if $same}class="disabled"{/if}>
            ...
            {if $manager->has('billto')}
                ...
                <span class="header-address">{\common\helpers\Address::address_format($bAddress['country']['address_format_id'], ['postcode' => $bAddress['postcode'], 'country_id' => $bAddress['country_id'], 'city' => $bAddress['city'] ], true, '', '|')}</span>                
            {/if}
        </h4>
        {if $same}
