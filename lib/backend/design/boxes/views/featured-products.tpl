...
<form action="{Yii::getAlias('@web')}/design/box-save" method="post" id="box-save">
  ...
  <div class="popup-heading">
    {$smarty.const.BOX_CATALOG_FEATURED}
  </div>
  <div class="popup-content">
    ...
    <div class="tabbable tabbable-custom">
      <ul class="nav nav-tabs">
        ...
        <li class="active"><a href="#type" data-toggle="tab">{$smarty.const.TEXT_MAXIMUM_FEATURED_PRODUCTS}</a></li>
      ...
      <div class="tab-content">
        <div class="tab-pane active menu-list" id="type">
          ...
          <div class="setting-row">
            <label for="">{$smarty.const.TEXT_FEATURED_TYPES}</label>
            <select name="setting[0][featured_type_id]" id="" class="form-control">
              {foreach $featuredTypes as $featuredTypeId => $featuredTypeName}
                <option value="{$featuredTypeId}"{if $settings[0].featured_type_id == $featuredTypeId} selected{/if}>{$featuredTypeName}</option>
              {/foreach}
            </select>
          </div>
