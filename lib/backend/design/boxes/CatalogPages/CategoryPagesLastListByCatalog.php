...
class CategoryPagesLastListByCatalog extends Widget
{
    ...
    public function run()
    {
        ...
        if (!isset($this->settings[0]['selectCatalogPageLastListById'])) {
            $this->settings[0]['selectCatalogPageLastListById'] = $catalogPages[0];
        }
        if (!isset($this->settings[0]['limitInformationLastListByIdPage'])) {
            $this->settings[0]['limitInformationLastListByIdPage'] = 6;
        }

        return $this->render('../../views/category-page-last-list-by-id.tpl', [
            'id' => $this->id,
            'params' => $this->params,
