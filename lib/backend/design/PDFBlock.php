...
class PDFBox extends \TCPDF {
        ...
        while ($item = tep_db_fetch_array($items_query)){
            ...
            if ($item['widget_name'] == 'BlockBox' || $item['widget_name'] == 'email\BlockBox' || $item['widget_name'] == 'cart\CartTabs'){
                ...
                $height = $pdf2->GetY() + $settings[0]['padding-top'] * $ds +
                    ...
                    $settings[0]['border-top_width'] * $ds +
    ...
    public function BlockCreate($name, $page_params, $position, $pdf_params)
    {
        ...
        while ($item = tep_db_fetch_array($items_query)) {
            ...
            if (($position['top'] + $height > $pdf_params['height'] - $pdf_params['pdf_margin_bottom'] * $ds) && $this->blockName != 'pdf_footer'){
    ...
    public function pageHeader($page_params, $pdf_params){
        ...
        if ($pdf_params['showHeader']) {
            ...
            if (count($sizes) > 0) {
                ...
                $this->sizes = array();
                $this->BlockSizes('pdf_header', $page_params, $this->widthSheet, $pdf_params);
                $this->setChooseHeight();
            }

            $this->BlockCreate('pdf_header', $page_params, ['top' => 0, 'left' => 0], $pdf_params);
    ...
    public function pageFooter($page_params, $pdf_params){
        ...
        if ($pdf_params['showFooter']) {
            ...
            if (count($sizes) > 0) {
                ...
                $this->sizes = array();
                $this->BlockSizes('pdf_footer', $page_params, $this->widthSheet, $pdf_params);
                $this->setChooseHeight();
            }

            $block = tep_db_fetch_array(tep_db_query("select id from " . TABLE_DESIGN_BOXES . " where block_name='pdf_footer'"));

            $this->blockName = 'pdf_footer';
            $this->BlockCreate('pdf_footer', $page_params, [
                'top' => $pdf_params['height'] - $this->sizes[$block['id']]['height'],
                'left' => 0
            ], $pdf_params);
            $this->blockName = '';
...
FD: equivalent to F + D option*/
            ...
            'showHeader' => false,//header created in designer
            'showFooter' => false,//footer created in designer
        ];
    ...
    {
        if ($this->pages[0]['params']['theme_name']){
        ...
        $pageSettingsQuery = \common\models\ThemesSettings::find()->where([
            ...
            'setting_name' => $this->pages[0]['name'],
        ])->asArray()->all();
