...
class OrdersService
{
    ...
    public function getTotalsFromDataOrder(array $totals)
    {
        ...
        for ($i = 0, $n = count($totals); $i < $n; $i++) {
            if (is_file(DIR_FS_CATALOG . DIR_WS_LANGUAGES . '/modules/order_total/' . $totals[$i]['class'] . '.php')) {
                include_once DIR_FS_CATALOG . DIR_WS_LANGUAGES . '/modules/order_total/' . $totals[$i]['class'] . '.php';
            }
    ...
    public function sendRequestPay(OrderAbstract $order,currencies $currencies)
    {
        ...
        \common\helpers\Mail::send(
            $order->customer['firstname'] . ' ' . $order->customer['lastname'], $order->customer['email_address'], $email_subject, $email_text, $STORE_OWNER, $STORE_OWNER_EMAIL_ADDRESS, [], '', '', ['add_br' => 'no']
        );
