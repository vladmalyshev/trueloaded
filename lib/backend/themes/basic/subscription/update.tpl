...
<div class="row w-or-prev-next">
    ...
    <a href="{$app->urlManager->createUrl(['subscription/process-order', 'orders_id' => $app->controller->view->order_prev])}" class="btn-next-prev-or btn-prev-or" title="{$smarty.const.TEXT_GO_PREV_ORDER} (#{$app->controller->view->order_prev})"></a>
    {else}
    ...
    <a href="{$app->urlManager->createUrl(['subscription/process-order', 'orders_id' => $app->controller->view->order_next])}" class="btn-next-prev-or btn-next-or" title="{$smarty.const.TEXT_GO_NEXT_ORDER} (#{$app->controller->view->order_next})"></a>
    {else}
...
function deleteOrder(orders_id) {
    ...
    if (r == true) {
        ...
        $.post("{$app->urlManager->createUrl('subscription/orderdelete')}", {
                    'orders_id': orders_id,
            ...
            if (status == "success") {
                ...
                window.location.href= "{$app->urlManager->createUrl('subscription/')}";
            } else {
                alert("Request error.");
            }
        },"html");
...
//ajax save
    ...
    $.post("{$app->urlManager->createUrl('subscription/submit-subscription')}", $('#status_edit').serialize(), function(data, status){
        if (status == "success") {
            //$('#order_management_data .scroll_col').html(data);
            $("#order_management_data").html(data);
    /*        
