...
<div class="order-wrap">
        ...
        <div class="col-md-12">
            <div class="widget-content">
                ...
                {if is_array($messages)}
                    ...
                    {/foreach}
                {/if}
                <table class="table table-striped table-bordered table-hover table-responsive table-checkable table-selectable datatable" checkable_list="0" data_ajax="xsell-types/list">
                    <thead>
                    <tr>
                        {foreach $app->controller->view->xsellTypeTable as $tableItem}
    ...
    <script type="text/javascript">
        ...
        function onClickEvent(obj, table) {
            ...
            var xsell_type_id = $(obj).find('input.cell_identify').val();
            $.post("xsell-types/actions", { 'xsell_type_id' : xsell_type_id }, function(data, status){
                if (status == "success") {
        ...
        function xsellTypeEdit(id){
            $("#currency_management").hide();
            $.get("xsell-types/edit", { 'xsell_type_id' : id }, function(data, status){
                if (status == "success") {
                    $('#currency_management_data').html(data);
                    $("#currency_management").show();
        ...
        function xsellTypeSave(id){
            $.post("xsell-types/save?xsell_type_id="+id, $('form[name=xsell_type]').serialize(), function(data, status){
                if (status == "success") {
                    //$('#currency_management_data').html(data);
                    //$("#currency_management").show();
        ...
        function xsellTypeDelete(id){
            if (confirm('Do you confirm?')){
                $.post("xsell-types/delete", { 'xsell_type_id' : id}, function(data, status){
                    if (status == "success") {
                        //$('.alert #message_plce').html('');
                        //$('.alert').show().removeClass('alert-error alert-success alert-warning').addClass(data['messageType']).find('#message_plce').append(data['message']);
                        if (data == 'reset') {
