<form action="" class="delete-type-form">
    <div class="popup-heading">{$smarty.const.IMAGE_DELETE} "{$title}"</div>
...
<script type="text/javascript">
    $(function(){
        ...
        $('.popup-buttons .cancel-delete').on('click', function(){
            $.get("customer-additional-fields/bar", {
                'group_id' : '{$group_id}',
                'field_id' : '{$field_id}',
        ...
        $('.delete-type-form').on('submit', function(e){
            ...
            if ($('.confirm-input', form).val().toLowerCase() === '{$smarty.const.TYPE_DELETE}'.toLowerCase()) {
                ...
                $.get('{Yii::$app->urlManager->createUrl(['customer-additional-fields/delete'])}', {
                    'group_id' : '{$group_id}',
                    'field_id' : '{$field_id}',
                    'level_type' : '{$level_type}'
                }, function(data){
