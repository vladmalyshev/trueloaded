        ...
        <div class="widget box box-wrapp-blue filter-wrapp">
          ...
          <div class="widget-content after">
              <div class="f_row f_row_pl_cus f_row_pl filter_show column-block">  
                ...
                <ul class="col col_1">
                  <li><input type="checkbox" class="shown sdef uniform" value="0" {if in_array(0, $app->controller->view->key_entity)}checked{/if} data-column ="0">&nbsp;{$smarty.const.TABLE_HEADING_LANGUAGE_KEY}</li>
                  <li><input type="checkbox" class="shown sdef uniform" value="1"  {if in_array(1, $app->controller->view->key_entity)}checked{/if} data-column ="1">&nbsp;{$smarty.const.TABLE_HEADING_LANGUAGE_ENTITY}</li>
                </ul><!--onclick="tableColumn(this)"-->
                <ul class="col">
                {foreach $languages as $_key => $_lang}
                  {if $languages|count > 4 && $_key is div by 4 }
                    </ul><ul class="col">
                  {/if}
                  <li><input type="checkbox" class="shown shownlang uniform" data-language="{$_lang['id']}" value="1" data-column ="{$_lang['data']}" {if $_lang['shown_language'] == 1}checked{/if}>&nbsp;{ucfirst($_lang['code'])}</li>
                {/foreach}
                </ul>
              </div>          
            
            <div class="filter_categories column-block filter-box filter-box-cus filter-box-pl filters-text">
                    ...
                    <div class="filter_checkboxes">
                        {foreach $languages as $sl}
                            <div>
                              ...
                              <span style="vertical-align: middle;">{$sl['name']}</span>
                            </div>
                        {/foreach}
                    </div>
                  </div>
        ...
        <div class="col-md-12">
            <div class="widget-content">
                ...
                <div class="ord_status_filter_row filter_texts">
                    <form id="filterForm" name="filterForm" onsubmit="return applyFilter();">
                        {$app->controller->view->filterEntity}
