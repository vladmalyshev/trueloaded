...
<div class="order-wrap">
        ...
        <div class="col-md-12">
            <div class="widget-content">
                ...
                {if is_array($messages)}
                    ...
                    {/foreach}
                {/if}
                <table class="table table-striped table-bordered table-hover table-responsive table-checkable table-selectable datatable" checkable_list="0" data_ajax="featured-types/list">
                    <thead>
                    <tr>
                        {foreach $app->controller->view->featuredTypeTable as $tableItem}
    ...
    <script type="text/javascript">
        ...
        function onClickEvent(obj, table) {
            ...
            var featured_type_id = $(obj).find('input.cell_identify').val();
            $.post("featured-types/actions", { 'featured_type_id' : featured_type_id }, function(data, status){
                if (status == "success") {
        ...
        function featuredTypeEdit(id){
            $("#currency_management").hide();
            $.get("featured-types/edit", { 'featured_type_id' : id }, function(data, status){
                if (status == "success") {
                    $('#currency_management_data').html(data);
                    $("#currency_management").show();
        ...
        function featuredTypeSave(id){
            $.post("featured-types/save?featured_type_id="+id, $('form[name=featured_type]').serialize(), function(data, status){
                if (status == "success") {
                    //$('#currency_management_data').html(data);
                    //$("#currency_management").show();
        ...
        function featuredTypeDelete(id){
            if (confirm('Do you confirm?')){
                $.post("featured-types/delete", { 'featured_type_id' : id}, function(data, status){
                    if (status == "success") {
                        //$('.alert #message_plce').html('');
                        //$('.alert').show().removeClass('alert-error alert-success alert-warning').addClass(data['messageType']).find('#message_plce').append(data['message']);
                        if (data == 'reset') {
