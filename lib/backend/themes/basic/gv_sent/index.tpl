...
<div class="row order-box-list">
    <div class="col-md-12">
            <div class="widget-content" id="reviews_list_data">
                <table class="table table-striped table-bordered table-hover table-responsive table-checkable datatable table-gv_sent"
                       checkable_list="0,1,2,3" data_ajax="gv_sent/list?cid={$cid}">
                    <thead>
                    <tr>
...
<script type="text/javascript">
    ...
    function saveItem() {
        $.post("gv_sent/submit", $('#save_item_form').serialize(), function (data, status) {
            if (status == "success") {
                $('#reviews_management_data .scroll_col').html(data);
                $("#reviews_management").show();
                ...
                var url = $base_url + "/gv_sent/categorieslist/";
    ...
    function deleteItemConfirm( item_id) {
        $.post("gv_sent/confirmitemdelete", {  'item_id': item_id }, function (data, status) {
            if (status == "success") {
                $('#reviews_management_data .scroll_col').html(data);
                $("#reviews_management").show();
                switchOnCollapse('reviews_management_collapse');
    ...
    function deleteItem() {
        $.post("gv_sent/itemdelete", $('#item_delete').serialize(), function (data, status) {
            if (status == "success") {
                resetStatement();
    ...
    $('body').on('click','.toggleStatus', function (e) {
        e.preventDefault();
        $("#pre-wait").addClass("preloader");
        var id = $(this).attr('data-id');
        var status = $(this).attr('data-status');
        $.post("{$app->urlManager->createUrl('gv_sent/toggle-status')}", {
            'id': id,
            'status': status,
        }, function (data, status) {
            $("#pre-wait").removeClass("preloader");
            if (status === "success" && data.hasOwnProperty('success') && data.success === true) {
                $('#wrapToggleStatus').html(data.html);
            } else {
                alert("Request error.");
            }
        });
