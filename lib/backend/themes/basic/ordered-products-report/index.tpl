{use class="\common\helpers\Html"}
{use class="\yii\helpers\Url"}
{\backend\assets\MultiSelectAsset::register($this)|void}
<!--=== Page Header ===-->
<div class="page-header">
  <div class="page-title">
    <h3>{$app->controller->view->headingTitle}</h3>
  </div> 
</div>
<!-- /Page Header -->
<!--<div>{$excluded}</div>-->
<style>
  tr:last-child{ background-color: aliceblue; }
</style>

  <!--=== Page Content ===-->
  <div class="widget box box-wrapp-blue widget-closed filter-wrapp widget-fixed">
        <div class="widget-header filter-title">
            <h4 class="s_filter_title">{$smarty.const.TEXT_FILTER}</h4>                        
            <div class="toolbar no-padding">
              <div class="btn-group">
                <span class="btn btn-xs widget-collapse"><i class="icon-angle-up"></i></span>
              </div>
            </div>
        </div>
        <div class="widget-content filter_values">
        {Html::beginForm('ordered-products-report', 'get', ['id'=>'filterForm', 'name'=>'ordered-products'])}
        <div class="wrap_filters after wrap_filters_4">
            <div class="item_filter item_filter_1">
                <fieldset style="display:inline-grid;">
                    <legend>{$smarty.const.TEXT_DATE_RANGE}</legend>
                    <div class="f_td f_td_radio ftd_block">
                      <div>
                        <label>
                          {$smarty.const.TEXT_FROM}
                          {Html::textInput('start_date', $app->controller->start_date, ['class' => "datepicker", 'autocomplete' => 'off'])}
                        </label>
                      </div>
                      <div>
                        <label>
                          {$smarty.const.TEXT_TO}
                          {Html::textInput('end_date', $app->controller->end_date, ['class' => "datepicker", 'autocomplete' => 'off'])}
                        </label>
                      </div>
                    </div>
                </fieldset>


            </div>
            <div class="item_filter item_filter_2">
                <div class="tl_filters_title"></div>
                
                <div class="wl-td">
                    <label>{$smarty.const.TEXT_STATUS}</label>
                    {Html::dropDownList('status[]', $app->controller->selected_status, $app->controller->status, ['multiple' => 'multiple', 'data-role' => 'multiselect', 'id' => 'statusId'])}
                </div>

                <div class="wl-td">
                    <label>{$smarty.const.TEXT_COMMON_PLATFORM_TAB}</label>
                    {Html::dropDownList('platforms[]', $app->controller->selected_platforms, $platforms, ['class' => 'form-control', 'multiple' => 'multiple', 'data-role' => 'multiselect'])}
                </div>
                
                <div class="wl-td">
                    <label>{$smarty.const.TABLE_HEADING_CATEGORY}</label>
                    {Html::dropDownList('categories', $app->controller->selected_categories, $categories, ['class' => 'form-control', 'multiple' => 'multiple', 'data-role' => 'multiselect'])}
                </div>
                
                <div class="wl-td">
                    <label>{$smarty.const.ENTRY_COUNTRY}</label>
                    {Html::dropDownList('countries', $app->controller->selected_countries, $countries, ['class' => 'form-control', 'multiple' => 'multiple', 'data-role' => 'multiselect'])}
                </div>

                <div class="wl-td">
                    <label>{$smarty.const.FILTER_BY_ORDERS}</label>
                    {Html::dropDownList('orders[]', $app->controller->orders, $orders, ['id' => 'filterOrders', 'class' => 'form-control', 'multiple' => 'multiple', 'data-role' => 'multiselect'])}
                </div>
            </div>

            <div class="item_filter item_filter_3">
              <div class="tl_filters_title">{$smarty.const.OPTIONS_SHOW_COLUMNS}</div>
              <div class="filter_checkboxes_column">
                <div class="wl-td">
                    <label>
                    {Html::checkbox('showcolumns[]', in_array('cat', $app->controller->showcolumns), ['value'=>'cat'])}
                    {$smarty.const.OPTIONS_SHOW_CATEGORIES}
                    </label>
                </div>
                <div class="wl-td">
                    <label>
                    {Html::checkbox('showcolumns[]', in_array('country', $app->controller->showcolumns), ['value'=>'country'])}
                    {$smarty.const.OPTIONS_SHOW_COUNTRY}
                    </label>
                </div>
                <div class="wl-td">
                    <label>
                    {Html::checkbox('showcolumns[]', in_array('sp', $app->controller->showcolumns), ['value'=>'sp'])}
                    {$smarty.const.OPTIONS_SHOW_SALE_PRICE}
                    </label>
                </div>
                <div class="wl-td">
                    <label>
                    {Html::checkbox('showcolumns[]', in_array('pp', $app->controller->showcolumns), ['value'=>'pp'])}
                    {$smarty.const.OPTIONS_SHOW_PURCHASE_PRICE}
                    </label>
                </div>
              </div>
            </div>

            <div style="float: right;bottom: 10px;position: absolute;right: 10px;">
            <a href="javascript:void(0)" onclick="return resetFilter();" class="btn">{$smarty.const.TEXT_RESET}</a>&nbsp;&nbsp;&nbsp;<button type="submit" class="btn btn-primary">{$smarty.const.TEXT_SEARCH}</button>
            </div>
        </div>
        {Html::endForm()}
        </div>
    </div>
  <div class="order-wrap">
  <input type="hidden" id="row_id">  
  <div class="row">
    <div class="col-md-12">
      <div class="widget-content">
        <div class="alert fade in" style="display:none;">
          <i data-dismiss="alert" class="icon-remove close"></i>
          <span id="message_plce"></span>
        </div>
        
        <div class="col-md-12">
			<table class="table table-striped table-selectable table-checkable table-hover table-responsive table-bordered datatable tabl-res double-grid table-orders table-colored" data_ajax="ordered-products-report/list" checkable_list="">
              <thead>
              <tr>
                {foreach $app->controller->view->reportTable as $tableItem}
                  <th{if $tableItem['not_important'] == 1} class="hidden-xs"{/if} width="{$tableItem['width']}">{$tableItem['title']}</th>
                {/foreach}
              </tr>
              </thead>

            </table>
        </div>


        <p class="btn-wr">        	
            <a style="display:none;" href="javascript:void(0);" ></a>
            <button class="btn btn-primary" onClick="_export(this);">{$smarty.const.TEXT_EXPORT}</button>
        </p>
        
      </div>

    </div>
  </div>
  <script>
  
    $(document).ready(function(){

      $( ".datepicker" ).datepicker({
          changeMonth: true,
          changeYear: true,
          showOtherMonths:true,
          autoSize: false,
          dateFormat: '{$smarty.const.DATE_FORMAT_DATEPICKER}',
      });

      $("form select[data-role=multiselect]").multipleSelect({
              multiple: true,
              filter: true
      });

      $('#filterOrders:not(.tlFilterUpdated)').addClass('tlFilterUpdated').on('tlFilterUpdated', function (){
        //console.log($(this));
       $.post("{Yii::$app->urlManager->createUrl('ordered-products-report/filter-list')}",
              $('#filterForm').serialize() + '&list=orders',
              function( data ) {
                //console.log( data.result ); // John
                //console.log( data.options ); // 2pm
             //   $('#filterOrders').multipleSelect('disable');
                $('#filterOrders').html(data.options);
                $('#filterOrders').multipleSelect('refresh');
             //   $('#filterOrders').multipleSelect('enable');
              }, 
        "json");
        return false;
      });

      $('#startDate, #statusId').not('.tlFilterUpdated').addClass('tlFilterUpdated').on('change', function() {
        $("form select.tlFilterUpdated[data-role=multiselect]").each(function(){
          $(this).trigger('tlFilterUpdated');
        });
      });

        
    })
    
    $('form.ordered-products').submit(function(e){
        return false;
    })
    
    function setFilterState() {
        orig = $('#filterForm').serialize();
        var url = window.location.origin + window.location.pathname + '?' + orig.replace(/[^&]+=\.?(?:&|$)/g, '')
        window.history.replaceState({ }, '', url);
    }
    function resetFilter(){
        window.location.href="{Yii::$app->urlManager->createUrl('ordered-products-report')}";
    }
    function resetStatement() {
    }
    function onClickEvent(obj, table) {        
    }

    function onUnclickEvent(obj, table) {          
    }

    function applyFilter() {
          resetStatement();
          return false;
    }
   
       
   function _export(obj){
        $.get('ordered-products-report/export', 
                $('#filterForm').serializeArray(),
                function(data, status, e){
                        var filename = 'export.csv';
                        var reader = new FileReader();
                        reader.onload = function(e) {
                          $(obj).prev().attr({ "href": e.target.result, "download": filename }).get(0).click();
                        }                
                        reader.readAsDataURL(new Blob([data]));
                });
                return false;
   }
  </script>
  <!-- /Page Content -->
</div>