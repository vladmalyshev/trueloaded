...
{tep_draw_hidden_field('admin_id', $mInfo->admin_id)}

<div class="or_box_head">{$smarty.const.CATEGORY_PERSONAL}</div>

<div class="main_row">
    <div class="main_title">{$smarty.const.ENTRY_FIRST_NAME}</div>
    <div class="main_value">{tep_draw_input_field('admin_firstname', $mInfo->admin_firstname, 'maxlength="32" class="form-control"', true)}</div>
</div>

<div class="main_row">
    <div class="main_title">{$smarty.const.ENTRY_LAST_NAME}</div>
    <div class="main_value">{tep_draw_input_field('admin_lastname', $mInfo->admin_lastname, 'maxlength="32" class="form-control"', false)}</div>
</div>

<div class="main_row">
    <div class="main_title">{$smarty.const.ENTRY_EMAIL_ADDRESS}</div>
    <div class="main_value">{tep_draw_input_field('admin_email_address', $mInfo->admin_email_address, 'maxlength="100" class="form-control"', true)}</div>
</div>

<div class="main_row">
    <div class="main_title">{$smarty.const.ENTRY_TELEPHONE_NUMBER}</div>
    <div class="main_value">{tep_draw_input_field('admin_phone_number', $mInfo->admin_phone_number, 'maxlength="100" class="form-control"', true)}</div>
</div>
...
<div class="main_row">
    ...
    <div class="main_value">{tep_draw_pull_down_menu('admin_two_step_auth', $adminTwoStepAuthArray, $mInfo->admin_two_step_auth, 'class="form-control"', false)}</div>
</div>

{if \common\helpers\Acl::rule(['MANAGE_MEMBERS', 'TEXT_MANAGE_CHAT'])}
    <div class="main_row">
        <div class="main_title">{$smarty.const.TEXT_CHAT_LOGIN}</div>
        <div class="main_value">{tep_draw_input_field('chat_email_address', $mInfo->chat_email_address, 'maxlength="100" class="form-control"', false)}</div>
    </div>
    <div class="main_row">
        <div class="main_title">{$smarty.const.TEXT_CHAT_PASSWORD}</div>
        <div class="main_value">{tep_draw_input_field('chat_password', $mInfo->chat_password, 'maxlength="100" class="form-control"', false)}</div>
    </div>
{/if}

{if ($ext = \common\helpers\Acl::checkExtension('Communication', 'adminActionAdminEditRender'))}
    {$ext::adminActionAdminEditRender($mInfo->admin_id)}
{/if}

<div class="main_row">
    <div class="main_title">
        {$smarty.const.TEXT_EDIT_TRANSLATIONS_FRONTEND}
        <input type="checkbox" name="frontend_translation" class="translate-frontend" {if $mInfo->frontend_translation}checked{/if}/>
    </div>
</div>
