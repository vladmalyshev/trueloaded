                ...
                <div class="tabbable tabbable-custom tabbable-ep">
                  <ul class="nav nav-tabs nav-tabs-scroll">
                    {foreach $tabs_data as $tab_data}
                      <li class="{$tab_data.class}{if $tab_data.active} active {/if}"><a href="#{$tab_data.id}" data-toggle="tab"><span>{$tab_data.tab_title}</span></a></li>
                    {/foreach}
                  </ul>
                  <div class="tab-content {if $isMultiPlatform}tab-content1{/if}">
                    {foreach $tabs_data as $tab_data}
                    <div id="{$tab_data.id}" class="tab-pane {if $tab_data.active} active{/if}">
                        {if $tab_data.class == 'deprecated-tags'}
                                ...
                                {foreach $languages as $lang_idx=>$lang}
                                  <div class="tab-pane{if $lang_idx == 0} active{/if}" id="{$tab_data.id}_{$platform['id']}_{$lang['id']}">
                                    ...
                                    <table border="0" width="100%" cellspacing="0" cellpadding="2" class="h-teg-table">
                                        {if $tab_data.class == 'add-keys'}
                                            <tr>
                                                <td class="bigText">&nbsp;</td>
                                                <td class="bigText"><a href="#keys-popup" class="btn add-key-btn" data-popup-option="{$tab_data.popupOption}">{$smarty.const.TEXT_TEMPLATES_KEYS_BUTTON}</a></td>
                                            </tr>
                                        {/if}
        ...
        <div class="pageLinksWrapper">
            <select name="key" class="form-control js-popup-keys">
                ...
                <option value="##LOCATION_NAME##"
                        data-delivery-location="1">{Translation::getTranslationValue('TEXT_LOCATION_NAME','admin/seo-delivery-location')}</option>
                ...
                <option value="##LOCATION_TITLE_TAG##"
                        data-delivery-location="1">{Translation::getTranslationValue('TEXT_PAGE_META_TITLE','admin/seo-delivery-location')}</option>
                <option value="##LOCATION_DESCRIPTION_TAG##"
                        data-delivery-location="1">{Translation::getTranslationValue('TEXT_PAGE_META_DESCRIPTION','admin/seo-delivery-location')}</option>
