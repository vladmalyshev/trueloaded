...
<div class="widget box box-no-shadow" style="margin-bottom: 0; border-bottom: 0;">
  ...
  <div class="widget-content{if is_array($app->controller->view->images) && count($app->controller->view->images) > 10} no-carusel{/if}" style="padding-bottom: 0;">
...
<div class="box-gallery after">
                ...
                <div class="tab-content tab-content-vertical">
                  <div class="tab-pane active one-img-gal{if $Item['use_external_images']} product-image-external-active{/if}" id="tab_4_{$Key}_0">
                    ...
                    <div class="one-img-gal-right">
                      ...
                      {if \common\helpers\Acl::checkExtension('ProductDesigner', 'imageBlock')}
      ...
      <div class="box-gallery-right">
        <div class="tabbable tabbable-custom">
          ...
          <div class="tab-content">
            ...
            {if $app->controller->view->showInventory == true}
                {if \common\helpers\Acl::checkExtension('InventortyImages', 'productBlock2')}
                    {\common\extensions\InventortyImages\InventortyImages::productBlock2($Key, $Item)}
                {else} 
  ...
  <script type="text/javascript">
    ...
    function addExternalImage()
    {
        ...
        var isFirstImage = $("#images-listing li").length==1;
        $.get("{Yii::$app->urlManager->createUrl('categories/product-new-image')}", { id: imagesQty, external: 1 }, function(data, status){
            if (status == "success") {
                var $data = $(data);
                if ( isFirstImage ) {
                    $data.find("input[name='default_image']").attr('checked','checked');
...
{/if}
        ...
        $('.js_image_inventory').each(function(){
          ...
          if (!img_inventory_allowed) {
            $cont.html('{$smarty.const.TOO_MANY_IMAGES_SETUP_ATTRIBUTES|escape:'html'}');
            return;
          }
        ...
        <div class="category-image-map form-container">
          <div class="row">
            ...
            <div class="col-md-6" style="padding: 20px 0">
              <input type="text" class="form-control map-name" name="map_name" value="{$app->controller->view->mapsTitle}" autocomplete="off"/>
              <input type="hidden" name="maps_id" value="{$app->controller->view->mapsId}"/>
              <div class="search-map"></div>
            </div>
            <div class="col-md-3">
              <div class="map-image-holder">
              <img {if $app->controller->view->mapsImage!=''}src="../images/maps/{$app->controller->view->mapsImage}"{/if} class="map-image" alt="" {if !$app->controller->view->mapsImage} style="display: none" {/if}>
                <div class="map-image-remove" {if !$app->controller->view->mapsImage} style="display: none" {/if}></div>
              </div>
            </div>
          </div>
        </div>
