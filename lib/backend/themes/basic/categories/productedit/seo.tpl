    ...
    {foreach $languages as $lKey => $lItem}
      <div class="tab-pane tab-seo{if $lKey == 0} active{/if}" id="tab_9_{$platform_id}_{$lItem['id']}">
        <div class="edp-line">
          ...
          <input type="text" name="pDescription[{$platform_id}][{$lItem['id']}][products_seo_page_name]" value="{$pDescription[$platform_id][$lKey]['products_seo_page_name']|escape}" class="form-control form-control-small" />
        </div>
        <div class="edp-line">
          <label>{$smarty.const.TEXT_NO_INDEX}</label>
          <input name="pDescription[{$platform_id}][{$lItem['id']}][noindex_option]" value="1" class="check_bot_switch_on_off" type="checkbox" {if $pDescription[$platform_id][$lKey]['noindex_option']}checked{/if} />
        </div>
        <div class="edp-line">
          <label>{$smarty.const.TEXT_NO_FOLLOW}</label>
          <input name="pDescription[{$platform_id}][{$lItem['id']}][nofollow_option]" value="1" class="check_bot_switch_on_off" type="checkbox" {if $pDescription[$platform_id][$lKey]['nofollow_option']}checked{/if} />
        </div>
        <div class="edp-line">
          <label>{$smarty.const.TEXT_CANONICAL}</label>
          <input type="text" name="pDescription[{$platform_id}][{$lItem['id']}][rel_canonical]" value="{$pDescription[$platform_id][$lKey]['rel_canonical']|escape}" class="form-control form-control-small" />
          {$dItem['rel_canonical']}
        </div>
        <div class="edp-line">
          <label>{$smarty.const.TEXT_SELF_SERVICE}</label>
          <textarea name="pDescription[{$platform_id}][{$lItem['id']}][products_self_service]" class="form-control form-control-small" wrap="soft" cols="70" rows="5">{$pDescription[$platform_id][$lKey]['products_self_service']}</textarea>
        </div>
        <div class="edp-line">
          <label>{$smarty.const.TEXT_PAGE_TITLE}</label>
          <input type="text" name="pDescription[{$platform_id}][{$lItem['id']}][products_head_title_tag]" value="{$pDescription[$platform_id][$lKey]['products_head_title_tag']|escape}" class="form-control form-control-small products_head_title_tag" />
          <input type="checkbox" name="pDescription[{$platform_id}][{$lItem['id']}][overwrite_head_title_tag]" value="1"{if $pDescription[$platform_id][$lKey]['overwrite_head_title_tag'] || !$pInfo->products_id} checked{/if} /> {$smarty.const.TEXT_OVERWRITE_PAGE_TITLE}
        </div>
        <div class="edp-line">
          <label>{$smarty.const.TEXT_HEADER_DESCRIPTION}</label>
          <textarea name="pDescription[{$platform_id}][{$lItem['id']}][products_head_desc_tag]" class="form-control form-control-small text-dox-01 products_head_desc_tag" wrap="soft" cols="70" rows="5">{$pDescription[$platform_id][$lKey]['products_head_desc_tag']}</textarea>
          <input type="checkbox" name="pDescription[{$platform_id}][{$lItem['id']}][overwrite_head_desc_tag]" value="1"{if $pDescription[$platform_id][$lKey]['overwrite_head_desc_tag'] || !$pInfo->products_id} checked{/if} /> {$smarty.const.TEXT_OVERWRITE_HEADER_DESCRIPTION}
        </div>
        <div class="edp-line">
          <label>{$smarty.const.TEXT_H1_TAG}</label>
          <input type="text" name="pDescription[{$platform_id}][{$lItem['id']}][products_h1_tag]" value="{$pDescription[$platform_id][$lKey]['products_h1_tag']|escape}" class="form-control form-control-small" />
        </div>
        <div class="edp-line">
          <label>{$smarty.const.TEXT_H2_TAG}</label>
          <div class="h-teg-table">
          <span id="products_h2_tag-{$platform_id}-{$lItem['id']}">{foreach explode("\n", $pDescription[$platform_id][$lKey]['products_h2_tag']) as $value}<span class="row"><input type="text" name="pDescription[{$platform_id}][{$lItem['id']}][products_h2_tag][]" value="{$value|escape}" class="form-control form-control-small" /><span class="del-pt del-tag"></span></span>{/foreach}</span><span onclick="addInput('products_h2_tag-{$platform_id}-{$lItem['id']}', '{htmlspecialchars('<span class="row"><input type="text" name="pDescription['|cat:$platform_id|cat:']['|cat:$lItem['id']|cat:'][products_h2_tag][]" value="" class="form-control form-control-small" /><span class="del-pt del-tag"></span></span>')}')" class="btn btn-add-more">{$smarty.const.TEXT_AND_MORE}</span>
        </div>
        </div>
        <div class="edp-line">
          <label>{$smarty.const.TEXT_H3_TAG}</label>
          <div class="h-teg-table">
          <span id="products_h3_tag-{$platform_id}-{$lItem['id']}">{foreach explode("\n", $pDescription[$platform_id][$lKey]['products_h3_tag']) as $value}<span class="row"><input type="text" name="pDescription[{$platform_id}][{$lItem['id']}][products_h3_tag][]" value="{$value|escape}" class="form-control form-control-small" /><span class="del-pt del-tag"></span></span>{/foreach}</span><span onclick="addInput('products_h3_tag-{$platform_id}-{$lItem['id']}', '{htmlspecialchars('<span class="row"><input type="text" name="pDescription['|cat:$platform_id|cat:']['|cat:$lItem['id']|cat:'][products_h3_tag][]" value="" class="form-control form-control-small" /><span class="del-pt del-tag"></span></span>')}')" class="btn btn-add-more">{$smarty.const.TEXT_AND_MORE}</span>
          </div>
        </div>
        <div class="edp-line">
          <label>{$smarty.const.TEXT_IMAGE_ALT_TAG_MASK}</label>
          <input type="text" name="pDescription[{$platform_id}][{$lItem['id']}][products_image_alt_tag_mask]" value="{$pDescription[$platform_id][$lKey]['products_image_alt_tag_mask']|escape}" class="form-control form-control-small" />
        </div>
        <div class="edp-line">
          <label>{$smarty.const.TEXT_IMAGE_TITLE_TAG_MASK}</label>
          <input type="text" name="pDescription[{$platform_id}][{$lItem['id']}][products_image_title_tag_mask]" value="{$pDescription[$platform_id][$lKey]['products_image_title_tag_mask']|escape}" class="form-control form-control-small" />
        </div>
        <div class="edp-line">
          <label>{$smarty.const.TEXT_GOOGLE_PRODUCT_CATEGORY}</label>
          <input type="text" name="pDescription[{$platform_id}][{$lItem['id']}][google_product_category]" value="{$pDescription[$platform_id][$lKey]['google_product_category']|escape}" class="form-control form-control-small" />
        </div>
        <div class="edp-line">
          <label>{$smarty.const.TEXT_GOOGLE_PRODUCT_TYPE}</label>
          <select name="pDescription[{$platform_id}][{$lItem['id']}][google_product_type]" class="form-control form-control-small">
          {foreach $gTypeTree as $gType}
            <option value="{$gType['id']|escape}" {if {isset($gType['id'])} && {$gType['id']==$pDescription[$platform_id][$lKey]['google_product_type']} }selected{/if}>{$gType['text']}</option>
          {/foreach}
          </select>
