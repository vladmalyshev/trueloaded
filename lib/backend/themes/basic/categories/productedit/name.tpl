...
<script>
    ...
    getOptions = function(except){
        let options = ['<option value="0">{$smarty.const.IMAGE_COPY_TO} All</option>'];
        $('.platform-tabs-names li').each(function (i, e){
            if ($("a", e).data('id') != except){
                options.push('<option value="'+ $("a", e).data('id') +'">{$smarty.const.IMAGE_COPY_TO} '+ $("span", e).html() +'</option>');
            }
        })
        return options;
    }
    ...
    $('.platform-language-name-actions').click(function(e){
        ...
        let laction = 0;
        bootbox.dialog({
            title: "{$smarty.const.TEXT_PLATFORM_LANGUAGE_MESSAGE} " + $('.platform-tabs-names a[data-id='+except+'] span').text() + " " + $(this).prev().text(),
            message: "<div><label>{$smarty.const.PULL_DOWN_DEFAULT}</label><br><select class='form-control action-lang-choice'>"+getOptions(except).join("")+"</select><br>+{$smarty.const.IMAGE_COPY_TO}<br>"+getLanguages(currentL),
            buttons: {
                cancel: {
                    label: "Cancel",
                    className: 'btn btn-cancel',
...
{function tabDesc}
    {if !$is_marketplace}
    <div class="platform-lang-tabs-switcher"><label>{strtoupper($smarty.const.TEXT_ALL_LANGUAGES)}&nbsp;</label><input type="checkbox" data-id="{$platform_id}" class="pl-tabs-switcher" {if $app->controller->view->sphl[$platform_id]}checked{/if}></div>
    {/if}
    ...
    {foreach $platform_languages[$platform_id] as $lKey => $lItem}
      <div class="tab-pane{if $lKey == 0} active{/if}" id="tab_1_4_{$platform_id}_{$lItem['id']}" data-language-id="{$lKey}">
        <div class="edp-line">
          ...
          <input type="text" name="pDescription[{$platform_id}][{$lItem['id']}][products_name]" value="{$pDescription[$platform_id][$lKey]->products_name|escape}" class="form-control form-control-small" />
        </div>
        <div class="edp-line">
          <label>{$smarty.const.TEXT_PRODUCTS_INTERNAL_NAME}</label>
          <input type="text" name="pDescription[{$platform_id}][{$lItem['id']}][products_internal_name]" value="{$pDescription[$platform_id][$lKey]->products_internal_name|escape}" class="form-control form-control-small" />
        </div>
        <div class="edp-line">
        ...
        <div class="edp-line">
          ...
          <textarea name="pDescription[{$platform_id}][{$lItem['id']}][products_description]" class="form-control ck-editor text-dox-02" id="txt_products_description_{$platform_id}_{$lItem['id']}" wrap="soft" cols="70" rows="15">{$pDescription[$platform_id][$lKey]->products_description}</textarea>
        </div>
        <div class="edp-line">
          <label>{$smarty.const.TEXT_DESCRIPTION_LINKS}</label>
        ...
        <div class="edp-line short-desc">
          ...
          <textarea name="pDescription[{$platform_id}][{$lItem['id']}][products_description_short]" class="form-control ck-editor text-dox-01" id="txt_products_description_short_{$platform_id}_{$lItem['id']}" wrap="soft" cols="70" rows="15">{$pDescription[$platform_id][$lKey]['products_description_short']}</textarea>
        </div>
        <div class="edp-line">
          <label>{$smarty.const.TEXT_KEYWORDS}</label>
          <textarea name="pDescription[{$platform_id}][{$lItem['id']}][products_head_keywords_tag]" class="form-control form-control-small" wrap="soft" cols="70" rows="5">{$pDescription[$platform_id][$lKey]->products_head_keywords_tag}</textarea>
        </div>
      </div>
    {/foreach}
  </div>
