				...
				<div class="row order-box-list">
					<div class="col-md-12">
							<div class="widget-content">
								...
								<table id="categoriesTable" class="table table-checkable table-striped table-bordered table-hover table-responsive table-selectable datatable tab-status sortable-grid catelogue-grid" data_ajax="categories/list">
									<thead>
										<tr>
                        {foreach $app->controller->view->catalogTable as $tableItem}
                            <th{if $tableItem['not_important'] == 1} class="hidden-xs"{/if}{if $tableItem['not_important'] == 2} class="checkbox-column"{/if}>{$tableItem['title']}</th>
                        {/foreach}
										</tr>
									</thead>
...
function batchMove() {
    ...
    if (count_items > 0) {
        ...
        bootbox.dialog({
                ...
                title: "{$smarty.const.TEXT_MOVE_OR_COPY_BACH}",
                buttons: {
                        success: {
