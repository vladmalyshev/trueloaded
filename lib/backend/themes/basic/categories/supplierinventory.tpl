...
<div class="widget box box-no-shadow supplier-product-widget js-supplier-product" style="margin-bottom: 10px;" data-id-part="{$idPart}" data-supplier-id="{$sInfo->suppliers_id}" id="suppliers{$idPart}">
    <div class="widget-header after">
        <h4>{Html::checkbox('suppliers_data['|cat:$uprid|cat:']['|cat:$sInfo->suppliers_id|cat:'][status]', $sInfo->status, ['class'=>'supplier-product-status'])} {$sInfo->supplier->suppliers_name}</h4>
...
$(document).ready(function() {
  ...
  initBTiSattus($('#suppliers{$idPart} .supplier-product-status'));
