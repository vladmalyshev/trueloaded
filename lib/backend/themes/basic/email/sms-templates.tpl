...
<div class="tabbable tabbable-custom" style="margin-bottom: 0;">
    <ul class="nav nav-tabs">
        {foreach $types as $id => $type}
          <li class="{if $id==$type_id} active {/if}"><a class="js_link_platform_modules_select" href="{Yii::$app->urlManager->createUrl(['email/sms-templates', 'type_id' => $id])}" data-type_id="{$id}"><span>{$type}</span></a></li>
        {/foreach}
    </ul>
</div>
    ...
    <div class="col-md-12">
      <div class="widget-content" id="groups_list_data">
        ...
        <table class="table table-striped table-bordered table-hover table-responsive table-checkable datatable double-grid"
               checkable_list="0" data_ajax="email/sms-templates-list">
          <thead>
          <tr>
  ...
  <script type="text/javascript">
    ...
    function deleteItemConfirm(item_id) {
      $.post("{Yii::$app->urlManager->createUrl('email/sms-templates-delete-confirm')}", { 'item_id': item_id }, function (data, status) {
      if (status == "success") {
        $('#groups_management_data .scroll_col').html(data);
    ...
    function deleteItem() {
      $.post("{Yii::$app->urlManager->createUrl('email/sms-templates-delete')}", $('#item_delete').serialize(), function (data, status) {
        if (status == "success") {
          resetStatement();
          $('#groups_management_data .scroll_col').html("");
          //switchOffCollapse('groups_management_collapse');
