        ...
        <div class="widget box">
            <div class="widget-header">
                ...
                <div class="toolbar no-padding">
                    <div class="btn-group">
                        <span id="reviews_list_box_collapse" class="btn btn-xs widget-collapse"><i
            ...
            <div class="widget-content" id="reviews_list_data">
                <table class="table table-striped table-bordered table-hover table-responsive table-checkable datatable"
                       checkable_list="0,1,2,3" data_ajax="{$Yii->baseUrl}/paypalipn_txn/list">
                    <thead>
                    <tr>
                        {foreach $this->view->catalogTable as $tableItem}
...
<script type="text/javascript">
    ...
    function preEditItem( item_id ) {
        $.post("{$Yii->baseUrl}/paypalipn_txn/itempreedit", {
            'item_id': item_id
        }, function (data, status) {
            if (status == "success") {
                $('#reviews_management_data').html(data);
    ...
    function editItem(item_id) {
        ...
        */
        ...
        $.post("{$Yii->baseUrl}/paypalipn_txn/itemedit", {
            'item_id': item_id
        }, function (data, status) {
            if (status == "success") {
                $('#reviews_management_data').html(data);
    ...
    function saveItem() {
        $.post("{$Yii->baseUrl}/paypalipn_txn/submit", $('#save_item_form').serialize(), function (data, status) {
            if (status == "success") {
                $('#reviews_management_data').html(data);
                $("#reviews_management").show();
                ...
                var url = $base_url + "/paypalipn_txn/categorieslist/";
    ...
    function deleteItemConfirm( item_id) {
        $.post("paypalipn_txn/confirmitemdelete", {  'item_id': item_id }, function (data, status) {
            if (status == "success") {
                $('#reviews_management_data').html(data);
                $("#reviews_management").show();
                switchOnCollapse('reviews_management_collapse');
    ...
    function deleteItem() {
        $.post("paypalipn_txn/itemdelete", $('#item_delete').serialize(), function (data, status) {
            if (status == "success") {
                resetStatement();
...
<!--===  reviews management ===-->
<div class="row" id="reviews_management" style="display: none;">
    <div class="col-md-12">
        <div class="widget box">
            <div class="widget-header">
                <h4><i class="icon-reorder"></i><span id="reviews_management_title">Transaction Details</span>
                </h4>

                <div class="toolbar no-padding">
                    <div class="btn-group">
                        <span id="reviews_management_collapse" class="btn btn-xs widget-collapse"><i
                                    class="icon-angle-down"></i></span>
                    </div>
                </div>
            </div>
