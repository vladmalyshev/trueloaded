...
$(document).ready(function(){
    // ----- filters -----
    $('#categories_ac, #platforms_ac').on('keydown',function(event){
    ...
    $('#categories_ac, #platforms_ac').autocomplete().data('ui-autocomplete')._renderMenu = function( ul, items ) {
         var that = this;
    ...
    $('#chkZipContainer').on('click',function() {
        if ( this.checked ) {
            ...
            var $ZIPoption = $formatSelect.find('option[value="ZIP"]');
            if ( $ZIPoption.css('display')!=='none' ) {
                $formatSelect.val('ZIP');
            }else{
                return false;
            }
        }
    ...
    $('#frmDownload [name="format"]').on('change',function(){
        if ( $(this).val()!='ZIP' && $('#chkZipContainer').length>0 ) {
