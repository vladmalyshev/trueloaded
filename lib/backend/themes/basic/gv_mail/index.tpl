...
<div class="popup-heading popup-heading-coup">{$smarty.const.IMAGE_SEND_EMAIL}</div>
    <div class="col-md-12" style="padding:10px;">
        <div class="widget-content ">
			...
			{/if} 
            <div align = "left">
                {tep_draw_form('mail', 'gv_mail/sendemailtouser', '', 'post', 'onSubmit="return checkForm();"')}
    ...
    function checkForm(mes){
        ...
        if (($('input[name=customers_email_address]').is('input') && !$('input[name=customers_email_address]').val())){
            if ($('input[name=customers_email_address_field]').is('input') && $('input[name=customers_email_address_field]').val()) {
                ...
                wrapMessage("{$smarty.const.TEXT_NO_SELECTED_CUSTOMERS|escape:'javascript'}");
                noerror = false;
            }
        }
        if (($('select[name=coupon_id]').size() > 0 && !$('select[name=coupon_id]').val())
            ...
            if (mes != 1) wrapMessage('please select coupon');
            noerror = false;
        }
        return noerror;
    }
