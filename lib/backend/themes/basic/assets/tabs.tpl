...
{function tabData }
  {* common for all types, $tabsType - variable to implement design tweaks.
    id=$id_prefix  tabData=$data fData=$fieldsData *}
  <div class="tab-content">
    {$fieldSuffixParent=$fieldSuffix}
    {$idSuffixParent=$idSuffix}
    {foreach $tabData as $data}
      <div class="tab-pane {$data['additionalCSS']}{if {$data@iteration}==1} active{/if} {$data['cssClass']}" id="{$id}_{$data@iteration}">
        {if $tabsType=='lTab'}
        <div class="left-pane">
        {/if}
          ...
          {/if}
          {if !is_array($defData) } {*//parent's def. data*}
            {$defData = []}
          {/if}
          {if is_array($data['def_data']) }{*merge with own def. data*}
...
{function lTabTabs}
  <!-- Tabs -->
  <ul class="nav nav-tabs nav-tabs-left" id="{$id_prefix}_ul">
    {foreach $tabData as $data}
      <li class="{if {$data@iteration}==1} active{/if} {$data['cssClass']}" id="{$id}_{$data@iteration}_li"><a href="#{$id}_{$data@iteration}" data-toggle="tab"><span>{$data['title']}</span></a></li>
          {/foreach}
  </ul>
...
{function hTabTabs}
  <!-- Tabs -->
  <ul class="nav nav-tabs" id="{$id_prefix}_ul">
    {foreach $tabData as $data}
      <li class="{if {$data@iteration}==1} active{/if} {$data['cssClass']}" id="{$id}_{$data@iteration}_li"><a href="#{$id}_{$data@iteration}" data-toggle="tab"><span>{$data['title']}</span></a></li>
          {/foreach}
  </ul>
  ...
  *}
  {for $i={$tabs|@count-1} to 0 step -1}
    ...
    {foreach $d as $k => $v}
      ...
      {if {is_array($_assetsTabsLevels[$i])} }
