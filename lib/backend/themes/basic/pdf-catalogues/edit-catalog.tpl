...
<div class="popup-heading">{$smarty.const.BUTTON_ASSIGN_PRODUCTS}</div>
...
<div class="noti-btn">
    <div class="btn-left">
        ...
        <button type="button" class="btn btn-confirm" id="btnAssignCatalog">{$smarty.const.TEXT_ASSIGN}</button>
    </div>
    <div class="btn-right">
        <button type="button" class="btn btn-confirm" id="btnGenerateCatalog">{$smarty.const.BUTTON_GENERATE_PDF_CATALOGUE}</button>
    </div>
</div>
<script type="text/javascript">
  ...
  $('#btnGenerateCatalog').on('click',function(){
    $.ajax({
      url: $('#tree').attr('data-data-save'),
      type: 'POST',
      data: {
      ...
      success: function (data) {
        if ( data.status && data.status=='ok' ) {
          $('#btnAssignCatalogCancel').trigger('click');
          resetStatement();
          window.location.href = '{Yii::$app->urlManager->createUrl(['pdf-catalogues/generate-pdf-catalogue', 'pdf_catalogues_id' => $pdf_catalogues_id])}';
        }
      }
    });
  });
