...
<div class="themes-menu">
  <a class="btn menu-themes{if $menu == 'themes'} active{/if}" href="{Yii::$app->urlManager->createUrl(['design/themes'])}">{$smarty.const.BACK_THEMES_LIST}</a>
  ...
  <a class="btn menu-theme-edit{if $menu == 'theme-edit'} active{/if}" href="{Yii::$app->urlManager->createUrl(['design/theme-edit', 'theme_name' => $theme_name])}">{$smarty.const.TEXT_CUSTOMIZE_THEME_STYLES}</a>
