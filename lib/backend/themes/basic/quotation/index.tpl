...
<div class="row order-box-list order-sc-text">
    <div class="col-md-12">
        <div class="widget-content dis_module">
            ...
            <table class="table table-striped table-selectable table-checkable table-hover table-responsive table-bordered datatable tabl-res double-grid table-orders" data_ajax="quotation/quotations-list" checkable_list="">
                <thead>
                    <tr>
...
function cancelStatement() {
    ...
    $.post("{$app->urlManager->createUrl('quotation/quotation-actions')}", { 'orders_id' : orders_id }, function(data, status){
        if (status == "success") {
...
//ajax save
    ...
    $.post("{$app->urlManager->createUrl('quotation/ordersubmit')}", $('#status_edit').serialize(), function(data, status){
        if (status == "success") {
            $('#order_management_data .scroll_col').html(data);
            $("#order_management").show();
    /*        
...
function deleteOrder() {
    $("#order_management").hide();
    $.post("{$app->urlManager->createUrl('quotation/orderdelete')}", $('#orders_edit').serialize(), function(data, status){
        if (status == "success") {
            resetStatement()
        } else {
            alert("Request error.");
...
function confirmDeleteOrder(orders_id) {
    $("#order_management").hide();
    $.post("{$app->urlManager->createUrl('quotation/confirmorderdelete')}", { 'orders_id' : orders_id }, function(data, status){
        if (status == "success") {
            $('#order_management_data .scroll_col').html(data);
            $("#order_management").show();
...
function reassignOrder(orders_id) {
    $("#order_management").hide();
    $.post("{$app->urlManager->createUrl('quotation/order-reassign')}", { 'orders_id' : orders_id }, function(data, status){
        if (status == "success") {
            $('#order_management_data .scroll_col').html(data);
            $("#order_management").show();
            switchOffCollapse('orders_list_collapse');
...
function confirmedReassignOrder() {
    $("#order_management").hide();
    $.post("{$app->urlManager->createUrl('quotation/confirmed-order-reassign')}", $('#orders_edit').serialize(), function(data, status){
        if (status == "success") {
            resetStatement()
        } else {
            alert("Request error.");
...
function createOrder(){
    $.post("{$app->urlManager->createUrl('quotation/createorder')}", $('form[name=create_order]').serialize(), function(data, status){
        if (status == "success") {
            switchOffCollapse('orders_list_collapse');
            $('#order_management_data .scroll_col').html(data);
            $("#order_management").show();
...
function createOrderProcess(){
    $.post("{$app->urlManager->createUrl('quotation/createorderprocess')}", $('form[name=create_order]').serialize(), function(data, status){
        if (status == "success") {
            switchOffCollapse('orders_list_collapse');
            $('#order_management_data .scroll_col').html(data);
            $("#order_management").show();
...
function loadCustomer(form){
    $.get("{$app->urlManager->createUrl('quotation/createorder')}", $(form).serialize(), function(data, status){
        if (status == "success") {
            $('#order_management_data .scroll_col').html(data);
            $("#order_management").show();
            switchOffCollapse('orders_list_collapse');
...
function invoiceSelectedOrders() {
    if (getTableSelectedCount() > 0) {
        ...
        form.target = "_blank";    
        form.method = "POST";
        form.action = 'quotation/ordersbatch?pdf=invoice&action=selected';
...
function packingslipSelectedOrders() {
    if (getTableSelectedCount() > 0) {
        ...
        form.target = "_blank";    
        form.method = "POST";
        form.action = 'quotation/ordersbatch?action=selected';
...
function exportSelectedOrders() {
    if (getTableSelectedCount() > 0) {
        ...
        form.target = "_blank";
        form.method = "POST";
        form.action = 'quotation/ordersexport';
        ...
        bootbox.dialog({
                ...
                buttons: {
                        success: {
                                ...
                                callback: function() {
                                    $.post("quotation/ordersdelete", { 'selected_ids' : selected_ids, 'restock' : '1' }, function(data, status){
                                        if (status == "success") {
                                            resetStatement();
                                        } else {
                                            alert("Request error.");
                        ...
                        danger: {
                                ...
                                className: "btn-delete",
                                callback: function() {
                                    $.post("quotation/ordersdelete", { 'selected_ids' : selected_ids, 'restock' : '0' }, function(data, status){
                                        if (status == "success") {
                                            resetStatement();
                                        } else {
                                            alert("Request error.");
...
$(document).ready(function(){
	...
	$( ".datepicker" ).datepicker({
        ...
        $('#selectState').autocomplete({
            // source: "quotation/state?country=" + $('#selectCountry').val(),
