...
<form action="{Yii::$app->urlManager->createUrl('product-designer/group_relations-update')}" method="post">
  ...
  <div class="after">
    <div class="attr-box attr-box-1">
      <div class="widget widget-attr-box box box-no-shadow" style="margin-bottom: 0;">
        <div class="widget-header">
          <h4>{$smarty.const.PRODUCTDESIGNER_FIND_ITEMS}</h4>
          <div class="box-head-serch after">
            <input type="search" id="element-search-by-products" placeholder="{$smarty.const.PRODUCTDESIGNER_SEARCH_BY_ITEM}" class="form-control">
    ...
    <div class="attr-box attr-box-3">
      <div class="widget-new widget-attr-box box box-no-shadow" style="margin-bottom: 0;">
        <div class="widget-header">
          <h4>{$smarty.const.PRODUCTDESIGNER_FIELDSET_ASSIGNED_ITEMS}</h4>
          <div class="box-head-serch after">
            <input type="search" id="search-elements-assigned" placeholder="{$smarty.const.PRODUCTDESIGNER_SEARCH_BY_ITEM}" class="form-control">
            <button onclick="return false"></button>
          </div>
        </div>
        <div class="widget-content">
...
<div class="btn-bar">
  <div class="btn-left"><a href="{Yii::$app->urlManager->createUrl(['product-designer/group'])}" class="btn btn-cancel-foot">{$smarty.const.IMAGE_CANCEL}</a></div>
...
<script type="text/javascript">
  function addSelectedElement() {
    $( 'select#element-search-products option:selected' ).each(function() {
      ...
      if ( $('input[name="item_id[]"][value="' + item_id + '"]').length ) {
        //already exist
      } else {
        $.post("{Yii::$app->urlManager->createUrl('product-designer/add-rel-item')}", { 'item_id': item_id }, function(data, status) {
          if (status == "success") {
  ...
  $(document).ready(function() {
    ...
    $('#element-search-by-products').on('focus keyup', function(e) {
      var str = $(this).val();
      $.post( "{Yii::$app->urlManager->createUrl('product-designer/get-items-list')}?q="+encodeURIComponent(str), function( data ) {
