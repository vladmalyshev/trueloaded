  ...
  <div class="row order-box-list">
    <div class="col-md-12">
      <div class="widget-content">     
        ...
        <table class="table table-striped table-bordered table-hover table-responsive table-checkable datatable table-suppliers" checkable_list="0" data_ajax="{$app->urlManager->createUrl('product-designer/template_list')}">
          <thead>
            <tr>
...
<script type="text/javascript">
  ...
  function onClickEvent(obj, table) {
    ...
    var template_id = $(obj).find('input[name=template_id]').val();
    if (global > 0) template_id = global;

    $.post("{$app->urlManager->createUrl('product-designer/template_statusactions')}", { 'template_id' : template_id }, function(data, status) {
            if (status == "success") {
    ...
    $('.table').find('input.cell_identify[value=' + template_id + ']').parents('tr').addClass('selected');
    global = '';
    url = window.location.href;
    if (url.indexOf('sID=') > 0) {
      url = url.replace(/sID=\d+/g, 'sID=' + template_id);
    } else {
      url += '?sID=' + template_id;
    }
    if (first) {
  ...
  function templateEdit(id) {
    deleteScroll();
    $("#suppliers_management").hide();
    $.get("{$app->urlManager->createUrl('product-designer/template_edit')}", { 'template_id' : id }, function (data, status) {
      if (status == "success") {
        $('#suppliers_management_data .scroll_col').html(data);
        $("#suppliers_management").show();
