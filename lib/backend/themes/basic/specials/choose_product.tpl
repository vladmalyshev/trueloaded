...
<div class="widget box box-wrapp-blue filter-wrapp">
  <div class="wl-td">
    <div class="customer_in auto-wrapp" style="position: relative; width: 100%;">
      <label accesskey="f" for="search_text">{$smarty.const.FIND_PRODUCTS}</label>
    {Html::textInput('product', '', ['class' => 'form-control', 'id' => 'search_text', 'tabindex'=> 1])}
    </div>
  </div>
</div>
        ...
        if (request.term.length > 2) {
          $.get("{\Yii::$app->urlManager->createUrl('stock-manufacturer/seacrh-product')}", {
            ...
            if (data.length) {
              ...
              response(
                new Array({
                  values: "{$smarty.const.TEXT_NOTHING_FOUND|escape:'html'}",
                  label: "{$smarty.const.TEXT_NOTHING_FOUND|escape:'html'}",
                  id: 0,
                })
              );
            }
