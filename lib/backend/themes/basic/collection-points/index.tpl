...
<div class="order-wrap">
    ...
    <div class="col-md-12">
      <div class="widget-content">
        ...
        {if {$messages|@count} > 0}
          ...
          {/foreach}
        {/if}
        <table class="table table-striped table-bordered table-hover table-responsive table-checkable table-selectable js-table-sortable datatable" checkable_list="0" data_ajax="collection-points/list">
          <thead>
          <tr>
  ...
  <script type="text/javascript">
      ...
      function onClickEvent(obj, table) {
          ...
          var collection_points_id = $(obj).find('input.cell_identify').val();
          $.post("collection-points/list-actions", { 'collection_points_id' : collection_points_id }, function(data, status){
              if (status == "success") {
      ...
      function itemEdit(id){
          $("#item_management").hide();
          $.get("collection-points/edit", { 'collection_points_id' : id }, function(data, status){
              if (status == "success") {
                  $('#item_management_data .scroll_col').html(data);
                  $("#item_management").show();
      ...
      function itemSave(id){
          $.post("collection-points/save?collection_points_id="+id, $('form[name=stock_indication]').serialize(), function(data, status){
              if (status == "success") {
      ...
      function itemDelete(id){
          if (confirm('{str_replace("'", "\'", $smarty.const.CONFIRM_DELETE_COLLECTION_POINT)}')){
              $.post("collection-points/delete", { 'collection_points_id' : id}, function(data, status){
                  if (status == "success") {
                      //$('.alert #message_plce').html('');
                      //$('.alert').show().removeClass('alert-error alert-success alert-warning').addClass(data['messageType']).find('#message_plce').append(data['message']);
                      if (data == 'reset') {
      ...
      $(document).ready(function(){
          $( ".js-table-sortable.datatable tbody" ).sortable({
              ...
              update: function( event, ui ) {
                  ...
                  $.post("{Yii::$app->urlManager->createUrl('collection-points/sort-order')}", post_data, function(data, status){
                      if (status == "success") {
                          resetStatement();
                      } else {
                          alert("Request error.");
