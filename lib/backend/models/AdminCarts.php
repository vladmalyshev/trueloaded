...
class AdminCarts extends Admin {
    ...
    public function createCart($instanceType, $order, $basket_id) {
        try {
            $cart = new $instanceType($order->orders_id);
            if ($order->orders_id) {
                $cartType = $this->getCartType($cart);
                $aCart = AdminShoppingCarts::find()
                                ->where(['admin_id' => $this->info['admin_id'], 'cart_type' => $cartType, 'order_id' => $order->orders_id])->one();
                if ($aCart) {
                    ...
                    if ($cart) {
                        ...
                        $this->setCurrentCartID($index, ($order->orders_id ? false : true));
                        return $cart;
                    }
                }
            }
    ...
    private function _getCartById($cartId) {
        if (preg_match("/(.*)\|([\d]*)\-([\d]*)/", $cartId, $mas)) {
            if (is_array($mas) && isset($mas[1])) {
                $index = $mas[1] . '|' . $mas[2] . '-' . $mas[3];
                return AdminShoppingCarts::find()->where(['admin_id' => $this->info['admin_id'], 'cart_type' => $mas[1], 'customers_id' => $mas[2], 'basket_id' => $mas[3]])->one();
            }
        }
        return false;
    }
    ...
    public function loadCustomersBaskets($type = 'cart', $full = true) {
        $this->carts = [];
        $cartsQuery = AdminShoppingCarts::find()
                        ->where(['admin_id' => $this->info['admin_id'], 'cart_type' => $type])->orderBy('updated_at DESC');
