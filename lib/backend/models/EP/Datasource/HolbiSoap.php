...
class HolbiSoap extends DatasourceBase
{
    ...
    public function update($settings)
    {
        ...
        try{
            ...
            if ( isset($config['status_map_local_to_server']) && is_array($config['status_map_local_to_server']) ) {
                Helper::putOrderStatusesOnServer($client, $config);
                if (preg_grep('/^create_on/', $config['status_map_local_to_server'])) {
    ...
    public function initRemoteData($configArray)
    {
        ...
        try{
            ...
            \Yii::error('HolbiSoap configure - initRemoteData exception '.$ex->getMessage().($client?"\nSoap Request:\n".$client->__getLastRequest()."\nSoap Response:\n".$client->__getLastResponse()."\n":''),'datasource');
        }
    }
