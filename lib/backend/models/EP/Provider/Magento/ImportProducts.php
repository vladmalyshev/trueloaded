    ...
    public function postProcess(Messages $message) {
        ...
        if (tep_db_num_rows($grouped)) { //do collections
            ...
            while ($row = tep_db_fetch_array($grouped)) {
                $collection = Collections::find()->where('collections_name = :name and language_id = :lid', [':name' => $row['remote_group_name'], ':lid' => $default_language])->one();
                if (!$collection) {
                    $cid = Collections::find()->max('collections_id') + 1;
