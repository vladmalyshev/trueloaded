        ...
        if ( count($this->site_modules)==0 ) {
            ...
            foreach ($MODULE_ORDER_TOTAL_INSTALLED as $MODULE_ORDER_TOTAL_) {
                ...
                if (empty($MODULE_ORDER_TOTAL_TITLE)) {
                    ...
                    if (is_file(DIR_FS_CATALOG . DIR_WS_LANGUAGES . '/modules/order_total/' . $MODULE_ORDER_TOTAL_)) {
                        include_once(DIR_FS_CATALOG . DIR_WS_LANGUAGES . '/modules/order_total/' . $MODULE_ORDER_TOTAL_);
                    }
                    try {
...
<div class="widget box box-no-shadow">
    ...
    protected function assignCustomer(\common\models\Orders $order)
    {
        ...
        if ($customer->isNewRecord){
            ...
            $customer->customers_password = \common\helpers\Password::encrypt_password(\common\helpers\Password::create_random_value(12));
    ...
    protected function persistCollectedOrder(Messages $message)
    {
        ...
        foreach ($importTotals as $__idx=>$_totalInfo){
            if ( strlen($_totalInfo['value_exc_vat'])>0 ) $importTotals[$__idx]['value_exc_vat'] = number_format($_totalInfo['value_exc_vat'],6,'.','');
            if ( strlen($_totalInfo['value_inc_tax'])>0 ) $importTotals[$__idx]['value_inc_tax'] = number_format($_totalInfo['value_inc_tax'],6,'.','');
            if ( strlen($_totalInfo['value'])>0 ) {
        ...
        if ( $isOrdersStatusChanged ){
            ...
            $statusHistory->admin_id = (int)$_SESSION['login_id'];
