...
class Categories extends ProviderAbstract implements ImportInterface, ExportInterface
{
    ...
    public function exportRow(){
        ...
        if( $this->withImages ) {
            ...
            foreach (['categories_image', 'categories_image_2'] as $imageColumn) {
    ...
    public function importRow($data, Messages $message)
    {
        ...
        if ($found_rows == 0) {
            ...
            foreach ($main_source['columns'] as $file_column => $db_column) {
                ...
                if (($file_column == 'categories_image' || $file_column == 'categories_image_2') && strlen($data[$file_column])>0){
                    $target_filename = $image_target_path . $data[$file_column];
                    if (!is_file($target_filename)){
                        if (is_file($image_path_import . $data[$file_column])){
                            if ( !is_dir( dirname($target_filename) ) ) {
                                try{
                                    FileHelper::createDirectory(dirname($target_filename),0777);
                                }catch (\Exception $ex){}
                            }
                            if (@copy($image_path_import . $data[$file_column], $target_filename)){
                                @chmod($target_filename,0666);
                            }
                        } else {
                            $message->info($file_primary_value . ' category image "'.\common\helpers\Output::output_string($data[$file_column]).'" not found');
                            $data[$file_column] = '';
                        }
                    }
                }
                $update_data_array[$db_column] = $data[$file_column];
            }
        ...
        foreach ($data_sources as $source_key => $source_data) {
            if ($source_data['table']) {
                ...
                foreach ($source_data['columns'] as $file_column => $db_column) {
                    if (!array_key_exists($file_column, $data)) continue;
                    $new_data[$db_column] = $data[$file_column];
                }
...
//            }
                ...
                if (tep_db_num_rows($data_sql_r) > 0) {
                    ...
                    if ( $source_data['table']==TABLE_CATEGORIES_DESCRIPTION ) {
                        if ($ext = \common\helpers\Acl::checkExtension('SeoRedirectsNamed', 'allowed')) {
                            $ext::trackProductLinks($categories_id, $insert_pk['language_id'], null, $new_data, $_old_data);
                        }
                    }
                } else {
                ...
                foreach ($source_data['columns'] as $file_column => $db_column) {
                    if (isset($export_columns[$db_column]['set']) && method_exists($this, $export_columns[$db_column]['set'])) {
                        call_user_func_array(array($this, $export_columns[$db_column]['set']), array($export_columns[$db_column], $this->data['categories_id']));
                    }
                }
            }
        }
