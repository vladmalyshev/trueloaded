...
class TrackOrders implements DatasourceInterface
{
    ...
    private $allow_update_order_products = true;
    private $allow_update_order_data = true;
    ...
    function __construct($config)
    {
        ...
        }elseif ( isset($this->config['order']['disable_order_update']) && $this->config['order']['disable_order_update']==2 ) {
            $this->allow_update_order_products = false;
            $this->allow_update_order_data = false;
        }
    }
    ...
    {
        $startFromDate = Helper::getKeyValue($this->config['directoryId'],'TrackOrders/lastOrderModifyGmt');
...
//$this->total_count = tep_db_num_rows($this->process_orders_r);
        ...
        while(true) {
            try {
                $response = $this->client->getOrdersInfo(
                    [
                        'searchCondition' => [
                            [
                                'operator' => '>=',
                ...
                if ($response && $response->status != 'ERROR') {
                    ...
                    if (is_array($orderInfoList)) {
                        foreach ($orderInfoList as $orderInfo) {
                            ...
                            }elseif(false && empty($orderInfo->client_order_id)){
    ...
    public function processRow(Messages $message)
    {
        ...
        if ( empty($data['orders_id']) ) {
            ...
            if ( $data['cfg_export_as']=='po_order' ) {
                $this->trackRemotePurchaseOrder($message, $data['orders_id'], $data['remote_orders_id']);
            }else {
    ...
    public function postProcess(Messages $message)
    {
        if ( $this->useModifyTimeCheck && !empty($this->startJobServerGmtTime) && !$this->isErrorOccurredDuringCheck ) {
            Helper::setKeyValue(
                ...
                'TrackOrders/lastOrderModifyGmt',
                $this->startJobServerGmtTime
            );
        }
    }
    ...
    protected function copyRemoteOrder(Messages $message, $remote_orders_id)
    {
        ...
        $orderData = false;
        try {
            $response = $this->client->getOrder($remote_orders_id);
            if ($response && $response->status != 'ERROR') {
    ...
    protected function trackRemoteOrder(Messages $message, $orders_id, $remote_orders_id)
    {
        ...
        try{
            ...
            if ( !$this->allow_update_order && isset($this->check_order_ids[$remote_orders_id]['info']) && is_object($this->check_order_ids[$remote_orders_id]['info']) ) {
    ...
    protected function trackRemotePurchaseOrder(Messages $message, $orders_id, $remote_orders_id)
    {
        ...
        try{
            ...
            if ($response && $response->status != 'ERROR') {
                $remoteOrderInfo = $response->order->info;
                $this->updateLocalOrderAsPurchase($message, $order, $response->order);
    ...
    protected function updateLocalOrderAsPurchase(Messages $message, $order, $orderData)
    {
    ...
    protected function updateLocalOrder(Messages $message, $order, $orderData)
    {
        ...
        if ($this->allow_update_order_data) {
            // check customer id
        ...
        if ( $this->allow_update_order_products ) {
            $order->products = [];
        ...
        if ( $insert_id ) {
            ...
            foreach ($missing_history_new_tracking_codes as $missing_history_new_tracking_code){
                tep_db_query("insert into tracking_numbers(orders_id,tracking_number) values('" . (int)$insert_id . "','".tep_db_input($missing_history_new_tracking_code)."')");
                $tracking_numbers_id = tep_db_insert_id();
                if ( $tracking_numbers_id ) {
                    $get_unshipped_products_r = tep_db_query(
                        "SELECT op.orders_id, op.orders_products_id, op.products_quantity, SUM(opt.products_quantity) as tracked_qty ".
                        "FROM orders_products op ".
                        " LEFT JOIN tracking_numbers_to_orders_products opt ON opt.orders_products_id=op.orders_products_id AND opt.orders_id=op.orders_id ".
                        "WHERE op.orders_id='" . (int)$insert_id . "' ".
                        "GROUP BY op.orders_products_id"
                    );
                    if ( tep_db_num_rows($get_unshipped_products_r)>0 ) {
                        while($_unshipped_product = tep_db_fetch_array($get_unshipped_products_r)){
                            $_track_qty = $_unshipped_product['products_quantity']-$_unshipped_product['tracked_qty'];
                            if ( $_track_qty>0 ) {
                                tep_db_perform('tracking_numbers_to_orders_products',[
                                    'tracking_numbers_id' => $tracking_numbers_id,
                                    'orders_id' => $_unshipped_product['orders_id'],
                                    'orders_products_id' => $_unshipped_product['orders_products_id'],
                                    'products_quantity' => $_track_qty,
                                ]);
                            }
                        }
                    }
    ...
    protected function updateLocalOrderPartial(Messages $message, $order, $remoteOrderInfo)
    {
        ...
        // {{ new tracking table asap addon
        foreach ($new_tracking_codes as $missing_history_new_tracking_code){
            tep_db_query("insert into tracking_numbers(orders_id,tracking_number) values('" . (int)$oID . "','".tep_db_input($missing_history_new_tracking_code)."')");
            $tracking_numbers_id = tep_db_insert_id();
            if ( $tracking_numbers_id ) {
                $get_unshipped_products_r = tep_db_query(
                    "SELECT op.orders_id, op.orders_products_id, op.products_quantity, SUM(opt.products_quantity) as tracked_qty ".
