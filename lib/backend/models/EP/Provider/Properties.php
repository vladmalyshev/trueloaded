...
class Properties extends ProviderAbstract implements ImportInterface, ExportInterface
{
    ...
    public function importRow($data, Messages $message)
    {
        ...
        $properties_type = $this->prop_data_id[$this->data['properties_id']]['properties_type'];
                ...
                if ( isset($this->prop_data_path[$walk_path_check]) ) {
                    ...
                    if ( tep_db_num_rows($lookup_r)>0 ) {
                        ...
                        tep_db_query(
                            ...
                            "SELECT '{$walk_property_id}', languages_id, '".tep_db_input(trim($path_chunk))."' FROM ".TABLE_LANGUAGES." "
                        );
                        $prop_data = array(
    ...
    {
        $type = $this->prop_data_id[$properties_id]['properties_type'];
        $prop_decimals = $this->prop_data_id[$properties_id]['decimals'];
        ...
        if (tep_db_num_rows($result_values_id) > 0) {
            ...
            if ($type == 'interval') {
                list($from, $upto) = explode('-', trim($value));
                tep_db_query(
                    "INSERT INTO " . TABLE_PROPERTIES_VALUES . " (values_id, properties_id, language_id, values_text, values_number, values_number_upto) ".
