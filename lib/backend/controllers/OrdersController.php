...
class OrdersController extends Sceleton {
    ...
    public function actionIndex() {
        ...
        $this->topButtons[] = '<a href="' . Yii::$app->urlManager->createUrl(['editor/order-edit', 'back' => 'orders']) . '" class="create_item"><i class="icon-file-text"></i>' . TEXT_CREATE_NEW_OREDER . '</a>';
        ...
        $markers = [];
        if ($ext = \common\helpers\Acl::checkExtension('OrderMarkers', 'getMarkersList')){
            $markers = $ext::getMarkersList(true);
        }
        $this->view->markers = $markers;
        $this->view->filters->marker = (int) $_GET['marker'];

        $flags = [];
        if ($ext = \common\helpers\Acl::checkExtension('OrderMarkers', 'getFlagsList')){
            $flags = $ext::getFlagsList(true);
        }
        $this->view->flags = $flags;
        $this->view->filters->flag = (int) $_GET['flag'];
        ...
        $this->view->filters->status_selected = $_GET['status'] ?? [];

        $this->view->filters->fcoupon = 'byId';
        $this->view->filters->fc_id = $_GET['fc_id'] ?? [];
        if (!empty($_GET['fc_code'])) {
          $this->view->filters->fc_code = htmlspecialchars($_GET['fc_code']);
        ...
        if (!empty($_GET['fp_from'])) { //summ
          $this->view->filters->fp_from = htmlspecialchars($_GET['fp_from']);
          $this->view->filters->fpFrom = true; //flag
        }
        if (!empty($_GET['fp_to'])) { //summ
          $this->view->filters->fp_to = htmlspecialchars($_GET['fp_to']);
          $this->view->filters->fpTo = true; //flag
        }
        $this->view->filters->fpClass = OrderHelper::getUsedTotalClassList($_GET['fp_class']??'');
        ...
        $this->view->filters->payments_selected = $_GET['payments'] ?? [];
        ...
        $this->view->filters->shipping_selected = $_GET['shipping'] ?? [];
    ...
    public function actionOrderlist() {
        ...
        if (isset($_GET['search']['value']) && tep_not_null($_GET['search']['value'])) {
            ...
            if ( is_numeric($keywords) ) {
                $quickFilter .= " or o.api_client_order_id LIKE '%" . tep_db_input($keywords) . "%' ";
            }
            $search_condition = " and (o.customers_lastname like '%" . $keywords . "%' or o.customers_firstname like '%" . $keywords . "%' or o.customers_email_address like '%" . $keywords . "%' or o.orders_id='" . $keywords . "' or op.products_model like '%" . tep_db_input($keywords) . "%' or op.products_name like '%" . tep_db_input($keywords) . "%' {$quickFilter} ) ";
        } else {
            $search_condition = "";
        }
        ...
        ;
        if ((tep_not_null($_GET['in_stock']) && $_GET['in_stock'] != '')){
            $_orders_products_joined = true;
            $orders_query_raw->leftJoin(TABLE_ORDERS_PRODUCTS . " op", "(op.orders_id = o.orders_id)");
            $orders_query_raw->addSelect("BIT_AND(" . (PRODUCTS_INVENTORY == 'True' ? "if(i.products_quantity is not null,if((i.products_quantity>=op.products_quantity),1,0),if((p.products_quantity>=op.products_quantity),1,0))" : "if((p.products_quantity>=op.products_quantity),1,0)") . ") as in_stock");
        ...
        if (tep_not_null($output['search'])) {
            ...
            switch ($output['by']) {
                ...
                case 'model': default:
                    ...
                    $orders_query_raw->andWhere(["like", "op.products_model", $search]);
                    break;
                case 'name':
                    if ( !$_orders_products_joined ) {
                        $_orders_products_joined = true;
                    ...
                    $orders_query_raw->andWhere(["like", "op.products_name", $search]);
                    break;
                case 'brand':
                    break;
                case 'fullname':
                    $orders_query_raw->andWhere(["like", "o.customers_name", $search]);
                    break;
                case 'email':
                    $orders_query_raw->andWhere(["like", "o.customers_email_address", $search]);
                    break;
                case 'tracking_number':
                    $orders_query_raw->andWhere(["like", "o.tracking_number", $search]);
                    break;
                case '':
                case 'any':
                    if ( !$_orders_products_joined ) {
                    ...
                    $orders_query_raw->andFilterWhere(['or',
                        ['o.orders_id' => $search],
                        ['like','op.products_model',$search],
                        ['like','op.products_name',$search],
                        ['like','o.customers_name',$search],
                        ['like','o.customers_email_address',$search],
                        ['like','o.tracking_number',$search],
                        ]);
                    break;
            }
        }
        ...
        if (is_array($output['fc_id']) && count($output['fc_id'])) {
        ...
        if (tep_not_null($output['fc_code'])) {
...
//echo $orders_query_raw->createCommand()->getRawSql()."\n\n";
        ...
        $markers = [];
        if ($ext = \common\helpers\Acl::checkExtension('OrderMarkers', 'getMarkers')){
            $markers = $ext::getMarkers();
        }

        $flags = [];
        if ($ext = \common\helpers\Acl::checkExtension('OrderMarkers', 'getFlags')){
            $flags = $ext::getFlags();
        }
            ...
            foreach($ordersAll as $orders){
                ...
                if (is_array($orders['ordersTotals']) && count($orders['ordersTotals'])){
                    foreach($orders['ordersTotals'] as $totals){
                        if (is_file(DIR_FS_CATALOG . DIR_WS_LANGUAGES . '/modules/order_total/' . $totals['class'] . '.php')) {
                            include_once(DIR_FS_CATALOG . DIR_WS_LANGUAGES . '/modules/order_total/' . $totals['class'] . '.php');
                        }
                ...
                if ($orders['customers_id']){
                    ...
                    '<a href="' . \Yii::$app->urlManager->createUrl(['customers/customeredit', 'customers_id' => $orders['customers_id']]) . '" title="' . strip_tags($orders['customers_name']) . '">' . self::cropStr($orders['customers_name'], 22) . '</a>':self::cropStr($orders['customers_name'], 22)) .
                    '</div><a href="mailto:' . $orders['customers_email_address'] . '" class="ord-name-email" title="' . strip_tags($customers_email_address) . '">' . self::cropStr($customers_email_address, 22) . '</a><div class="ord-location" style="margin-top: 5px;">' . $orders['customers_postcode'] . '<div class="ord-total-info ord-location-info"><div class="ord-box-img"></div><b>' . Html::encode($orders['customers_name']) . '</b>' . Html::encode($orders['customers_street_address']) . '<br>' . Html::encode($orders['customers_city'] . ', ' . $orders['customers_state']) . '&nbsp;' . Html::encode($orders['customers_postcode']) . '<br>' . $orders['customers_country'] . '</div></div>';
                } elseif ($orders['admin_id']) {
                    $cusColumn = '<div class="ord-name click_double" data-click-double="' . \Yii::$app->urlManager->createUrl(['orders/process-order', 'orders_id' => $orders['orders_id']]) . '">' . (defined('TEXT_WALKIN_ORDER')? TEXT_WALKIN_ORDER: '') . $orders['admin_firstname'] . ' ' . $orders['admin_lastname']. '</div>';
                }

                $orderRow = [];
                if ( $orders['hold_on_date'] ) {
                    $orderRow['DT_RowClass'] .= ' holdOnOrder';
                ...
                if ( isset($orders['isFraud']) && $orders['isFraud'] ) {
                    $orderRow['DT_RowClass'] .= ' fraudOrder';
                }
                ...
                if ($ext = \common\helpers\Acl::checkExtensionAllowed('OrderMarkers', 'allowed')) {
                    ...
                    $paint = '<div class="fa-paint-brush" onclick="sendOrderMarker(' . (int)$orders['orders_id'] . ', ' . (int)$orderMarkers['markers'] . ')"></div>';
                ...
                $orderRow[] = '<div class="ord-desc-tab click_double" data-click-double="' . \Yii::$app->urlManager->createUrl(['orders/process-order', 'orders_id' => $orders['orders_id']]) . '"><a href="' . \Yii::$app->urlManager->createUrl(['orders/process-order', 'orders_id' => $orders['orders_id']]) . '" class="order-inf"><span class="ord-id">' . TEXT_ORDER_NUM . $orders['orders_id'] . '</span> ' . $departmentInfo . (tep_not_null($orders['payment_method']) ? (SHOW_PRODUCTS_ON_ORDER_LIST === 'False' ? '<br>' : ' ') . TEXT_VIA . ' ' . strip_tags($orders['payment_method']) : '') . (tep_not_null($orders['shipping_method']) ? ' ' . TEXT_DELIVERED_BY . ' ' . strip_tags($orders['shipping_method']) : '') . '</a>' . (SHOW_PRODUCTS_ON_ORDER_LIST !== 'False' ? $p_list : '') . '</div>';
    ...
    public function actionProcessOrder() {
        ...
        $oQuery = $order->getARModel()->where(['orders_id' => $oID]);
        if (!$oQuery->exists()){
        ...
        if ($filter && defined('EXTENSION_HANDLERS_ENABLED') && EXTENSION_HANDLERS_ENABLED == 'True') {
...
/** @var \common\classes\Currencies $currencies*/
        ...
        */
        ...
        if ($update_paid_amount_flag && is_numeric($update_paid_amount) && $update_paid_amount) {
            ...
            if ($totals['ot_due'] && $totals['ot_paid']){
                ...
                $value = ($_POST['paid_prefix'] == '-' ? -$value: $value);
                $comment = $comments . " " . TEXT_PAID_AMOUNT . " " . $_POST['paid_prefix'] . $currencies->format($update_paid_amount, true, $order->info['currency'], 1);
...
// EOF: WebMakers.com Added: Downloads Controller
            ...
            if ($ext = \common\helpers\Acl::checkExtension('Ebay', 'setUpdateOrder')) {
                $ext::setUpdateOrder($oID);
            }
            if ($ext = \common\helpers\Acl::checkExtension('Amazon', 'setUpdateOrder')) {
                $ext::setUpdateOrder($oID);
            }
            ...
            while ($orders = tep_db_fetch_array($orders_query)) {
                ...
                if ($isInvoice && $invoices){
                    foreach($invoices as $invoice){
                        ...
                        $pages[] = ['name' => ($isInvoice ?'invoice': 'packingslip'),
                                    'params' => [
        ...
        if (!empty($search)){
            ...
            foreach ($cRep->search($search, [0,1], [0,1])->all() as $customer) {
                $customers[] = ['id' => $customer->customers_id,
                  'value' => $customer->customers_firstname . ' ' . $customer->customers_lastname . ' (' . $customer->customers_email_address . ')'
...
// }}
    ...
    public function actionSendRequest() {
        ...
        if ($customer_id){
            ...
            $update_and_pay_amount = number_format($ot_total_value, 2) - number_format($ot_paid_value, 2);
            ...
            if ($order->customer['email_address']){
                ...
                $order->addLegend('Sent payement Request', $_SESSION['login_id']);
 ...
 */
    ...
    public function actionPrintLabel()
    {
        ...
        list($new_module, $new_method) = explode('_', $new_module_method, 2);
        ...
        list($module, $method) = explode('_', $order->info['shipping_class']);
        ...
        if ($order->canBeDelivered() /*|| (is_object($shipping) && $shipping->hasLabelModule())*/) {
            ...
            if ($orders_label_id > 0) {
                $oLabel = \common\models\OrdersLabel::findOne(['orders_label_id' => $orders_label_id, 'orders_id' => $orders_id]);
                list($label_module, $label_method) = explode('_', $oLabel->label_class, 2);
            ...
            $auto_selected_label = '';
            if ($ext = \common\helpers\Acl::checkExtension('ShippingCarrierPick', 'allowed')) {
                $auto_selected_label = $ext::suggestOnOrder($order);
                if (!empty($auto_selected_label)) {
                    if (strpos($auto_selected_label, '_') !== false) {
                        list($auto_selected_label_class, $auto_selected_label_method) = explode('_', $auto_selected_label, 2);
    ...
    public function actionMakeOrderLabel()
    {
        ...
        $ext = \common\helpers\Acl::checkExtension('ShippingCarrierPick', 'allowed');
        if ( !$ext ) {
            Yii::$app->response->data = [
                'status' => 'error',
                        ...
                        if ($payment){
                            ...
                            if (!$transaction){
                                if ($tManager->addTransaction($transaction_id, 'undefined', 0, null, 'Manually assigned transaction')){
                                    $payment->getTransactionDetails($transaction_id, $tManager, true);
                                    $transaction = $tManager->getTransaction($transaction_id);
...
//                            $tManager->stopPropagination();
    ...
    public function actionMerge() {
        if ($ext = \common\helpers\Acl::checkExtension('MergeOrders', 'actionMerge')) {
    ...
    public function actionPaymentList()
    {
      Translation::init('admin/orders');
        ...
        foreach (\common\helpers\OrderPayment::getArrayByOrderId($oID) as $paymentRecord) {
            ...
            if (($paymentRecord['orders_payment_id_parent'] <= 0) AND in_array($paymentRecord['orders_payment_status'], [
        ...
        if ( extension_loaded('openssl') ) {
          ...
          if (count($due)>0) {
          $onBehalfUrl = tep_catalog_href_link('account/login-me', 'order_id=' . (int)$oID . '&payer=1&aup='.$aup);
          }
    ...
    public function actionPaymentSave()
    {
        ...
        if ($return['message'] == '') {
            ...
            if ($orderPaymentRecord->orders_payment_amount != $orderPaymentAmount
            ...
            try {
                ...
                $return['message'] = 'Error while updating Payment record!';
            }
        }
        if ($return['message'] != '') {
            $return['reload'] = 0;
