...
*/
    ...
    public function actionBackup(){
        echo '<div class="or_box_head">' . TEXT_INFO_HEADING_NEW_BACKUP . '</div>';
        echo '<div class="col_desc">' . TEXT_INFO_NEW_BACKUP . '</div>';
        echo tep_draw_form('backup', 'backup/backupnow' , tep_session_name() . '=' . tep_session_id());
        echo '<br>' . tep_draw_radio_field('compress', 'no', true) . ' ' . TEXT_INFO_USE_NO_COMPRESSION;
        if ($this->exec_gzip_available || extension_loaded('zlib')) echo '<br>' . tep_draw_radio_field('compress', 'gzip') . ' ' . TEXT_INFO_USE_GZIP;
        if ($this->exec_zip_available || extension_loaded('zip')) echo tep_draw_radio_field('compress', 'zip') . ' ' . TEXT_INFO_USE_ZIP;
        ...
        echo '<input type="submit" class="btn btn-primary" value="' . IMAGE_BACKUP . '">&nbsp;' . '<button class="btn btn-cancel"  onClick="return resetStatement();">' . IMAGE_CANCEL . '</button>';
        echo '</form>';
    }
