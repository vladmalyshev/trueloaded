...
class LanguagesController extends Sceleton {
    ...
    public function actionIndex() {
        ...
        $this->topButtons[] = '<a href="' . Url::to('languages/predefine') . '" class="create_item new-language">' . TEXT_INFO_HEADING_NEW_LANGUAGE . '</a>';
    ...
    public function actionLanguageActions() {
        ...
        if ($languages_id) {
            ...
            if (tep_not_null($lang)) {
                ...
                echo '</div>';
                //echo '<div class="smallText">' . TEXT_INFO_LANGUAGE_DIRECTORY .  DIR_WS_CATALOG_LANGUAGES . $lang['directory'] . '</div>';
...
//{{
                    ...
                    $categories_query = tep_db_query("select c.categories_id, cd.categories_name, cd.affiliate_id, cd.categories_seo_page_name, cd.google_product_category from " . TABLE_CATEGORIES . " c left join " . TABLE_CATEGORIES_DESCRIPTION . " cd on c.categories_id = cd.categories_id where cd.language_id = '" . (int) $languages_id . "'");
                    while ($categories = tep_db_fetch_array($categories_query)) {
                        tep_db_query("insert into " . TABLE_CATEGORIES_DESCRIPTION . " (categories_id, language_id, categories_name, affiliate_id, categories_seo_page_name, google_product_category) values ('" . (int) $categories['categories_id'] . "', '" . (int) $insert_id . "', '" . tep_db_input($categories['categories_name']) . "', '" . (int) $categories['affiliate_id'] . "', '" . tep_db_input($categories['categories_seo_page_name']) . "', '" . tep_db_input($categories['google_product_category']) . "')");
                    }
                    tep_db_free_result($categories_query);

                    // create additional products_description records
                    $products_query = tep_db_query("select p.products_id, pd.products_name, pd.products_description, pd.products_url, pd.platform_id, pd.products_seo_page_name, pd.google_product_category from " . TABLE_PRODUCTS . " p left join " . TABLE_PRODUCTS_DESCRIPTION . " pd on p.products_id = pd.products_id where pd.language_id = '" . (int) $languages_id . "'");
                    while ($products = tep_db_fetch_array($products_query)) {
                        tep_db_query("insert into " . TABLE_PRODUCTS_DESCRIPTION . " (products_id, language_id, products_name, products_description, products_url, platform_id, products_seo_page_name, google_product_category) values ('" . (int) $products['products_id'] . "', '" . (int) $insert_id . "', '" . tep_db_input($products['products_name']) . "', '" . tep_db_input($products['products_description']) . "', '" . tep_db_input($products['products_url']) . "', '" . (int) $products['platform_id'] . "', '" . tep_db_input($products['products_seo_page_name']) . "', '" . tep_db_input($products['google_product_category']) . "')");
                    }
                    ...
                    $settings_query = tep_db_query("select backup_id, box_id, setting_name, setting_value from " . TABLE_DESIGN_BOXES_SETTINGS_BACKUPS . " where language_id = '" . (int) $languages_id . "'");
                    while ($settings = tep_db_fetch_array($settings_query)) {
                        tep_db_query("insert into " . TABLE_DESIGN_BOXES_SETTINGS_BACKUPS . " (	backup_id, box_id, setting_name, setting_value, language_id ) values ('" . (int) $settings['backup_id'] . "', '" . (int) $settings['box_id'] . "', '" . tep_db_input($settings['setting_name']) . "', '" . tep_db_input($settings['setting_value']) . "', '" . (int) $insert_id . "')");
                    }
...
//        }}          
        ...
        $code = strtolower(tep_db_prepare_input($_POST['code']));
        $image = tep_db_prepare_input($_POST['image']);
        $image_svg = tep_db_prepare_input($_POST['image_svg']);
        $directory = tep_db_prepare_input($_POST['directory']);
        //$sort_order = tep_db_prepare_input($_POST['sort_order']);
        $status = tep_db_prepare_input($_POST['languages_status']);
        $locale = tep_db_prepare_input($_POST['locale']);
        ...
        if (is_array($_POST['configuration_key'])) {
            foreach ($_POST['configuration_key'] as $lang => $data) {
                ...
                foreach ($data as $key => $value) {
                    if (tep_not_null($value) && tep_not_null($_POST['configuration_value'][$lang][$key])) {
                        $sql_array = ['configuration_key' => tep_db_prepare_input($value),
                            'configuration_value' => tep_db_prepare_input($_POST['configuration_value'][$lang][$key]),
                            'language_id' => (int) $lang,
                        ];
                        tep_db_perform(TABLE_LANGUAGES_FORMATS, $sql_array);
                        if (tep_not_null($_POST['configuration_description'][$lang][$key])) {
        ...
        if ($lID) {
            ...
            tep_db_query("update " . TABLE_LANGUAGES . " set name = '" . tep_db_input($name) . "', code = '" . tep_db_input($code) . "', image = '" . tep_db_input($image) . "', image_svg = '" . tep_db_input($image_svg) . "', directory = '" . tep_db_input($directory) . "'/*, sort_order = '" . tep_db_input($sort_order) . "'*/, locale = '" . tep_db_input($locale) . "' where languages_id = '" . (int) $lID . "'");
            ...
            if (isset($_POST['flag'])) {
                if ($lID != \common\helpers\Language::get_default_language_id()/* DEFAULT_LANGUAGE */) {
                    if ($_POST['flag'] == 1) {
                        tep_db_query("update " . TABLE_LANGUAGES . " set languages_status = 1 where languages_id = '" . (int) $lID . "'");
                    } elseif ($_POST['flag'] == 0) {
        ...
        if ($lID) {
            ...
            if ($lng['languages_id'] == $lID) {
                ...
                if (tep_db_num_rows($code_query)) {
                    ...
                    tep_db_query("delete from " . TABLE_DESIGN_BOXES_SETTINGS_BACKUPS . " where language_id = '" . (int) $lID . "'");
