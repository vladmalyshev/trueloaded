...
class Geo_zonesController extends Sceleton {
    ...
    public function actionConfirmcategorydelete() {
        ...
        echo tep_draw_form('zones', FILENAME_GEO_ZONES, 'zpage=' . $_GET['zpage'] . '&zID=' . $zInfo->geo_zone_id . '&action=deleteconfirm_zone', 'post', 'id="option_delete" onsubmit="return deleteCategory();"');
    ...
    public function actionProductedit() {
        ...
        echo '<div class="row_or row_or_block"><label>' . TEXT_INFO_COUNTRY_ZONE . '</label>' . tep_draw_pull_down_menu('zone_id', \common\helpers\Zones::prepare_country_zones_pull_down($sInfo->zone_country_id), $sInfo->zone_id, 'class="form-control"').'</div>';
        echo '<div class="row_or row_or_block"><label>' . TEXT_POSTCODE_START . '</label>' . Html::textInput('postcode_start', $sInfo->postcode_start, ['class'=>'form-control']).'</div>';
        echo '<div class="row_or row_or_block"><label>' . TEXT_POSTCODE_END . '</label>' . Html::textInput('postcode_end', $sInfo->postcode_end, ['class'=>'form-control']).'</div>';
        echo '<div class="btn-toolbar btn-toolbar-order">' . tep_draw_hidden_field( 'association_id', $sInfo->association_id ) . tep_draw_hidden_field( 'geo_zone_id', $sInfo->geo_zone_id ) . '<button class="btn btn-no-margin">' . IMAGE_UPDATE . '</button><input type="button" class="btn btn-cancel" value="' . IMAGE_CANCEL . '" onClick="return resetStatement()"></div>';
        echo '</form>';
    }
