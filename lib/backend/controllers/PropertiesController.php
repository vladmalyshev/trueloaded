...
class PropertiesController extends Sceleton {
  ...
  public function actionList() {
    ...
    $properties_query_raw = "select p.properties_id, p.properties_type, {$selectColumns} pd.properties_name, pd.properties_image, p.date_added, p.last_modified from " . TABLE_PROPERTIES . " p, " . TABLE_PROPERTIES_DESCRIPTION . " pd where p.properties_id = pd.properties_id and pd.language_id = '" . (int)$languages_id . "' and parent_id = '" . (int)$filter['parID'] . "' " . $search . " order by (p.properties_type = 'category') desc, p.sort_order, pd.properties_name";
  ...
  function actionEdit()
  {
    ...
    $properties_values_query = tep_db_query("select values_id, properties_id, language_id, values_text, values_number, values_number_upto, values_alt, values_seo_page_name as values_seo, values_color, values_image, values_prefix, values_postfix, maps_id, sort_order from " . TABLE_PROPERTIES_VALUES . " where properties_id = '" . (int)$properties_id . "' order by sort_order, " . ($properties['properties_type'] == 'number' || $properties['properties_type'] == 'interval' ? 'values_number' : 'values_text'));
    while($properties_values = tep_db_fetch_array($properties_values_query)) {
      ...
      if ($properties_values['maps_id']) {
        if ($map = \common\models\ImageMaps::findOne($properties_values['maps_id'])) {
  ...
  function actionSave()
  {
    ...
    for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
      ...
      if (trim($properties_color[$languages[$i]['id']]) == '' || $same_all_languages) $properties_color[$languages[$i]['id']] = $properties_color[$default_language_id];
