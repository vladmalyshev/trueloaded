...
class EasypopulateController extends Sceleton
{
    ...
    public function actionIndex()
    {
        ...
        */
        ...
        $view_data = array(
            ...
            'dataSourcesActive' => !is_null(Yii::$app->request->get('datasources')),
    ...
    function actionProcessExport()
    {
        ...
        if ( $job_id ) {
            ...
            if (true) {
                if ( $format=='ZIP' ) {
                    ...
                    $extension = 'zip';
                }elseif(strpos($format,'XML')!==false){    /*ZRADA - Adding headers for your format /**/
            ...
            if ( $format=='ZIP' && $job->job_provider!='product\catalog' ) {
        ...
        if ( $format=='ZIP' && $exportJob->job_provider!='product\catalog' ) {
    ...
    public function actionImportConfigure()
    {
        ...
        if ( $job_record instanceof EP\JobZipFile ){
            ...
            if ( $job_record->job_provider == 'orders\orders' ) {
                die;
            }
    ...
    public function actionEmpty()
    {
        if ($_POST['products']){
          ...
          $schemaCheck = Yii::$app->get('db')->schema->getTableSchema('google_pagespeed_urls');
          if ( $schemaCheck ) {
              tep_db_query("TRUNCATE TABLE google_pagespeed_urls");
          }
        ...
        if ($_POST['customers'] == 1){
          ...
          tep_db_query("TRUNCATE TABLE promotions_customer_codes");
    ...
    {
        if ((int)$encode > 0) {
            return @base64_encode(bzcompress(json_encode($data), 9));
        } else {
            $return = [];
            try {
                $return = @json_decode(bzdecompress(base64_decode($data)), true);
            } catch (\Exception $exc) {}
