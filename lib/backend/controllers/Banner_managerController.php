...
class Banner_managerController extends Sceleton
{
    ...
    public function actionIndex()
    {
      ...
      <label>' . TEXT_COMMON_PLATFORM_FILTER . '</label><div class="f_row"><div class="f_td f_td_radio ftd_block"><label class="radio_label"><input type="checkbox" name="platform[]" class="js_platform_checkboxes uniform" value="">' . TEXT_COMMON_PLATFORM_FILTER_ALL . '</label></div></div>';
            foreach (\common\classes\platform::getList(false, true) as $platform) {
    ...
    public function actionView()
    {
        ...
        if (is_object($bInfo)) {
            ...
            echo '<div class="btn-toolbar btn-toolbar-order"><a class="btn btn-no-margin btn-edit" href="' . tep_href_link('banner_manager/banneredit', 'banners_id=' . $bInfo->banners_id) . '">' . IMAGE_EDIT . '</a><button class="btn btn-delete"  onClick="return deleteItemConfirm(' . $bInfo->banners_id . ');">' . IMAGE_DELETE . '</button></div>';
        }
    }
    ...
    public function actionSubmit()
    {
        ...
        if (!empty($_POST['expires_date'])) {
            $expires_date = \common\helpers\Date::prepareInputDate($_POST['expires_date'], true);
        }
        ...
        if (!empty($_POST['date_scheduled'])) {
            $date_scheduled = \common\helpers\Date::prepareInputDate($_POST['date_scheduled'], true);
        }
    ...
    public function actionBanneredit()
    {
        ...
        $groups_query = tep_db_query("select distinct banners_group from " . TABLE_BANNERS_NEW . " order by banners_group");
        while ($groups = tep_db_fetch_array($groups_query)) {
            if ($uniqueElements[$groups['banners_group']]) continue;
            $groups_array[] = array('id' => $groups['banners_group'], 'text' => $groups['banners_group']);
        ...
        foreach ($groups as $group) {
            if ($uniqueElements[$group['banners_group']]) continue;
    ...
    function actionGetSvg()
    {
        ...
        if (!$banner['banners_image']) {
            return '<svg width="640" height="480" xmlns="http://www.w3.org/2000/svg" xmlns:svg="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"></svg>';
        }
  ...
  <image width="' . $size[0] . '" height="' . $size[1] . '" id="svg_1" x="0" xlink:href="' . HTTP_CATALOG_SERVER . DIR_WS_CATALOG_IMAGES . $banner['banners_image'] . '" y="0"/>
    ...
    public function actionBannerGroupImages()
    {
        ...
        foreach ($languages as $language) {
            ...
            $response[$language['id']] = $this->render('banner-group-images.tpl', [
                'banners_group' => $banners_group,
