...
class CountriesController extends Sceleton  {
    ...
    public function actionIndex() {
        ...
        if (NSHelper::anyConfigured()) {
            $params['ns_countries'] = NSHelper::getCountriesList();
        }
    ...
    public function actionEdit(){
      ...
      if (\common\helpers\Acl::checkExtensionAllowed('UploadCustomerId', 'allowed')) {
        ...
        echo tep_draw_checkbox_field('is_customer_id_required', 1, $cInfo->is_customer_id_required) . '<span>' . TEXT_CUSTOMER_ID_REQUIRED . '</span>';
        echo '</div>';
      }
    ...
    public function actionNsSyncUpdateId() {
        if (\common\helpers\Acl::rule(['BOX_HEADING_CATALOG', 'BOX_CATALOG_EASYPOPULATE'])) {
            ...
            if ($d_id>0 && !empty($n_id) && !empty($n_id) && !empty($n_id) ) {
              NSHelper::setKeyValue($d_id, $l_id, $n_id);
              $ret = ['status'=>"OK"];
            } else {
