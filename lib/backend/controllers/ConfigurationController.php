...
class ConfigurationController extends Sceleton {
    ...
    public function actionIndex()
    {
        ...
        if ($ext = \common\helpers\Acl::checkExtension('WeddingRegistry', 'allowed')){
            $ext::initConfiguration();
        }

        if ($ext = \common\helpers\Acl::checkExtension('OrderMarkers', 'initConfiguration')){
            $ext::initConfiguration();
        }
    ...
    public function actionPreedit()
    {
        ...
        <div class="btn-toolbar btn-toolbar-order">
            ...
            if ( tep_db_num_rows($check_platform_query) > 0 ) {
                while ($check_platform = tep_db_fetch_array($check_platform_query)) {
                    echo $check_platform['configuration_value'] . '<br>';
                }
    ...
    public function actionGetparam()
    {
        ...
        if( (string) $configuration['set_function']  && (method_exists ('backend\models\Configuration', $method) || method_exists ('backend\models\Configuration', strtolower($method)) ) ) {
        ...
        echo tep_draw_form(
            ...
            tep_draw_hidden_field( 'group_id', $configuration['configuration_group_id']  ) .
