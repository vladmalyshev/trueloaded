...
class WarehousesController extends Sceleton {
    ...
    public function actionIndex() {
        ...
        $this->topButtons[] = '<a href="' . Yii::$app->urlManager->createUrl('warehouses/edit') . '" class="create_item addprbtn"><i class="icon-tag"></i>' . TEXT_CREATE_NEW_WAREHOUSE . '</a>';
        $this->topButtons[] = '<a href="' . Yii::$app->urlManager->createUrl('warehouses/location-blocks') . '" class="create_item">' . TEXT_MANAGE_LOCATION_BLOCKS . '</a>';
    ...
    public function actionEdit() {
        ...
        $this->navigation[] = array('link' => Yii::$app->urlManager->createUrl('warehouses/'), 'title' => $text_new_or_edit . ' ' . $pInfo->warehouses_name);
        $this->selectedMenu = array('catalog', 'warehouses');
        ...
        foreach ($_platforms as $_idx => $_platform) {
            ...
            $_platforms[$_idx]['status'] = $check['status'];
        }
    ...
    public function actionSubmit() {
        ...
        $status = (int) Yii::$app->request->post('status', $platform_status[\common\classes\platform::defaultId()]);
        ...
        if ($error === FALSE) {
            ...
            if ($item_id > 0) {
                ...
                tep_db_perform(TABLE_WAREHOUSES, $sql_data_array, 'update', "warehouse_id = '" . (int) $item_id . "'");
                $warehouse_updated = true;
            } else {
                // Insert
                $message = "Item inserted";
                $sql_data_array['date_added'] = 'now()';
                tep_db_perform(TABLE_WAREHOUSES, $sql_data_array);
        ...
        if (\common\classes\platform::isMulti(false)) {
            ...
            foreach ($_platforms as $_idx => $_platform) {
                ...
                if ($check['warehouse_id'] == $item_id) {
                    ...
                    tep_db_query("INSERT INTO " . TABLE_WAREHOUSES_TO_PLATFORMS . " SET warehouse_id = '" . (int) $item_id . "', platform_id = '" . (int) $_platform['id'] . "', status = '" . (int) $platform_status[$_platform['id']] . "'");
                }
            }
        } else {
    ...
    private function locationTree($locations, $parrent_id = 0, $blocksList) {
        $tree = [];
    ...
    public function actionLocations() {
        ...
        $locations = \common\models\Locations::find()->where(['warehouse_id' => $item_id])->asArray()->all();
    ...
    private function updateLocation($data, $parrent_id, $warehouse_id) {
        if (!is_array($data)) {
            return false;
        }
        if (count($data) != 4) {
        ...
        if (is_array($children) && count($children) > 0) {
            ...
            foreach ($children as $child) {
                $this->updateLocation($child, $obj->location_id, $warehouse_id);
            }
        } else {
            $obj->is_final = 1;
            $obj->save();
    ...
    public function actionSaveLocation() {
        ...
        if (is_array($locations[0])) {
            ...
            foreach ($locations[0] as $location) {
                $this->updateLocation($location, 0, $item_id);
            }
    ...
    public function actionLocationBlocksEdit() {
        ...
        echo '<div class="main_row"><div class="main_title">' . TEXT_INFO_BLOCK_NAME . '</div><div class="main_value">' . tep_draw_input_field('block_name', $block->block_name) . '</div></div>';
