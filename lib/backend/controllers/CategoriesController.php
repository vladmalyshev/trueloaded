...
class CategoriesController extends Sceleton {
    ...
    public function __construct(
        $id,
        $module = null,
        ProductsNotesService $productsNotesService,
        ProductsDocumentsService $productsDocumentsService,
        array $config = []
    )
    {
...
//         }
    ...
    public function actionIndex() {
        ...
        $this->topButtons[] = '<a href="' . Yii::$app->urlManager->createUrl(['categories/productedit', 'bundle' => '1']) . '" class="js_create_new_product create_item addprbtn"><i class="icon-cubes"></i>' . TEXT_CREATE_NEW_BUNDLE . '</a>';
        ...
        if (false === \common\helpers\Acl::rule(['BOX_HEADING_FRONENDS'])) {
            $filter_by_platform = [];
...
//2do check " for extra escapeing  echo $p->createCommand()->rawSql; die;
        ...
        $this->view->filters->type_listing = (int) $_GET['type_listing'];
        $this->view->filters->type_not_listing = (int) $_GET['type_not_listing'];
        $this->view->filters->sub_children = (int) $_GET['sub_children'];
        ...
        $this->view->categoriesClosedTree = array_diff(array_map('intval', explode('|', $_COOKIE['closed_data'])), $this->view->categoriesOpenedTree);
...
//            $filter_prod .= ' and p.products_id IN (SELECT products_id FROM '.TABLE_PLATFORMS_PRODUCTS.' WHERE platform_id IN(\''.implode("','",$filter_by_platform).'\'))  ';
        ...
        if (tep_not_null($output['type_listing'])) {
            $onlyProducts = true;
            $filter_prod .= " and p.is_listing_product = '1' ";
        }

        if (tep_not_null($output['type_not_listing'])) {
            $onlyProducts = true;
            $filter_prod .= " and p.is_listing_product = '0' ";
        }

        if (tep_not_null($output['sub_children'])) {
            $onlyProducts = true;
        ...
        if (!empty($filter_prod) || !empty($filter_cat)) {
            ...
            if (!$onlyProducts) {
                ...
                if ($remind_page_number == $current_page_number) {// all categories showed, now show only products
                    while ($categories = tep_db_fetch_array($categories_query)) {
                        $responseList[] = array(
                            '<input type="checkbox" class="'. ($categoriesQty < CATALOG_SPEED_UP_DESIGN ? 'uniform' : '' ) .' js-cat-batch" name="batch[]" value="c_'.$categories['categories_id'].'">',
                ...
                if ($rowsCounter < $length) {
                    ...
                    while ($products = tep_db_fetch_array($products_query)) {
                        ...
                        $responseList[] = array(
                            ...
                            '<span class="prodIDsC"><span title="' . \common\helpers\Output::output_string($products['products_model']) . '">' . ( $products['products_model'] ? TEXT_SKU . ': ' . $products['products_model'] . '<br>' : '') . TEXT_PRODUCTS_QUANTITY_INFO . ': ' . $products['products_quantity'] . '<br>' . TABLE_HEADING_ID . ': ' . $products['products_id'] . '</span>' . $productMarkers.'</span>' .
                            '<input class="cell_identify" type="hidden" value="' . $products['products_id'] . '"><input class="cell_type" type="hidden" value="product">' .
                            '</div>' .
                            '</div>',
                            ($products['products_status'] == 1 ? '<input type="checkbox" value="' . $products['products_id'] . '" name="products_status" class="check_on_off" checked="checked">' : '<input type="checkbox" value="' . $products['products_id'] . '" name="products_status" class="'. ($productsQty < CATALOG_SPEED_UP_DESIGN ? 'check_on_off' : 'check_on_off_check' ) .'">')
                        );
             ...
             */
            $products_query_raw = "select p.products_id, p.is_listing_product, p.sub_product_children_count, p.parent_products_id, p.products_model, ".ProductNameDecorator::instance()->listingQueryExpression('pd','pdd')." as products_name, p.products_status, p.manual_control_status, p.products_image, p.products_quantity from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd, " . TABLE_PRODUCTS_DESCRIPTION . " pdd, " . TABLE_PRODUCTS_TO_CATEGORIES . " p2c where p.products_id = pd.products_id and pd.language_id = '" . (int) $languages_id . "' and p.products_id = pdd.products_id and pdd.language_id = '" . \common\helpers\Language::get_default_language_id() . "' and p.products_id = p2c.products_id " . $search . " and pd.platform_id = '".intval(\common\classes\platform::defaultId())."' and pdd.platform_id = '".intval(\common\classes\platform::defaultId())."' and pd.department_id=0 and pdd.department_id=0 and p2c.categories_id = '" . (int) $current_category_id . "' {$platform_filter_products} order by " . $orderByProduct;
            ...
            $products_query_raw = "select *, ".ProductNameDecorator::instance()->listingQueryExpression('pd','')." as products_name from " . TABLE_PRODUCTS . " p " . (intval($output['brand_id']) == -1 ? " left join " . TABLE_MANUFACTURERS . " m ON m.manufacturers_id=p.manufacturers_id " : '') . " left join " . TABLE_PRODUCTS_DESCRIPTION . " pd on (p.products_id = pd.products_id and pd.language_id='" . intval($languages_id) . "') where pd.platform_id = '".intval(\common\classes\platform::defaultId())."' " . (intval($output['brand_id']) > 0 ? " and manufacturers_id = '" . intval($output['brand_id']) . "' " : (intval($output['brand_id']) == -1 ? ' and m.manufacturers_id IS NULL' : '')) . $ff . " {$platform_filter_products} group by p.products_id ORDER BY " . $order;
    ...
    public function actionProductactions() {
        ...
        echo '<div class="row_or">
                    ...
                    <div>' . number_format($pInfo->localRating[0]->average_rating, 2) . '%</div>
             </div>';
        echo '<div class="row_or">
...
/* @var $ext \common\extensions\ProductEasyView\ProductEasyView */
        if ($ext = \common\helpers\Acl::checkExtension('ProductEasyView', 'adminAdctionProduct')) {
        ...
        if (\common\helpers\Acl::rule(['TABLE_HEADING_PRODUCTS', 'IMAGE_COPY_TO'])) {
            if (isset($_POST['products_id']) && isset($_POST['categories_id'])) {
                ...
                if ($_POST['copy_as'] == 'link') {
                    ...
                    \common\helpers\Product::duplicate($products_id, $categories_id, true);
                }
...
//2do if _left and _right is updated everywhere - replace wth 2 queries.
          ...
          set_time_limit(0);
          $sdn = \common\helpers\Acl::checkExtension('SeoRedirectsNamed', 'allowed');
          for ($i = 0, $n = sizeof($categories); $i < $n; $i++) {
      ...
      $this->view->categoriesTree = $this->getCategoryTree();
      $this->view->categoriesOpenedTree = [];
      $this->view->categoriesClosedTree = array_map('intval', explode('|', $_COOKIE['closed_data']));
        ...
        if ($products_id > 0) {
          ...
          $pInfo = new \objectInfo($product);
            ...
            $pInfo = new \objectInfo(array_merge($parent_data,[
                ...
                'without_inventory' => (defined('PRODUCTS_INVENTORY_DEFAULT_DISABLED') && PRODUCTS_INVENTORY_DEFAULT_DISABLED=='True')?1:0,
...
//// extensions
        ...
        if ($ext = \common\helpers\Acl::checkExtension('ProductTemplates', 'productedit')) {
...
//improve - 1 query
      ...
      ///Description
        ...
        if (count($platforms) > 1){
            foreach($platforms as $platform){
                if (!$this->view->sphl[$platform->platform_id] && !$platform->is_marketplace){
            ...
            if (!$this->view->sphl[$def_platformId]) {
...
/// assigned attributes
          ...
          $this->view->suppliers = [];
          $service->get('\common\models\SuppliersProducts', 'sProduct');
          if (!\common\helpers\Attributes::has_product_attributes($pInfo->products_id ) || $pInfo->without_inventory){
...
//// image map
        if ($pInfo->maps_id) {
            $map = \common\models\ImageMaps::findOne($pInfo->maps_id);
            $mapImage = $map->image;
            ...
            if ($properties_id > 0) {
                ...
                foreach ($values_array[$key] as $values_key =>$values_id) {
                    if ($values_id > 0) {
                        ...
                        $properties_hiddens .= tep_draw_hidden_field('val_extra[' . $properties_id . '][]', $extra_values[$key][$values_key]);
                        $values_ids[$properties_id][] = $values_id;
                        $val_extra[$properties_id][] = $extra_values[$key][$values_key];
                    }
                }
            }
        }
...
////currencies tabs and params
        ...
        while ($values = tep_db_fetch_array($values_query)) {
          if ($products_options_id != $values['products_options_id']) {
            if ($products_options_id) {
              ...
              if (!$inventoryDisable && ($ext = \common\helpers\Acl::checkExtension('Inventory', 'getProductNewOption')) && PRODUCTS_INVENTORY == 'True' && !$is_virtual_option) {
    ...
    public function actionProductInventoryBox() {
        ...
        if (($ext = \common\helpers\Acl::checkExtension('Inventory', 'productInventoryBox')) && PRODUCTS_INVENTORY == 'True') {
...
////currencies tabs and params
        ...
        if (!$inventoryDisable && ($ext = \common\helpers\Acl::checkExtension('Inventory', 'productAttributesBox')) && PRODUCTS_INVENTORY == 'True') {
...
////currencies tabs and params
    ...
    //// groups tabs and params
        ...
        while ($values = tep_db_fetch_array($values_query)) {
            ...
            if (!$inventoryDisable && ($ext = \common\helpers\Acl::checkExtension('Inventory', 'getProductNewAttribute')) && PRODUCTS_INVENTORY == 'True' && !$is_virtual_option) {
    ...
    public function actionProductNewImage($id) {
        ...
        // {{
        $use_external_images = !!Yii::$app->request->get('external',0);
        $_ext_images_array = [];
    ...
    public function actionProductSubmit() {
        ...
        if ($TabAccess->tabDataSave('TAB_BUNDLES')) {
            if (PRODUCTS_BUNDLE_SETS == 'True') {
                $sql_data_array['is_bundle'] = tep_db_prepare_input($_POST['is_bundle']);
                //$sql_data_array['products_sets_price'] = tep_db_prepare_input($_POST['products_sets_price']);
                $sql_data_array['use_sets_discount'] = tep_db_prepare_input($_POST['use_sets_discount']);
                $sql_data_array['products_sets_discount'] = tep_db_prepare_input($_POST['products_sets_discount']);
                $sql_data_array['products_sets_price_formula'] = tep_db_prepare_input($_POST['products_sets_price_formula']);
            }
        }
...
////////////////////////////////////
            ...
            if ($ext = \common\helpers\Acl::checkExtension('UserGroupsRestrictions', 'saveProduct')) {
         ...
         */
        $languages = \common\helpers\Language::get_languages();
        if  ($TabAccess->tabDataSave('TEXT_NAME_DESCRIPTION')) {
...
////////////////////////////////
        ...
        if ($TabAccess->tabDataSave('TEXT_VIDEO')) {
            $productVideos = new \backend\models\ProductEdit\SaveProductVideos($productModel);
            $productVideos->save();
        }
         ...
         * Handlers
         */
        if ($ext = \common\helpers\Acl::checkExtension('Handlers', 'productSave')) {
         ...
         * linked ext
         */
        if ($TabAccess->tabDataSave('TEXT_LINKED_PRODUCTS')) {
            if ($extClass = \common\helpers\Acl::checkExtensionAllowed('LinkedProducts', 'allowed')){
                $extClass::productSave($productModel);
            }
        }

        /**
         * Obsolete Products ext
         */
        if ($TabAccess->tabDataSave('TAB_OBSOLETE_PRODUCTS')) {
         ...
         * Documents
         */
        if ($TabAccess->tabDataSave('TAB_DOCUMENTS')) {
        ...
        if ($TabAccess->tabDataSave('TEXT_MAIN_DETAILS')) {
            if ($ext = \common\helpers\Acl::checkExtension('ProductTemplates', 'productSubmit')) {
        ...
        // save post modification
        $productModel->save(false);

        if ($es = \common\helpers\Acl::checkExtensionAllowed('EventSystem', 'allowed')){
            $es::programme()->exec('saveProgramme', [$products_id, Yii::$app->request->post()]);
        }
        ...
        if ($ext && $ext::isEnabled()) {
          $ext::reindex($productModel->products_id);
        }
        ...
        if ($extP && $extP::isEnabled()) {
          $extP::reindex($productModel->products_id);
        }
    ...
    public function actionProductSearch() {
        ...
        if ($this->defaultCollapsed) {
          ...
          if ($bundle_skip > 0 ) {
            $pQ->andWhere(" p.is_bundle = 0 and p.manual_stock_unlimited = 0");
          }
          if ($linked_skip > 0 ) {
          ...
          foreach ($categories as $category) {
              $pQ = (new \yii\db\Query())
                  ...
                  ->andWhere(['p2c.categories_id' => (int)$category['id']])
                  ;
    ...
    public function actionCategoryedit() {
        ...
        if ($categories_id > 0) {
            ...
            $category['maps_id'] = 0;
            $category['google_product_type'] = 0;
        }
        $cInfo = new \objectInfo($category);

        if ($category['maps_id']) {
            $map = \common\models\ImageMaps::findOne($category['maps_id']);
            if ($map) {
                $mapImage = $map->image;
                $mapTitle = $map->getTitle($languages_id);
            }
        ...
        $pSettings[0] = [
          ...
          'categories_image_2' => $cInfo->categories_image_2,
          'categories_image_3' => $cInfo->categories_image_3,
          'show_on_home' => $cInfo->show_on_home,
        ...
        for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
            ...
            $cDescription[$i]['categories_description'] = tep_draw_textarea_field('categories_description[' . $languages[$i]['id'] . ']', 'soft', '70', '15', $categoryDescription->categories_description, 'class="ckeditor form-control" id="txt_category_description_'.$languages[$i]['id'].'"');
            ...
            $cDescription[$i]['noindex_option'] = tep_draw_checkbox_field('noindex_option[' . $languages[$i]['id'] . ']', '1', $categoryDescription->noindex_option == 1, '', 'class="check_on_off"');
            $cDescription[$i]['nofollow_option'] = tep_draw_checkbox_field('nofollow_option[' . $languages[$i]['id'] . ']', '1', $categoryDescription->nofollow_option == 1, '', 'class="check_on_off"');
            $cDescription[$i]['rel_canonical'] = tep_draw_input_field('rel_canonical[' . $languages[$i]['id'] . ']', $categoryDescription->rel_canonical, 'class="form-control form-control-small"');
            ...
            $cDescription[$i]['categories_h1_tag'] = tep_draw_input_field('categories_h1_tag[' . $languages[$i]['id'] . ']', $categoryDescription->categories_h1_tag, 'class="form-control"');
            $cDescription[$i]['categories_h2_tag'] = $categoryDescription->categories_h2_tag; //tep_draw_input_field('categories_h2_tag[' . $languages[$i]['id'] . ']', $categoryDescription->categories_h2_tag, 'class="form-control"');
            $cDescription[$i]['categories_h3_tag'] = $categoryDescription->categories_h3_tag; //tep_draw_input_field('categories_h3_tag[' . $languages[$i]['id'] . ']', $categoryDescription->categories_h3_tag, 'class="form-control"');
            $cDescription[$i]['categories_image_alt_tag_mask'] = tep_draw_input_field('categories_image_alt_tag_mask[' . $languages[$i]['id'] . ']', $categoryDescription->categories_image_alt_tag_mask, 'class="form-control"');
            $cDescription[$i]['categories_image_title_tag_mask'] = tep_draw_input_field('categories_image_title_tag_mask[' . $languages[$i]['id'] . ']', $categoryDescription->categories_image_title_tag_mask, 'class="form-control"');
        }

        $categories_google_producttype_query = tep_db_query("select category_name from " . TABLE_GOOGLE_CATEGORIES . " where categories_id = '" . $category['google_product_type'] . "' and language_id = '" . (int) $languages[$i]['id'] . "'");
        $categories_google_producttype = tep_db_fetch_array($categories_google_producttype_query);
        $category_description['google_product_type'] = $categories_google_producttype['category_name'] ?? '';
        $this->view->google_product_type = 
            \common\helpers\GoogleCategories::drawProductCategoryInputs(\Yii::$app->settings->get('languages_id'), $category_description, $category);           
        ...
        if ( $categories_id == 0 ) {
            $editCategoryInPath = (defined('TEXT_CATEGORY_CREATE_IN')?TEXT_CATEGORY_CREATE_IN:'').' '.'<ul class="category_path_list top_bead-items"><li class="category_path">'.\common\helpers\Categories::output_generated_category_path($cInfo->parent_id,'category','<span class="category_path__location">%2$s</span>','</li><li class="category_path onemore">').'</li></ul>';
        }else{
        ...
        foreach (\common\classes\platform::getList(false) as $frontend) {
            if ($this->view->platform_assigned[$frontend['id']]) {
                if ($seo_url['categories_seo_page_name']) {
                    $this->view->preview_link[] = [
...
/* platform settings main tab*/
        if (!empty($post['plaformsettings']) && is_array($post['plaformsettings'])) {
          foreach ($post['plaformsettings'] as $platform_id => $ps) {
            ...
            foreach (['gallery' => '', 'hero' => '_2', 'homepage' => '_3'] as $imageType => $mod) {
                ...
                $delete_image = $post['delete_image' . $mod][$platform_id];
                ...
                if ($delete_image || $categories_image_loaded != '') {
                    $removeImage = ($platform_id == 0 ? $category->$categoriesImage: (empty($ps->$categoriesImage) ? '' : $ps->$categoriesImage));
        ...
        if ($ext = \common\helpers\Acl::checkExtension('AutomaticallyStatus', 'onCategorySave')) {
        ...
        $update_sql_data = [
            'last_modified' => 'now()',
            'google_product_type' => (int)Yii::$app->request->post('google_product_type_id', 0),
        ];
        ...
        for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
            ...
            $sql_data_array = [
                ...
                'noindex_option' => (int)$noindex_option[$language_id],
                'nofollow_option' => (int)$nofollow_option[$language_id],
                ...
                'categories_image_alt_tag_mask' => tep_db_prepare_input($categories_image_alt_tag_mask[$language_id]),
                'categories_image_title_tag_mask' => tep_db_prepare_input($categories_image_title_tag_mask[$language_id]),
                'google_product_category' => tep_db_prepare_input($google_product_category[$language_id]),
            ];
...
// }}
        if ($ext = \common\helpers\Acl::checkExtension('UserGroupsRestrictions', 'saveCategory')) {
            $ext::saveCategory($categories_id);
        }
    ...
    public function actionGetgooglecategories()
    {
        $term = tep_db_prepare_input(trim(Yii::$app->request->get('term')));
        $with = Yii::$app->request->get('with','');
        $languageCode = Yii::$app->request->get('language_id','');
        
        $language_id = !$languageCode ? \Yii::$app->settings->get('languages_id') : \common\helpers\Language::get_language_id($languageCode)['languages_id'];
        $search = "1";        
        if ($term) {
            $search = "category_name like '%" . tep_db_input($term) . "%'";
            if (intval($term)) {
                $search = "(".$search." OR categories_id like '" . intval($term) . "%')";
            }
        } else {
            echo json_encode([]);
            exit();
        ...
        $googleCategories = [];
        if ( !empty($with) ) {            
            $googleCategories_query = tep_db_query("select categories_id as id, category_name as `value`, parent_id "
                ."from " . TABLE_GOOGLE_CATEGORIES . " where " . $search . " AND language_id='".$language_id."' AND categories_status!=0 order by category_name LIMIT 30");
            while ($response = tep_db_fetch_array($googleCategories_query)) {
                $response['text'] = \common\helpers\GoogleCategories::getCategoryHierarchy($response['parent_id'], $response['value'],$language_id);
                $googleCategories[] = $response;
            }           
        } else {
            $googleCategories_query = tep_db_query("select category_name, parent_id  from " . TABLE_GOOGLE_CATEGORIES 
                . " where " . $search . " AND language_id='".$language_id."' AND categories_status!=0 order by category_name LIMIT 30");
            while ($response = tep_db_fetch_array($googleCategories_query)) {
                $googleCategories[] = \common\helpers\GoogleCategories::getCategoryHierarchy($response['parent_id'], $response['category_name'],$language_id);
            }
        }
        echo json_encode($googleCategories);
    }
    
    public function actionGetupdatedgooglecategories()
    {
        $languages_id = \Yii::$app->settings->get('languages_id');      
        if (Yii::$app->request->isPost && Yii::$app->request->get('categories_id')!== null) {
            echo \common\helpers\GoogleCategories::getGoogleCategoriesDropdowns(Yii::$app->request->get('categories_id'),$languages_id)[0];
        }
    }
    ...
    public function actionBrandedit() {
        ...
        if ($manufacturers['maps_id']) {
          if ($map = \common\models\ImageMaps::findOne($manufacturers['maps_id'])) {
...
// {{ Filters
        if ($ext = \common\helpers\Acl::checkExtension('ProductPropertiesFilters', 'saveManufacturer')) {
            $ext::saveManufacturer($manufacturers_id);
        }
// }}
...
<script type="text/javascript">
  $("#document_list tbody tr").each(function(i){
    ...
    public function actionStockHistory() {
        $this->layout = false;
        if ($ext = \common\helpers\Acl::checkExtension('ProductStockHistory', 'actionStockHistory')) {
    ...
    public function actionWarehousesRelocate() {
        ...
        return $this->renderAjax('warehouses-relocate', ['warehouses' => $warehouses, 'prid' => $prid, 'suppliers_id' => $suppliers_id]);
    }
    ...
    public function actionProductAssets() {
        ...
        if ($ext = \common\helpers\Acl::checkExtension('ProductAssets', 'adminProductPopup')) {
            return $ext::adminProductPopup($prid, $action);
        }
    }
    ...
    public function actionEasyView(){
        $this->layout = false;
        if ($ext = \common\helpers\Acl::checkExtension('ProductEasyView', 'adminActionEasyView')) {
    ...
    public function getCompetitorsData(){
        return [
            'competitors' => \common\models\Competitors::find()->asArray()->all(),
            'competitors_table_header' => \common\models\CompetitorsProducts::getTableFields(),
            'competitors_products' => [],
        ];
    }

    public function actionCompetitorProductsList(){
        $pID = Yii::$app->request->get('pID', null);
        $draw = Yii::$app->request->get('draw', 0);
        $competitors_products = [];
        $our_price = 0;
        if ($pID){
            $competitors_products = \common\models\CompetitorsProducts::find()->where('products_id = :pid', [':pid' => (int)$pID])->with('competitor')->all();
            $isUprid = strpos($pID, '{') !== false;
            $product = \common\models\Products::find()->where("products_id = '" . (int)$pID . "'")->one();
            if (PRODUCTS_INVENTORY == 'True' ){
                $inventory = \common\models\Inventory::find()->where('products_id =:prid', [':prid' => $pID])->one();
            }
            $priceInstance = \common\models\Product\Price::getInstance($pID);
            if ($isUprid){
                $our_price = $priceInstance->getInventoryPrice(['qty' => 1]);
            } else {
                $our_price = $priceInstance->getProductPrice(['qty' => 1]);
            }
            $our_price = \common\helpers\Tax::add_tax_always($our_price, \common\helpers\Tax::get_tax_rate($product->products_tax_class_id));
        }
        $data = [];

        $currencies = Yii::$container->get('currencies');

        if ($competitors_products){
            foreach($competitors_products as $cp){
                $admin = new \backend\models\Admin($cp->admin_id);
                $competitor_price = $currencies->format_clear($cp->products_price, true, $cp->competitor->competitors_currency);
                $div_class = '';
                if ($our_price){
                    switch($competitor_price){
                        case $competitor_price ==0:$div_class = '';break;
                        case $competitor_price > $our_price: $div_class = 'green';break;
                        case $competitor_price == $our_price: $div_class = 'yellow';break;
                        case $competitor_price < $our_price: $div_class = 'red';break;
                    }
                }
                $data[] = [
                    $cp->competitor->competitors_name . '<input type="hidden" value="' . $cp->competitors_products_id . '" name="pcID">',
                    $cp->products_model,
                    $cp->products_name,
                    '<a href="'.$cp->products_url.'" target="_blank">'.$cp->products_url.'</a>',
                    '<a href="'.rtrim($cp->competitor->competitors_site, '/'). '/' . ltrim($cp->products_url_short, '/').'" target="_blank">' . (mb_strlen($cp->products_url_short)>50?mb_substr($cp->products_url_short, 0, 50):$cp->products_url_short) . '</a>',
                    $cp->competitor->competitors_currency,
                    '<div class="color_price" data-class="'.$div_class.'">' . $currencies->format($cp->products_price, false, $cp->competitor->competitors_currency) . '</div>',
                    //mb_substr(htmlspecialchars($cp->products_price_mask), 0, 20) . (mb_strlen($cp->products_price_mask)>20?'...':''),
                    \common\helpers\Date::date_short($cp->last_modified),
                    $admin->getInfo('admin_firstname') . ' ' . $admin->getInfo('admin_lastname'),
                    '<a href="' .Yii::$app->urlManager->createUrl(['categories/edit-competitor-product', 'pID' => $pID, 'pcID' => $cp->competitors_products_id ]). '" class="new-product-popup icon-pencil"></a><div class="del-pt"></div>'
                ];
            }
        }
    ...
    public function actionEditCompetitorProduct(){
        $pID = Yii::$app->request->get('pID');
        $pcID = Yii::$app->request->get('pcID', 0);
        \common\helpers\Translation::init('admin/competitors');
        \common\helpers\Translation::init('admin/categories');
        if ($pcID){
            $cProduct = \common\models\CompetitorsProducts::find()->where('competitors_products_id = :id', [':id' => $pcID])->one();
        }
        if (!$cProduct){
            $cProduct = new \common\models\CompetitorsProducts();
        }
        $competitors = [];

        if ($pID){
            $competitors = \common\models\CompetitorsProducts::find()->select('competitors_id')
                       ->where('products_id = :products_id', [':products_id' => (int)$pID])->asArray()->all();

           if ($competitors){
               $competitors = \yii\helpers\ArrayHelper::getColumn($competitors, 'competitors_id');
           }
        }
        $all_competitors = \common\models\Competitors::find()->asArray()->all();

        if ($all_competitors && $competitors && !$pcID){
            $all_competitors = array_filter($all_competitors, function($item) use($competitors) {
                if (!in_array($item['competitors_id'], $competitors)){
                    return true;
                } else return false;
            });
        }
        if (is_array($all_competitors) && count($all_competitors)){
            return $this->renderAjax('productedit/product-competitor-edit.tpl', [
                'pID' => $pID,
                'cProduct' => $cProduct,
                'competitors' => $all_competitors,
                'fields' => \common\models\CompetitorsProducts::getTableFields(),
            ]);
        } else {
            $this->layout = false;
            return $this->renderContent( "<script>$('.popup-box-wrap:last').remove();wrapMessage('".TEXT_ADD_NEW_COMPETITOR."');</script>");
        }
    }

    public function actionSaveCompetitorProduct(){
        global $login_id;
        $pID = Yii::$app->request->post('pID');
        $pcID = Yii::$app->request->post('pcID', 0);
        \common\helpers\Translation::init('admin/competitors');
        \common\helpers\Translation::init('admin/categories');
        if ($pcID){
            $cProduct = \common\models\CompetitorsProducts::find()->where('competitors_products_id = :id', [':id' => $pcID])->one();
        ...
        $competitors_id = Yii::$app->request->post('competitors_id');
        $products_model = Yii::$app->request->post('products_model');
        $products_name = Yii::$app->request->post('products_name');
        $products_url = Yii::$app->request->post('products_url');
        $products_url_short = Yii::$app->request->post('products_url_short');
        $products_price = (float)Yii::$app->request->post('products_price');
        $products_price_mask = Yii::$app->request->post('products_price_mask');

        $response = ['error' => false, 'message' => ''];
        if ($cProduct && $pID){
            $cProduct->setAttributes([
                'competitors_id' => $competitors_id,
                'products_id' => (int)$pID,
                'products_model' => $products_model,
                'products_name' => $products_name,
                'products_url' => $products_url,
                'products_url_short' => $products_url_short,
                'products_price' => $products_price,
                'products_price_mask' => $products_price_mask,
                'admin_id' => $login_id,
                'status' => 1,
            ], false);
            if ($cProduct->save(false)){
                $d_language = \common\helpers\Language::get_default_language_id();
                $keyword_tag = \common\models\ProductsDescription::find()->select('products_head_keywords_tag, products_id, platform_id, language_id')
                        ->where(['products_id' => (int)$pID, 'language_id' => $d_language, 'platform_id' => \common\classes\platform::defaultId() ])->one();
                if ($keyword_tag){
                    if (!preg_match("/". preg_quote($products_model)."/", $keyword_tag->products_head_keywords_tag)){
                        $keyword_tag->products_head_keywords_tag .= " " . $products_model;
                    }
                    if (!preg_match("/". preg_quote($products_name)."/", $keyword_tag->products_head_keywords_tag)){
                        $keyword_tag->products_head_keywords_tag .= " " . $products_name;
                    }
                    if ($keyword_tag->getOldAttribute('products_head_keywords_tag') != $keyword_tag->products_head_keywords_tag)
                        $keyword_tag->save(false);
                }

                if (\common\helpers\Acl::rule(['TABLE_HEADING_PRODUCTS', 'IMAGE_EDIT', 'TEXT_SEO'])) {
                    if (empty($products_url_short)){
                        $competitor = \common\models\Competitors::find()->where('competitors_id =:cid', [':cid' => $competitors_id])->one();
                        if ($competitor){
                            $products_url_short = ltrim(str_replace($competitor->competitors_site, "", $products_url), "/");
                        }
                        if (preg_match("/http.*/", $products_url_short)) $products_url_short = '';
                    }
                    if (!empty($products_url_short)){
                        if ($ext = \common\helpers\Acl::checkExtension('SeoRedirectsNamed', 'allowed')){
                            $ext::saveProductLinks((int)$pID, [
                                'products_old_seo_page_name_list' =>[
                                    $d_language => [
                                        $products_url_short
                                    ]
                                ]
                ...
                $response = ['error' => false, 'message' => TEXT_PRODUCT_UPDATED_NOTICE];
            } else {
    ...
    public function actionDeleteCompetitorProduct(){
        $pcID = Yii::$app->request->post('pcID', 0);
        if ($pcID){
            $cProduct = \common\models\CompetitorsProducts::find()->where('competitors_products_id = :id', [':id' => $pcID])->one();
            if ($cProduct){
    ...
    public function actionEasySave(){
        if ($ext = \common\helpers\Acl::checkExtension('ProductEasyView', 'adminActionEasySave')) {
    ...
    public function actionWarehouseLocation() {
        ...
        if (isset($existLocations['location_id']) && $existLocations['location_id'] > 0) {
            ...
            if (count($backBuild) > 1) {
                ...
                foreach ($backBuild as $backKey =>$backItem) {
                    ...
                    $locations = \common\models\Locations::find()->where(['warehouse_id' => $warehouse_id, 'parrent_id' => $backItem['parrent_id']])->asArray()->all();
        ...
        $locations = \common\models\Locations::find()->where(['warehouse_id' => $warehouse_id, 'parrent_id' => 0])->asArray()->all();
        if (is_array($locations) && count($locations) > 0) {
            $locationList = [];
            $locationList[''] = PULL_DOWN_DEFAULT;
            foreach ($locations as $location) {
    ...
    public function actionWarehouseLocationChild() {
        ...
        $locations = \common\models\Locations::find()->where(['warehouse_id' => $warehouse_id, 'parrent_id' => $location_id])->asArray()->all();
        if (is_array($locations) && count($locations) > 0) {
        ...
        foreach (\common\helpers\Product::getAllocatedTemporaryArray($uProductId) as $opTemporaryRecord) {
            if ((int)$opTemporaryRecord['customers_id'] > 0) {
                ...
                if (is_array($customerRecord) AND isset($customerRecord['customers_lastname'])) {
                    $opTemporaryRecord['customer_name'] = trim(trim($customerRecord['customers_firstname']) . ' ' . trim($customerRecord['customers_lastname']));
                }
