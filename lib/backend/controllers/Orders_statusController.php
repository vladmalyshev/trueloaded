...
class Orders_statusController extends Sceleton  {
    ...
    public function actionIndex() {
        ...
        $this->topButtons[] = '<a href="' . \Yii::$app->urlManager->createUrl(['orders_status/edit', 'type_id' => $type_id]) . '" class="create_item">' . TEXT_INFO_HEADING_NEW_ORDERS_STATUS . '</a>';
    ...
    public function actionEdit() {
        ...
        if ($orders_status_id == 0 OR $osgRecord instanceof \common\models\OrdersStatusGroups) {
            ...
            if ($orders_status_id == 0 OR $osgRecord->orders_status_type_id == \common\helpers\PurchaseOrder::getStatusTypeId()) {
                $osOesListPointer = &$osOesList;
                if (is_array($osOesList)) {
                    $stList = \common\helpers\Status::getStatusTypeList();
                    unset($osOesList[0]);
                    $osOesList = [0 => '', $stList[\common\helpers\Order::getStatusTypeId()] => $osOesList, $stList[\common\helpers\PurchaseOrder::getStatusTypeId()] => []];
                    $osOesListPointer = &$osOesList[$stList[\common\helpers\PurchaseOrder::getStatusTypeId()]];
                    unset($stList);
                } else {
                    $osOesListPointer = [0 => ''];
                }
                foreach (\common\helpers\PurchaseOrder::getEvaluationStateArray() as $poesId => $poesArray) {
            ...
            $oInfo->orders_status_send_ga = (int)$osgRecord->orders_status_groups_send_ga;
        }
        unset($osgRecord);

        $comment_templates['selected'] = $oInfo->comment_template_id;
    ...
    public function actionSave()
    {
        ...
        if ($order_evaluation_state_id !== false) {
            ...
            if ($osgRecord instanceof \common\models\OrdersStatusGroups) {
                ...
                } elseif ($osgRecord->orders_status_type_id == \common\helpers\PurchaseOrder::getStatusTypeId()) {
                    $esArray = \common\helpers\PurchaseOrder::getEvaluationStateArray();
                }
        ...
        if (\common\helpers\Acl::checkExtension('OrderStatusRules', 'orderStatusSave')){
    ...
    {
        if (\common\helpers\Acl::checkExtension('OrderStatusRules', 'orderStatusAction')){
