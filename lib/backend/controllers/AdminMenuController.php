...
class AdminMenuController extends Sceleton {
    ...
    public function actionIndex() {
        ...
        $this->topButtons[] = '<a href="'.Yii::$app->urlManager->createUrl('admin-menu/reset').'" onclick="return confirm(\'' . TEXT_RESET_TO_DEFAULT . '\')" class="create_item"><i class="icon-refresh"></i>' . TEXT_RESET . '</a>';
        $this->topButtons[] = '<a href="'.Yii::$app->urlManager->createUrl('admin-menu/export-menu').'" class="create_item backup"><i class="icon-file-text"></i>' . TEXT_EXPORT . '</a>';
        $this->topButtons[] = '<a href="javascript:void(0)" class="btn-import create_item backup"><i class="icon-file-text"></i>' . TEXT_IMPORT . '</a>';
        $this->topButtons[] = '<a href="' . Yii::$app->urlManager->createUrl(['admin-menu/add']) . '" class="create_item create_item_popup">' . IMAGE_ADD . '</a>';
    ...
    public function actionSavePopup(){
      ...
      echo json_encode(['status' => 'ok', 'val' => $langPost, 'message' => TEXT_MESSEAGE_SUCCESS]);
    }
