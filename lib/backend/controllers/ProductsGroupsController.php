...
class ProductsGroupsController extends Sceleton {
    ...
    public function actionIndex() {
        ...
        $this->topButtons[] = '<a href="#" class="create_item" onclick="return productsGroupEdit(0)">' . IMAGE_NEW_PRODUCTS_GROUP . '</a>';
...
//            $image = \common\helpers\Image::info_image($products_groups['products_groups_image'], $products_groups['products_groups_name'], 50, 50);
    ...
    public function actionEdit() {
        ...
        echo tep_draw_form('products_group', FILENAME_PRODUCTS_GROUPS . '/save', 'products_groups_id=' . $eInfo->products_groups_id, 'post', 'onsubmit="return productsGroupSave(' . ($eInfo->products_groups_id ? $eInfo->products_groups_id : 0) . ');"');
        ...
        $languages = \common\helpers\Language::get_languages();
        for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
          $products_group_inputs_string .= '<div class="langInput">' . $languages[$i]['image'] . tep_draw_input_field('products_groups_name[' . $languages[$i]['id'] . ']', \common\helpers\Product::products_groups_name($eInfo->products_groups_id, $languages[$i]['id'])) . '</div>';
        }
...
*/
        echo '<div class="btn-toolbar btn-toolbar-order">';
        if ($products_groups_id) {
            echo '<input type="submit" value="' . IMAGE_UPDATE . '" class="btn btn-no-margin"><input type="button" value="' . IMAGE_CANCEL . '" class="btn btn-cancel" onclick="resetStatement(' . (int)$eInfo->products_groups_id . ')">';
        } else {
            echo '<input type="submit" value="' . IMAGE_NEW . '" class="btn btn-no-margin"><input type="button" value="' . IMAGE_CANCEL . '" class="btn btn-cancel" onclick="resetStatement(' . (int)$eInfo->products_groups_id . ')">';
        }
...
*/
    ...
    public function actionProducts() {
        ...
        $groupProducts = [];
        $query = tep_db_query("select p.products_id, p.products_status, pd.products_name from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_id = p.products_id and p.products_id = pd.products_id and pd.language_id = '" . (int)$languages_id . "' and pd.platform_id = '".intval(\common\classes\platform::defaultId())."' and p.products_groups_id = '" . (int)$eInfo->products_groups_id . "' order by pd.products_name");
...
//          $this->layout = false;
    ...
    public function actionProductSearch() {
        ...
        foreach ($categories as $category) {
            $products_query = tep_db_query("select distinct p.products_id, pd.products_name, p.products_status from " . TABLE_PRODUCTS . " p LEFT JOIN " . TABLE_PRODUCTS_TO_CATEGORIES . " p2c ON p.products_id = p2c.products_id LEFT JOIN " . TABLE_PRODUCTS_DESCRIPTION . " pd ON p.products_id = pd.products_id where p2c.categories_id = '" . $category['id'] . "' and pd.language_id = '" . (int) $languages_id . "' and p.products_groups_id = '0' and (p.products_model like '%" . tep_db_input($q) . "%' or pd.products_name like '%" . tep_db_input($q) . "%') group by p.products_id order by p.sort_order, pd.products_name limit 0, 100");
    ...
    public function actionNewProduct() {
        ...
        $query = tep_db_query("select p.products_id, pd.products_name, p.products_status from " . TABLE_PRODUCTS_DESCRIPTION . " pd," . TABLE_PRODUCTS . " p where language_id = '" . (int)$languages_id . "' and platform_id = '".intval(\common\classes\platform::defaultId())."' and p.products_id = '" . (int)$products_id . "' and p.products_id =  pd.products_id limit 1");
