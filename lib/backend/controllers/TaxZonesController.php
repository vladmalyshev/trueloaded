...
class TaxZonesController extends Sceleton {
    ...
    public function actionConfirmcategorydelete() {
        ...
        echo tep_draw_form('zones', 'geo_zones', 'zpage=' . (int)$_GET['zpage'] . '&zID=' . $zInfo->geo_zone_id . '&action=deleteconfirm_zone', 'post', 'id="option_delete" onsubmit="return deleteCategory();"');
    ...
    public function actionProductedit() {
        ...
        echo '<div class="btn-toolbar btn-toolbar-order">' . tep_draw_hidden_field( 'association_id', $sInfo->association_id ) . tep_draw_hidden_field( 'geo_zone_id', $sInfo->geo_zone_id ) . tep_draw_hidden_field( 'association_id', $sInfo->association_id ) . '<button class="btn btn-no-margin">' . IMAGE_UPDATE . '</button><input type="button" class="btn btn-cancel" value="' . IMAGE_CANCEL . '" onClick="return resetStatement()"></div>';
        echo '</form>';
    }
