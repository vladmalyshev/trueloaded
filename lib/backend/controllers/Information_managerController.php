  ...
  public function actionCreate()
  {
    ...
    foreach( $platforms as $platform ) {
      ...
      foreach ($languages as $i => $_language) {
        ...
        $page_data[$i]['c_h2_tag'] = $data['information_h2_tag']; //tep_draw_input_field('information_h2_tag[' . $_lang_id . ']['.$platform['id'].']', isset($data['information_h2_tag']) ? $data['information_h2_tag'] : '', 'class="form-control"');
        $page_data[$i]['c_h3_tag'] = $data['information_h3_tag']; //tep_draw_input_field('information_h3_tag[' . $_lang_id . ']['.$platform['id'].']', isset($data['information_h3_tag']) ? $data['information_h3_tag'] : '', 'class="form-control"');
      }
  ...
  public function actionEdit()
  {
    ...
    foreach( $platforms as $platform ) {
      ...
      foreach ($languages as $i => $_language) {
        ...
        $page_data[$i]['c_seo_page_name'] = tep_draw_input_field('seo_page_name[' . $_lang_id . ']['.$platform['id'].']', isset($data['seo_page_name']) ? $data['seo_page_name'] : '', 'class="form-control"');
        $page_data[$i]['noindex_option'] = tep_draw_checkbox_field('noindex_option[' . $_lang_id . ']['.$platform['id'].']', '1', $data['noindex_option'] == 1, '', 'class="check_on_off"');
        $page_data[$i]['nofollow_option'] = tep_draw_checkbox_field('nofollow_option[' . $_lang_id . ']['.$platform['id'].']', '1', $data['nofollow_option'] == 1, '', 'class="check_on_off"');
        $page_data[$i]['rel_canonical'] = tep_draw_input_field('rel_canonical[' . $_lang_id . ']['.$platform['id'].']', $data['rel_canonical'], 'class="form-control form-control-small"');
...
//    echo tep_draw_hidden_field('information_id', "$information_id");
    ...
    foreach ($platforms as $item){
      ...
      if ($mapsId) {
          $map = \common\models\ImageMaps::findOne($mapsId);
          $mapImage = $map->image;
  ...
  public function actionPageSave(){
    ...
    if ($info_id > 0) {
      ...
      Information::updateAdditionalField((int)$_POST['information_id']);
