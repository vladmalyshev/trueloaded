...
class UpdateOrderResponse extends SoapModel
{
    ...
    public function setOrder(Order $orderData)
    {
        ...
        if ($orderData->client_order_id) {
            ...
            if ( !ServerSession::get()->acl()->siteAccessPermission() ) {
                $ownCheck =
                    (ServerSession::get()->getDepartmentId()?"AND department_id='".(int)ServerSession::get()->getDepartmentId()."' ":'').
        ...
        $order = $manager->getOrderInstanceWithId('\common\classes\Order',$this->updateOrderId);
        $orderOriginal = clone $order;
        if (ServerSession::get()->getDepartmentId()) {
            $order->info['department_id'] = ServerSession::get()->getDepartmentId();
        }elseif (ServerSession::get()->getPlatformId() && empty($order->info['platform_id'])) {
...
//        }
        ...
        foreach (array_keys($order->info) as $key) {
            if ( ServerSession::get()->getDepartmentId() && $key=='department_id' ) {
        ...
        if ( !ServerSession::get()->getDepartmentId() && ServerSession::get()->getPlatformId() && isset($infoData['platform_name']) && !empty($infoData['platform_name']) && $order->info['platform_id'] ) {
    ...
    protected function persistOrder()
    {
        ...
        $insert_id = $this->order->save_order($this->updateOrderId);
        if ( $insert_id ) {
            ...
            foreach ( array_keys($order_totals) as $i ) {
                $sql_data_array = array(
                    ...
                    'currency_value' => $this->order->info['currency_value'],
                );
                $order_totals[$i]['sql_array'] = $sql_data_array;
            }
            $get_order_total_r = tep_db_query("SELECT * FROM ".TABLE_ORDERS_TOTAL." WHERE orders_id='".(int)$this->updateOrderId."' order by sort_order");
