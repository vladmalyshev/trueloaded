...
class CreateCustomerResponse extends SoapModel
{
    /**
     * @var string
     * @soap
    ...
    public function build()
    {
        ...
        if (ServerSession::get()->getPlatformId()){
            $customerArray['platform_id'] = ServerSession::get()->getPlatformId();
        }
        ...
        if ( isset($customerArray['addresses']) && !empty($customerArray['addresses']) ) {
        ...
        if ( $customerObj ) {
            $this->warning('Customer email already exists. Update data.');
        }else {
            $customerObj = new \common\api\models\AR\Customer();
        }
