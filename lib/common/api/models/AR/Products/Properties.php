                ...
                if ( $lookupId==0 ) {
                    ...
                    foreach ($levelNameArray as $lang_id=>$prop_name){
                        if ( PropertyDescription::updateAll([
                            ...
                            'properties_seo_page_name' => Seo::makeSlug($prop_name),
        ...
        if ( is_array($prop_name) ) {
            ...
            foreach ($prop_names as $langId=>$_prop_name ){
                ...
                tep_db_query(
                    ...
                    "VALUES ('" . (int)$propId . "', '".(int)$langId."', '" . tep_db_input($_prop_name) . "', '" . tep_db_input(Seo::makeSlug($_prop_name)) . "')"
                );
            }
        }
        tep_db_query(
            "INSERT IGNORE INTO " . TABLE_PROPERTIES_DESCRIPTION . " (properties_id, language_id, properties_name, properties_seo_page_name) " .
            "SELECT '" . (int)$propId . "', languages_id, '" . tep_db_input($prop_name) . "', '" . tep_db_input(Seo::makeSlug($prop_name)) . "' FROM " . TABLE_LANGUAGES . " WHERE languages_status=1"
        );
