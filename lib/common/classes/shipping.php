...
// Show either normal shipping modules or free shipping module when Free Shipping Module is On
    ...
    public function getSurcharge(){
        ...
        if (is_null($this->surcharge)){
            ...
            foreach ($this->manager->getCart()->get_products() as $product) {
                $mProduct = \common\models\Products::find()->select('shipping_surcharge_price')
                        ->where(['products_id' => (int) $product['id']]) ->one();
                if ($mProduct) {
                    $this->surcharge += $mProduct['shipping_surcharge_price'] * $product['quantity'];
                }
        ...
        if (is_array($this->modules)) {
          ...
          $CustomerModules = \common\helpers\Acl::checkExtension('CustomerModules', 'checkAvailable');
            ...
            foreach ($this->include_modules as $_module){
              if ( $_module->getGroupVisibily(\common\classes\platform::currentId(), $groups_id) || ($CustomerModules::checkAllowed(\common\classes\platform::currentId(), \Yii::$app->user->getId(), $_module->code, 'shipping'))) {
                if (tep_not_null($module)) {
                    if (( $module == $_module->code ) && ( $_module->enabled ) && $_module->getVisibily($visibility) ) {
                        $include_quotes[] = $_module;
                    }
                } elseif ($_module->enabled && $_module->getVisibily($visibility)) {
                    $include_quotes[] = $_module;
                }
              }
            }
...
/**/
                        ...
                        if ($_module->useDelivery()){
                            ...
                            $this->deliveryMethodsCount += (is_array($quotes['methods'])? count($quotes['methods']): 0);
                        } else {
                            $this->pickupQuotes[] = $quotes;
                            $this->pickupMethodsCount += (is_array($quotes['methods'])? count($quotes['methods']): 0);
                        }
                        $this->allMethodsCount += (is_array($quotes['methods'])? count($quotes['methods']): 0);
                    }
                }
