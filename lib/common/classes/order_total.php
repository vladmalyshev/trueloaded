...
// class constructor
    function __construct($reconfig = false, \common\services\OrderManager $manager)
    {
        global $language;
    ...
    function process($reProcess = []) {
        ...
        if (is_null($this->order_total_array) || $reProcess){
            ...
            foreach($processinModules as $module){
                ...
                if ($module->getVisibily($module->manager->getModulesVisibility())){
...
//
    function update_credit_account($i) {
        if (MODULE_ORDER_TOTAL_INSTALLED) {
            foreach($this->getEnabledModules() as $module){
                if (($module->enabled && $module->credit_class)) {
    ...
    function collect_posts($limit_class = '', $post_data= []) {
        ...
        if (defined('MODULE_ORDER_TOTAL_INSTALLED') && MODULE_ORDER_TOTAL_INSTALLED) {
            foreach($this->getEnabledModules() as $class => $module){
                if ($module->credit_class || method_exists($module, 'collect_posts')) {
                    $post_var = 'c' . $module->code;
                    if ($post_data[$post_var]) {
                        if ($module->manager){
