        ...
        if (defined('MODULE_DROPSHIPPING_INSTALLED') && tep_not_null(MODULE_DROPSHIPPING_INSTALLED)) {
            ...
            for ($i = 0, $n = sizeof($include_modules); $i < $n; $i++) {
                ...
                if (\frontend\design\Info::isTotallyAdmin()) {
                    if (is_file(DIR_FS_CATALOG . DIR_WS_LANGUAGES . 'modules/dropshipping/' . $include_modules[$i]['file'])) {
                        include_once(DIR_FS_CATALOG . DIR_WS_LANGUAGES . 'modules/dropshipping/' . $include_modules[$i]['file']);
                    }
                    include_once(DIR_FS_CATALOG . DIR_WS_MODULES . 'dropshipping/' . $include_modules[$i]['file']);
                } else {
                    if (is_file(DIR_WS_LANGUAGES . 'modules/dropshipping/' . $include_modules[$i]['file'])) {
                        include_once(DIR_WS_LANGUAGES . 'modules/dropshipping/' . $include_modules[$i]['file']);
                    }
