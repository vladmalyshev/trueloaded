...
/** @var \common\extensions\MultiCart\MultiCart $MultiCart */
    ...
    function restoreContentProducts(){
...
/// there could be problem with auto_giveaways.
    ...
    // {{
                ...
                if (defined('PRODUCTS_INVENTORY') AND (PRODUCTS_INVENTORY == 'True')) {
    ...
    function count_contents() {  // get total number of items in cart
        ...
        if (is_array($this->contents)) {
            foreach ($this->contents as $products_id => $value) {
                ...
                if (tep_not_null($this->contents[$products_id]['parent'])) continue;
...
// }}
    ...
    function remove($products_id) {
        ...
        $_tmp = [
            ...
            'data' => $this->contents[$products_id],
        ];
...
// }}
    ...
    function calculate() {
        ...
        $customer_id = Yii::$app->user->getId();
        // Collections addon
        $this->collections = \common\helpers\Collections::get_collections_from_shopping_cart($this->contents);
...
// }}
                ...
                if (PRODUCTS_INVENTORY == 'True' && !InventoryHelper::disabledOnProduct($products_id)){
...
*/
                if ($swProduct){
                  ...
                  /** @var \common\extensions\Inventory\Inventory $ext */
                    if (($ext = \common\helpers\Acl::checkExtension('Inventory', 'getInventoryStock')) && PRODUCTS_INVENTORY == 'True' && !InventoryHelper::disabledOnProduct($stock_products_id) ) {
...
/* PC configurator addon begin */
                    'parent' => $this->contents[$products_id]['parent'],
...
// }}
                ...
                /** @var \common\extensions\ProductBundles\ProductBundles $ext*/
                if ($products['is_bundle'] && $ext = \common\helpers\Acl::checkExtension('ProductBundles', 'getVolume')) {
...
// {{ Bonus Points
                if (defined('MODULE_ORDER_TOTAL_BONUS_POINTS_STATUS') && MODULE_ORDER_TOTAL_BONUS_POINTS_STATUS == 'true'){
                    ...
                    if ($bonuses){
                        ...
                        $products_array[$products_id]['bonus_coefficient'] = $bonuses['bonus_coefficient'];
...
// }}
    ...
    public function hasBlockedProducts(){
        ...
        foreach ($this->get_products() as $product){
            ...
            if($cProduct){
                $has = (bool)$cProduct['block_in_cart'] || $has;
            }
        }
        return $has;
    }
...
// }}
    ...
    function configurator_price($parent_id, $products = '', $withTax = true, $_default_field = 'final_price') {
        ...
        for ($i=0, $n=sizeof($products); $i<$n; $i++) {
            if ($products[$i]['parent'] == $parent_id) {
    ...
    function get_subproducts($parent_id) {
        ...
        if (is_array($this->contents)) foreach ($this->contents as $products_id => $value) {
            if ($this->contents[$products_id]['parent'] == $parent_id) {
                $ar[] = $products_id;
            }
        }
    ...
    function add_bundle($products_id, $bundle_products = array(), $qty = '1', $attributes = '', $notify = true, $relationType='') {
        ...
        $sub = InventoryHelper::get_uprid($products_id, $bundle_products_attributes[$products_id]);
        $sub = InventoryHelper::normalize_id($sub);
        $products_uprid = $sub . '{tpl}';
        $key = 0;
        foreach ($bundle_products as $prid => $bundle_qty) {
            $sub_product_id = InventoryHelper::normalize_id(InventoryHelper::get_uprid($prid, $bundle_products_attributes[$prid])) . '{sub}' . $sub;
            $products_uprid .= ++$key . '|' . $sub_product_id . '|';
        }
        ...
        $__ids = [];
        $__ids[] = $this->add_cart_cfg($products_uprid, $qty, $bundle_products_attributes[$products_id], $notify, false, 'update', $relationType);

        foreach ($bundle_products as $prid => $bundle_qty) {
            $sub_product_id = InventoryHelper::normalize_id(InventoryHelper::get_uprid($prid, $bundle_products_attributes[$prid])) . '{sub}' . $sub;
            if (count(\common\helpers\Product::getChildArray($sub_product_id)) == 0) {
                $__ids[] = $this->add_cart_cfg($sub_product_id, $qty * $bundle_qty, $bundle_products_attributes[$prid], false, $products_uprid, $type, $relationType);
            }
        }
        $this->cleanup();
        Yii::$app->get('PropsHelper')::cartChanged($this);
    ...
    function add_custom_bundle($notify = true, $type = 'update') {
        ...
        $sub = InventoryHelper::normalize_id(InventoryHelper::get_uprid($_POST['products_id'], $_POST['id']));
        $products_uprid = $sub . '{tpl}';
        reset($_POST['custom_bundles']);
        foreach ($_POST['custom_bundles'] as $key => $value) {
            if ($value == 'on' || $value == '0')
                continue;
            if (is_array($_POST['custom_bundles_attr'][$key][$value])) {
                $sub_product_id = InventoryHelper::normalize_id(InventoryHelper::get_uprid($value, $_POST['custom_bundles_attr'][$key][$value])) . '{sub}' . $sub;
            } else {
                $sub_product_id = InventoryHelper::normalize_id(InventoryHelper::get_uprid($value, $_POST['custom_bundles_attr'][$value])) . '{sub}' . $sub;
            }
            $products_uprid .= $key . '|' . $sub_product_id . '|';
        }
        ...
        $__ids = [];
        $__ids[] = $this->add_cart_cfg($products_uprid, $qty, $_POST['id'], true);
        reset($_POST['custom_bundles']);
        foreach ($_POST['custom_bundles'] as $key => $value) {
            if ($value == 'on' || $value == '0')
                continue;
            $custom_bundles_qty = ($_POST['custom_bundles_qty'][$value] > 0 ? $_POST['custom_bundles_qty'][$value] : 1);
            if (is_array($_POST['custom_bundles_attr'][$key][$value])) {
                $sub_product_id = InventoryHelper::normalize_id(InventoryHelper::get_uprid($value, $_POST['custom_bundles_attr'][$key][$value])) . '{sub}' . $sub;
                ...
                $sub_product_id = InventoryHelper::normalize_id(InventoryHelper::get_uprid($value, $_POST['custom_bundles_attr'][$value])) . '{sub}' . $sub;
                $__ids[] = $this->add_cart_cfg($sub_product_id, $qty * $custom_bundles_qty, $_POST['custom_bundles_attr'][$value], false, $products_uprid, $type);
            }
        }
        $this->cleanup();
        Yii::$app->get('PropsHelper')::cartChanged($this);
    ...
    public function productAmountData($products_id,$add = false)
    {
        ...
        if (($ext = \common\helpers\Acl::checkExtension('Inventory', 'getInventoryStock')) && PRODUCTS_INVENTORY == 'True' && !InventoryHelper::disabledOnProduct($products_id)) {
