...
abstract class OrderAbstract extends OrderShadowAbstract{
    ...
    function query($order_id) {
        ...
        foreach($orders_products_list as $orders_products){
            $this->products[$index] = array('qty' => $orders_products['products_quantity'],
                ...
                'relation_type' => strval($orders_products['relation_type']),
                //'configurator_price' => $cart->configurator_price($products[$i]['id'], $products), // TODO
                /* PC configurator addon end */
                'sort_order' => $orders_products['sort_order'],
                'weight' => $orders_products['products_weight'],
                'status' => $orders_products['orders_products_status'],
                'status_manual' => $orders_products['orders_products_status_manual'],
                'orders_products_id' => $orders_products['orders_products_id'],
                'promo_id' => $orders_products['promo_id'],
                'qty_cnld' => $orders_products['qty_cnld'],
                'qty_rcvd' => $orders_products['qty_rcvd'],
                'qty_dspd' => $orders_products['qty_dspd'],
                'qty_dlvd' => $orders_products['qty_dlvd']
            );
            ...
            if (!isset($tax_groups[$selected_tax])) {
                $tax_groups[$selected_tax] = \common\helpers\Tax::roundTax($this->products[$index]['final_price'] * $this->products[$index]['qty'] * $this->products[$index]['tax'] / 100);
            } else {
                $tax_groups[$selected_tax] += \common\helpers\Tax::roundTax($this->products[$index]['final_price'] * $this->products[$index]['qty'] * $this->products[$index]['tax'] / 100);
            }
    ...
    {
        if ($ext = \common\helpers\Acl::checkExtension('LinkedProducts', 'allowed')) {
            $_parent_ref = [];
            $_linked_children = [];
    ...
    public function save_subscription($subscription_id = 0, $order_id = 0, $i = 0, $uuid = '', $info = '') {
        if($ext = \common\helpers\Acl::checkExtension('Subscriptions', 'saveSubscription')){
...
// BOF: WebMakers.com Added: Downloads Controller
        $sql_data_array = array(
            ...
            'customers_country' => isset($this->customer['country']['title']) ? $this->customer['country']['title'] : '',
...
// EOF: Lango Added for print order mod
            ...
            'bonus_points_redeem' => (float)$this->info['bonus_points_redeem'],
            'products_price_qty_round' => (PRODUCTS_PRICE_QTY_ROUND == 'true'),
            'pointto' => isset($this->info['pointto'])?$this->info['pointto']:0,
        ...
        $_delivery = [
            'delivery_address_book_id' => isset($this->delivery['address_book_id']) ? (int)$this->delivery['address_book_id'] : 0,
            ...
            'delivery_company' => isset($this->delivery['company']) ? $this->delivery['company'] : '',
            'delivery_company_vat' => $this->delivery['company_vat'],
            'delivery_company_vat_status' => $this->delivery['company_vat_status'],
            'delivery_country' => isset($this->delivery['country']['title']) ? $this->delivery['country']['title'] : '' ,
...
// EOF: WebMakers.com Added: Downloads Controller
        ...
        if ($ext = \common\helpers\Acl::checkExtension('Neighbour', 'toOrder')) {
    ...
    private function processAdditionalShippingParams(?int $refId = null){
    	...
    	if( isset($this->info['shipping_class'])){
            ...
            if(method_exists($module, 'setAdditionalOrderParams')){
                $params = [];
    ...
    protected function searchOrderProduct(array $array_data){
        if (isset($array_data['template_uprid']) && !empty($array_data['template_uprid'])){
            $opQuery = $this->getProductsARModel()->where(['orders_id' => $this->order_id, 'template_uprid' => (string)$array_data['template_uprid'], 'is_giveaway' => (int)$array_data['is_giveaway']]);
        } else if (isset($array_data['uprid']) && !empty($array_data['uprid'])){
            $opQuery = $this->getProductsARModel()->where(['orders_id' => $this->order_id, 'uprid' => (string)$array_data['uprid'], 'is_giveaway' => (int)$array_data['is_giveaway']]);
        } else if (isset($array_data['products_id']) && !empty($array_data['products_id'])){
            $opQuery = $this->getProductsARModel()->where(['orders_id' => $this->order_id, 'products_id' => $array_data['products_id'], 'is_giveaway' => (int)$array_data['is_giveaway']]);
        }
    ...
    public function save_products($notify = true) {
        ...
        $dontUpdateStock = ($sPayment ? $sPayment->dont_update_stock: $dontUpdateStock);
        $opInOrder = [];
        for ($i = 0, $n = sizeof($this->products); $i < $n; $i++) {
            ...
            if ((STOCK_LIMITED == 'true' && !$dontUpdateStock && $this->table_prefix != 'purchase_') || \common\helpers\Order::is_stock_updated(intval($this->order_id))) {
                ...
                \common\helpers\Warehouses::update_stock_of_order($this->order_id, (strlen($this->products[$i]['template_uprid']) > 0 ? $this->products[$i]['template_uprid'] : $this->products[$i]['id']), $this->products[$i]['qty']);
...
// Update products_ordered (for bestsellers list)
            ...
            $sql_data_array = array('orders_id' => $this->order_id,
                ...
                'bonus_points_cost' => $this->products[$i]['bonus_points_cost'],
                /* PC configurator addon end */
            ...
            if ($order_products_id){
              ...
              if (empty($sql_data_array['orders_products_status'])) {
                ...
                if ($ext = \common\helpers\Acl::checkExtension('Rma', 'setOrderProductReturnDate')) {
            ...
            if (($this->table_prefix != 'quote_' || SHOW_PRICE_FOR_QUOTE_PRODUCT == 'True') && !(defined('GROUPS_IS_SHOW_PRICE') && GROUPS_IS_SHOW_PRICE==false)) {
    ...
    protected function getHistoryLink(){
        if (\frontend\design\Info::isTotallyAdmin()) {
            $link = \common\helpers\Output::get_clickable_link(tep_catalog_href_link(FILENAME_ACCOUNT_HISTORY_INFO, 'order_id=' . $this->order_id, 'SSL', false));
        } else {
    ...
    public function notify_customer($products_ordered,$emailParams = [], $emailTemplate = '') {
        ...
        list($class, $method) = explode('_', $this->info['shipping_class']);
        $collect = false;
        ...
        }elseif( $this->manager->isDeliveryUsed() || !$this->manager->isChargedOrder()){
        ...
        if ($this->manager->isChargedOrder()){
            ...
            if ($selectedPayment){
                if ($selectedPayment->email_footer) {
        ...
        $customer_notified = 0;
        if (!empty($emailTemplate)) {
            list($email_subject, $email_text) = \common\helpers\Mail::get_parsed_email_template($emailTemplate, $email_params, -1, $this->info['platform_id'], -1, $emailDesignTemplate);
            ...
            if (defined('ATTACH_PDF_INVOICE') && ATTACH_PDF_INVOICE =='True' && in_array($this->info['order_status'], \common\helpers\Order::extractStatuses(ATTACH_PDF_STATUS) ) ) {
              $attachment[] = ['name' => TEXT_INVOICE . '.pdf',
                              'file' => $this->getPdfInvoice()];
            }
            if ($this->customer['email_address']){
                \common\helpers\Mail::send(
                    $this->customer['firstname'] . ' ' . $this->customer['lastname'], $this->customer['email_address'], $email_subject, $email_text, STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, [], '', $attachment, ['add_br' => 'no']
                );
            }


            if (SEND_EXTRA_ORDER_EMAILS_TO == '') {
                \common\helpers\Mail::send(STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, $email_subject, $email_text, STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, [], '', $attachment, ['add_br' => 'no']);
            } else {
                \common\helpers\Mail::send(STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, $email_subject, $email_text, STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, array(), 'CC: ' . SEND_EXTRA_ORDER_EMAILS_TO, $attachment, ['add_br' => 'no']);
            }
            $customer_notified = 1;
        }
    ...
    public function send_status_notify($notify_comments='', $extra_template_params=[], $email_headers=[])
    {
        ...
        if(!empty($emailTemplate)) {
            ...
            \common\helpers\Mail::send(
                ...
                [], $email_headers, $attachment, ['add_br' => 'no']
            );
            $customer_notified = 1;
  ...
  public function getPdfInvoice() {
    ...
    if (defined("THEME_NAME")) {
        ...
        $theme_id = \common\models\PlatformsToThemes::findOne($this->order_id)->theme_id;
  ...
  public function isUnpaid () {
    ...
    public function getProductsHtmlForEmail()
    {
        ...
        foreach ($this->products as $i => $product) {
            ...
            $hidePrice = ($this->table_prefix == 'quote_' && SHOW_PRICE_FOR_QUOTE_PRODUCT != 'True') ||
