 ...
 */
    public static function product_info($data_array)
    {
        ...
        if ($ext = \common\helpers\Acl::checkExtension('LinkedProducts', 'filterStockIndication')) {
 ...
 */
        ...
        if ($ext = \common\helpers\Acl::checkExtension('ShowInactive', 'getFlag'))
        {
            if (!$products_status)
            {
                $invert = $ext::getFlag();
       ...
       /**
         ...
         * 2 - hide if zero
         */
        $backOrderInfo = \common\helpers\PurchaseOrder::getOrderedProduct($oProduct->products_id);
        $backOrderFirst = [];
        if (count($backOrderInfo)) {
          $inQty = min(0, \common\helpers\Product::getAvailable($oProduct->products_id)); //suppose it could be negative
          /*reset($backOrderInfo);
        ...
        if (($stock_indication_id >0 && $stock_info_pre_lookup['allow_out_of_stock_add_to_cart'] != 0) || $data_array['products_quantity']>0) {
          ///indication allow add to cart with any stock level
          // or product is in stock
            $add_to_cart = $oProduct->cart_button;
        } elseif ($data_array['products_quantity'] <= 0 || $stock_info_pre_lookup['allow_out_of_stock_add_to_cart'] == 0) {
          // often default is out of stock so allow_out_of_stock_add_to_cart == 0

          if ($oProduct->allow_backorder==1 || ($oProduct->allow_backorder==0 && defined('STOCK_ALLOW_BACKORDER_BY_DEFAULT') && STOCK_ALLOW_BACKORDER_BY_DEFAULT == 'True' )) {
        ...
        if ($stock_indication_id == 0 && ($instock_condition || ($on_cart_page && $data_array['products_quantity'] > 0)))
        {
            ...
            $stock_info['flags'] = [
                ...
                'ask_sample' => $ask_sample,
                'can_add_to_cart' => $stock_info_pre_lookup['allow_out_of_stock_add_to_cart'],
