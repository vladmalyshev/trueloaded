...
class xml_node
{
    ...
    function &item($name, $offset = 0) { //Versucht ein Item zu finden und gibt es zurück, bei mehreren Items auf der gleichen Ebene kann mit dem Offset gearbeitet werden                                  //Es kann auch ein Pfad übergeben werden, Syntax: item1/item2 - Der Offset bezieht sich dann auf die Elemente des letzten Elements
    ...
    function print_structure() {  //Gibt die XML Daten aus - zum Debuggen geeignet
        for ($i = 0; $i < count($this->subitems); $i++) {
            ...
            $ausgabe = "";
            if (is_array($this->subitems[$i]->attributes)) foreach ($this->subitems[$i]->attributes as $key => $val) {
                $ausgabe .= "$key = '$val', ";
            }
