...
abstract class ModuleShipping extends Module {
    ...
    public function getLabels($platform_id) {
        ...
        $response = '<br><br><table width="50%"><thead><tr><th>' . MODULE_SHIPPING_TRACKING_STATUS . ' ' . tep_draw_checkbox_field('enable_tracking', '1', !is_null($modulesLabels), '', 'onchange="return updateLabels(this);"' ) . '</th></thead><tbody>';
        ...
        $response .= '<script type="text/javascript">function updateLabels(obj) { if ( $(obj).is(":checked") ) { $("input[name^=\'labels\']").prop("disabled", false); } else { $("input[name^=\'labels\']").prop("disabled", true); } }</script>';
        return $response;
