...
class Images {
    ...
    public static function getImageTags($productsId, $imageId = 0, $languageId = -1) {
        ...
        $result_tags = array();
        if ($imageId > 0) {
            $product_image = tep_db_fetch_array(tep_db_query("select pi.products_images_id, if(length(pid1.image_alt) > 0, pid1.image_alt, pid.image_alt) as image_alt, if(length(pid1.image_title) > 0, pid1.image_title, pid.image_title) as image_title from " . TABLE_PRODUCTS_IMAGES . " pi, " . TABLE_PRODUCTS_IMAGES_DESCRIPTION . " pid left join " . TABLE_PRODUCTS_IMAGES_DESCRIPTION . " pid1 on pid.products_images_id = pid1.products_images_id and pid1.language_id = '" . (int) $languageId . "' where pi.products_id = '" . (int) $productsId . "' and pi.products_images_id = '" . (int) $imageId . "' and pi.products_images_id = pid.products_images_id and pid.language_id = '0'"));
        } else {
            $product_image = tep_db_fetch_array(tep_db_query("select pi.products_images_id, if(length(pid1.image_alt) > 0, pid1.image_alt, pid.image_alt) as image_alt, if(length(pid1.image_title) > 0, pid1.image_title, pid.image_title) as image_title from " . TABLE_PRODUCTS_IMAGES . " pi, " . TABLE_PRODUCTS_IMAGES_DESCRIPTION . " pid left join " . TABLE_PRODUCTS_IMAGES_DESCRIPTION . " pid1 on pid.products_images_id = pid1.products_images_id and pid1.language_id = '" . (int) $languageId . "' where pi.products_id = '" . (int) $productsId . "' and pi.default_image = '1' and pi.products_images_id = pid.products_images_id and pid.language_id = '0'"));
        }
        $result_tags['alt_tag'] = $product_image['image_alt'];
        $result_tags['title_tag'] = $product_image['image_title'];
        ...
        if (empty($result_tags['alt_tag']) || empty($result_tags['title_tag'])) {
            ...
            foreach ($categories_array as $categoriesId) {
                ...
                if (empty($result_tags['alt_tag'])) {
                    $result_tags['alt_tag'] = $category['categories_image_alt_tag_mask'];
                }
                if (empty($result_tags['title_tag'])) {
                    $result_tags['title_tag'] = $category['categories_image_title_tag_mask'];
                }
                if (!empty($result_tags['alt_tag']) && !empty($result_tags['title_tag'])) {
                    break;
                }
    ...
    public static function getImageUrl($productsId = 0, $typeName = 'Thumbnail', $languageId = -1, $imageId = 0, $getPath = false, $inWebp = true)
    {
        ...
        if (file_exists($image_location . $imageName)) {
            ...
            $publicFileName = $publicFilenamePrefix . $imageId . '/' . $image_types['folder_name'] . '/' . (!empty($language) ? $language . '/' : '') . urlencode($imageName);
    ...
    public static function getImageId($productsId = 0)
    {
        ...
        return $images['products_images_id'];
    }
    ...
    {
        $resolutions = \common\models\ImageTypes::find()->where(['image_types_name' => $typeName])->asArray()->all();
  ...
  public static function getTypeFromFile($file_name)
    ...
    public function createImage($source_image, $destination_image, $width, $height) {
        return self::tep_image_resize($source_image, $destination_image, $width, $height);
    }
...
// }}
    ...
    protected static function removeReferenceFilename($filename)
    {
        ...
        if ($checkEmptyDirectory!='.') while($checkEmptyDirectory){
    ...
    public static function getImageSet($sourceImage, $imageType = '', $attributes = [], $naImage = '', $lazy_load = false)
    {
        ...
        if (!$naImage) $naImage = Info::themeFile('/img/na.png');
        ...
        foreach ($imageTypes as $imageType) {
            ...
            $imageUrl = $urlPath . self::getWebp($filePath . DIRECTORY_SEPARATOR . urlencode($fileName) . '-' . $width . '.' . $ext);
     ...
     * @return string
     */
    public static function moveImage($sourceImage, $destination)
    {
        ...
        copy($path . $sourceImage, $path . $destination . DIRECTORY_SEPARATOR . $fileName);
