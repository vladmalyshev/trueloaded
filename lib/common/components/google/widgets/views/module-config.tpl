...
{if is_array($elements['fields'])}
    {foreach $elements['fields'] as $_key => $field}
        <tr>
            ...
            <td><input type="{$field['type']}" name="fields[{$_key}][{$field['name']}]" value="{\common\helpers\Output::output_string($field['value'])}"  {if $field['type'] == 'checkbox' && $field['value']} checked{/if} {if $field['type'] == 'text'}class="form-control"{/if}></td>
        </tr>
    {/foreach}
{/if}
