...
use common\models\promotions\PromotionsCustomerCodes;
...
class Customer  extends \common\models\Customers implements \yii\web\IdentityInterface {
    ...
    public function validateCustomer($checkParam){
        ...
        switch ($this->loginType) {
            case static::LOGIN_STANDALONE :
                if (!\common\helpers\Password::validate_password($checkParam, $this->customers_password))
                    $success = false;
                break;
    ...
    public function loginCustomer($email_address, $checkParam) {
        ...
        if (($success = $_user->validateCustomer($checkParam)) !== false) {
            $_user->_afterAuth();
    ...
    private function _afterAuth(){
        ...
        if ($this->customers_id){
            ...
            if ($ext = \common\helpers\Acl::checkExtension('Quotations', 'restoreCart')) {
                $ext::restoreCart();
            }
            if ($ext = \common\helpers\Acl::checkExtension('Samples', 'restoreCart')) {
                $ext::restoreCart();
            }
        }
    }
    ...
    private function setCustomersData(){
        if ($this->customers_id){
            ...
            $addressBook = $this->getDefaultAddress()->one();
            $this->data['customer_country_id'] = $addressBook->entry_country_id;
            $this->data['customer_zone_id'] = $addressBook->entry_zone_id;
            if ($addressBook->entry_company_vat_status>1) {
    ...
    public function createCustomerQo($qoModel){
        ...
        /*if($customer = static::findByEmail($email_address)){
            ...
            $customer->setAttributes([
                ...
                'customers_password' => \common\helpers\Password::encrypt_password(\common\helpers\Password::create_random_value(ENTRY_PASSWORD_MIN_LENGTH)),
    ...
    */    
    ...
    public function applyPromoCode($code){
        if ($this->customers_id){
            $promo = Promotions::getPromotionByPromoCode($code)->one();
            if ($promo){
                return PromotionsCustomerCodes::saveCode($promo, $this->customers_id);
            }
        } else {
    ...
    public function getPromoCodes(){
        if ($this->customers_id){
            return PromotionsCustomerCodes::getCustomerRegisteredPromo($this->customers_id);
        } else {
            $code = $this->getRegisteredPromoCode();
            if ($code){
                return Promotions::getPromotionByPromoCode($code)->all();
            }
        }
    ...
    public function hasBonusHistory($customer_id){
        return models\CustomersCreditHistory::find()->where('customers_id = :id', [':id' => $customer_id])->count() || models\promotions\PromotionsBonusHistory::getFullHistoryAmount($customer_id);
    }
    ...
    public function registerCustomer(CustomerRegistration $model = null, $withLogin = true, \common\forms\AddressForm $addressModel = null) {
        if ($model){
            ...
            $this->setAttributes([
                ...
                'customers_status' => ($login ? 1 : 0),
                'customers_password' => \common\helpers\Password::encrypt_password($model->password),
            ...
            if (property_exists(Yii::$app->controller, 'promoActionsObs')){
    ...
    public function registerGuestCustomer(CustomerRegistration $customerModel, \common\forms\AddressForm $addressModel){
        ...
        $this->customers_password = \common\helpers\Password::encrypt_password($customerModel->password);
...
//        }
    ...
    public function addAddress($attributes){
        ...
        if ($VatOnOrder = \common\helpers\Acl::checkExtension('VatOnOrder', 'check_vat_status')) {
          try {
