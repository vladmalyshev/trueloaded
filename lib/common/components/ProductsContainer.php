 ...
 */
    public function loadProducts(array $params = []){
        ...
        if (isset($params['products_id']) && count($params)== 1){
            ...
            if ($this->container[$params['products_id']] instanceof ProductItem){
    ...
    protected function _preloadProduct($products_id){
        if ($products_id){
            ...
            if ($product){
                ...
                $data['products_description'] = \common\classes\TlUrl::replaceUrl($data['products_description']);
                $data['products_description_short'] = \common\classes\TlUrl::replaceUrl($data['products_description_short']);
                $data['tax_rate'] = Tax::get_tax_rate($product['products_tax_class_id']);
