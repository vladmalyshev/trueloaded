...
class ot_due extends ModuleTotal {
    ...
    function process() {
        ...
        $this->output = [];
        $amount = $order->info['total_exc_tax'] - $order->info['total_paid_exc_tax'] + $order->info['total_refund_exc_tax'];
        $amount_tax = $order->info['total_inc_tax'] - $order->info['total_paid_inc_tax'] + $order->info['total_refund_inc_tax'];

        if ($order->info['total_refund_inc_tax'] >= ($order->info['total_paid_inc_tax'] - 0.01) ){
            $amount_tax = $amount = 0;
        }
