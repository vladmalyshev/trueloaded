...
class OrderManager {
    ...
    public function getShippingQuotesByChoice() {
        static $_quotes = null;
    ...
    public function checkFreeShipping() {
        ...
        if (\common\models\promotions\PromotionService::confirmFreeShipping($this)){
            $free_shipping = true;
        }
            ...
            for ($i = 0, $n = count($quotes); $i < $n; $i++) {
                if (!isset($quotes[$i]['error'])) {
                    for ($j = 0, $n2 = count($quotes[$i]['methods']); $j < $n2; $j++) {
                        ...
                        $quotes[$i]['methods'][$j]['no_cost'] = $quotes[$i]['methods'][$j]['no_cost'];
    ...
    public function resetShipping() {
        if (array_intersect(['admin'], $this->getModulesVisibility()) && defined('SHIPPING_UNSELECTED') && SHIPPING_UNSELECTED == 'unselected')
            return;
        if ($this->checkFreeShipping()) {
    ...
    private function _getShipingAsArray($quote, $module, $method, $method_index, $free_shipping = false){
        ...
        return [
            'module' => $module,
            'id' => $module . '_' . $method,
            'title' => (($free_shipping == true) ? $quote['methods'][$method_index]['title'] : $quote['module'] . ' (' . $quote['methods'][$method_index]['title'] . ')'),
            'cost' => $cost,
            'no_cost' => $quote['methods'][$method_index]['no_cost'],
    ...
    protected function loadDefaultAddressValues($postfix) {
        if ($this->has('estimate' . $postfix)) {
            ...
            foreach (['company_vat', 'company_vat_date', 'company_vat_status'] as $k) {
              if (!empty($estimate[$k])) {
    ...
    public function changeCustomerAddressSelection($type, $address) {
        if ($this->isCustomerAssigned() && $address) {
            if ($type == 'shipping') {
                ...
                /* $ab = $this->getCustomersIdentity()->getAddressBook($ab_id);
                  if ($ab) {
                ...
                $ab = $this->getCustomersIdentity()->getAddressBook($address);
                if ($ab) {
                    $this->getCustomersIdentity()->set('customer_country_id', $ab->entry_country_id, true);
                    $this->getCustomersIdentity()->set('customer_zone_id', $ab->entry_zone_id, true);
                }
 ...
 */
    ...
    public function registerCustomerAccount($opc_temp_account = 0) {
        if (!$this->isCustomerAssigned()) {
            $customer = $this->getCustomersIdentity();
            $shippingAddress = $this->getShippingForm();
            $billingAddress = $this->getBillingForm();
            $contactForm = $this->getCustomerContactForm(false);
 ...
 */
    ...
    public function totalProcess() {
        $this->triggerEvents(__FUNCTION__);
        return $this->getTotalCollection()->process();
    }
    ...
    public function wrapTotals(array $order_total_output, $context, $all = false) {
        ...
        foreach ($order_total_output as $total) {
            ...
            if (method_exists($module, 'visibility')) {
    ...
    public function getCreditModules() {
        ...
        $credit_modules = array(
            'applied_coupon_code' => ($this->has('cc_id') && $this->get('cc_id') > 0 ? \common\helpers\Coupon::get_coupon_name($this->get('cc_id')) : ''),
    ...
    public function getCustomerContactForm($preload = true) {
        ...
        if ($preload) {
            $customer = $this->getCustomersIdentity();
            if (!$customer->isNewRecord || $customer->get('fromOrder')) {
    ...
    public function defineOrderTaxAddress() {
        ...
        $scheck = \common\helpers\Tax::getTaxZones($sAddress['country_id'], $sAddress['zone_id']);
        $bcheck = \common\helpers\Tax::getTaxZones($bAddress['country_id'], $bAddress['zone_id']);
        $address = null;
        ...
        if ($address && $this->isInstance()) {
            $this->orderInstance->tax_address = [
                'entry_country_id' => $address['country_id'],
                'entry_zone_id' => $address['zone_id'],
                'postcode' => $address['postcode'],
                'company_vat' => $address['company_vat'],
                'company_vat_date' => $address['company_vat_date'],
                'company_vat_status' => $address['company_vat_status']
            ];
        }
    }
                ...
                try {
                    ...
                    if (is_object($_order)) {
                        ...
                        if ($this->isCustomerAssigned()) {
                            ...
                            if ($_order->delivery) {
                                $this->set('sendto', $_order->delivery);
                                $this->set('estimate_ship', ['country_id' => $_order->delivery['country_id'], 'postcode' => $_order->delivery['postcode'], 'zone' => $_order->delivery['state'], 'company_vat' => $_order->delivery['company_vat'], 'company_vat_date' => $_order->delivery['company_vat_date'], 'company_vat_status' => $_order->delivery['company_vat_status']]); // need here as bill as ship just copies out manager's data
                            }
                            if ($_order->billing) {
                                $this->set('billto', $_order->billing);
                                $this->set('estimate_bill', ['country_id' => $_order->billing['country_id'], 'postcode' => $_order->billing['postcode'], 'zone' => $_order->billing['state'], 'company_vat' => $_order->billing['company_vat'], 'company_vat_date' => $_order->billing['company_vat_date'], 'company_vat_status' => $_order->billing['company_vat_status']]
                                    );
                            }
                        }
