                        ...
                        <div class="wl-td1">                            
                            {if is_object($service->currencies) && !$service->currencies_editor_simple}
                                ...
                                {foreach $service->currencies->currencies as $code => $_curr}
                                    <tr class="act {if isset($service->currenciesMap[$_curr['id']]) && $service->currenciesMap[$_curr['id']]->status}active_row{else}hide_row{/if} currency_{$code}">
                                        <td>{$_curr['title']}</td>
                                            ...
                                            <table class="tl-grid currency-table">
                                                ...
                                                {foreach $service->currencies->currencies as $code => $_curr}
                                                    <tr class="act popup_cur_{$code}">
                                                        <td>{$_curr['title']}</td>
                                                        <td>
                                                        {Html::checkbox('suppliers_data[currencies]['|cat:$_curr['id']|cat:'][status]', $service->currenciesMap[$_curr['id']]->status , ['value' => 1, 'class' => 'currencies_status _on_off', 'data-code' => $code]) }</td>
                ...
                <div class="widget-content">
                    <div class="w-line-row w-line-row-1">
                        <div class="wl-td">
                            ...
                            {Html::input('text', 'suppliers_data[email_address]', $service->supplier->authData->email_address, ['class' => "form-control"])}
                         </div>
                    </div>
                    <div class="w-line-row w-line-row-1">
                        <div class="wl-td input-group">
                            <label>{$smarty.const.ENTRY_PASSWORD}</label>
                            {Html::textInput('suppliers_data[password]', $service->supplier->authData->password, ['class' => "form-control pwd-field"])}
