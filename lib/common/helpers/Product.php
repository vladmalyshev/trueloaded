    ...
    public static function normalizePricePrid($productsId)
    {
        ...
        if ( !isset($lastNormalized[$productsId]) ) {
            ...
            if ( self::priceProductIdColumn()!=='products_id' ) {
                $parentage = \common\models\Products::find()
                    ->where(['products_id' => (int)$productsId])
                    ->select(self::priceProductIdColumn())
                    ->asArray()
                    ->one();
    ...
    public static function normalizePrid($productsId)
    {
        ...
        if ( !isset($lastNormalized[$productsId]) ) {
          ///2do check in the storage first (* from products)
            ...
            $parentage = \common\models\Products::find()
                ->where(['products_id' => (int)$productsId])
                ->select(self::stockProductIdColumn())
                ->asArray()
                ->one();
            if (is_array($parentage) && $parentage[self::stockProductIdColumn()] > 0) {
...
//                }
    ...
    public static function getProductWeight($uprid, $qty=1) {
      $products_weight = $qty * self::get_products_weight($uprid);
          if (PRODUCTS_INVENTORY == 'True' && !InventoryHelper::disabledOnProduct($uprid)){
    ...
    public static function get_products_volume(int $products_id, bool $weight = false) {
        ...
        if ($product['is_bundle']) {
          if ($ext = \common\helpers\Acl::checkExtension('ProductBundles', 'getVolume')) {
    ...
    public static function get_products_stock($products_id) {
        ...
        if (defined('PRODUCTS_INVENTORY') && PRODUCTS_INVENTORY == 'True' && !\common\helpers\Inventory::disabledOnProduct($products_id)) {
            ...
            if (tep_db_num_rows($stock_query)) {
                ...
                if (defined('ENABLE_FREEZE_STOCK') && ENABLE_FREEZE_STOCK == '1') {
                    $freezeInventory = \common\models\FreezeInventory::find()->where(['products_id' => $products_id])->asArray()->one();
                ...
                if (defined('ENABLE_FREEZE_STOCK') && ENABLE_FREEZE_STOCK == '1') {
                    $freezeProducts = \common\models\FreezeProducts::find()->where(['products_id' => (int)$products_id])->asArray()->one();
        ...
        return $stock_values['products_quantity'] + $stock_values['suppliers_stock_quantity'] + $customers_temporary_stock_quantity;
    }
    ...
    public static function update_customers_temporary_stock_quantity($products_id, $qty, $warehouse_id = 0, $suppliers_id = 0, $not_available = false, $original_products_id = '', $keepUntil = 'now()') {
        if (defined('STOCK_LIMITED') && defined('TEMPORARY_STOCK_ENABLE') && STOCK_LIMITED == 'true' && TEMPORARY_STOCK_ENABLE == 'true') {
          ...
          if ($keepUntil != 'now()') {
            ...
            unset($match);
            if ($temporary_stock_data['temporary_stock_id'] > 0) {
...
*/
    ...
    public static function remove_product($product_id) {
        ...
        if ($ext = \common\helpers\Acl::checkExtension('ProductTemplates', 'productDelete')) {
    ...
    public static function duplicate($products_id, $categories_id, $copy_attributes)
    {
        ...
        $__data['products_date_added'] = date('Y-m-d');
        ...
        if ($copy_attributes) {
            ...
            ob_start();
            \common\helpers\Attributes::copy_products_attributes($products_id_from, $products_id_to/*, $copy_attributes_delete_first, $copy_attributes_duplicates_skipped*/);
            ob_get_clean();
        }
/// images (after attributes and inventory
    ...
    public static function save_specials_prices($specials_id, $group_id, $currencies_id = 0,$specials_groups_prices){
        $sql_data_array = [];
...
/**
 * product price in selected currency for specified group (no q-ty discount)
 *
 ...
 */
    ...
    public static function products_groups_name($products_groups_id, $language_id = '') {
        ...
        return $products_groups['products_groups_name'];
    }
    ...
    public static function set_status($productId, $status)
    {
        ...
        if ($ext = \common\helpers\Acl::checkExtension('AutomaticallyStatus', 'productAutoSwitchOff')) {
...
// 2do new products by name
    ...
    public static function writeHistory($uProductId = 0, $warehouseId = 0, $supplierId = 0, $locationId = 0, $productQuantity = 0, $parameterArray = [])
    {
        ...
        if ($productQuantity != 0 AND $warehouseId > 0 AND $supplierId > 0) {
            ...
            if ($productRecord instanceof \common\models\FreezeProducts) {
            ...
            if ($platformId <= 0) {
                ...
                if (\common\helpers\Inventory::isInventory($uProductId) != true) {
                    ...
                    if ($productRecord instanceof \common\models\Products AND $productRecord->stock_control == 1) {
                        $isStockControl = 0;
                        $platformStockControl = \common\models\PlatformStockControl::findOne(['products_id' => $uProductId, 'platform_id' => $platformId]);
                    ...
                    if ($inventoryRecord instanceof \common\models\Inventory AND $inventoryRecord->stock_control == 1) {
                        $isStockControl = 0;
                        $platformInventoryControl = \common\models\PlatformInventoryControl::findOne(['products_id' => $uProductId, 'platform_id' => $platformId]);
    ...
    public static function getAllocatedTemporaryArray($uProductId = 0, $asArray = true, $isBackend = false)
    {
        ...
        if ((int)$isBackend > 0) {
            ...
            if (defined('TEMPORARY_STOCK_ENABLE') AND TEMPORARY_STOCK_ENABLE == 'true') {
                if (defined('ENABLE_FREEZE_STOCK') && ENABLE_FREEZE_STOCK == '1') {
                    $return = \common\models\FreezeTemporaryStock::find()
    ...
    public static function getStockOrdered($uProductId = 0, $isStrict = false)
    {
        $return = 0;
        $uProductId = \common\helpers\Inventory::normalize_id_excl_virtual($uProductId);
        foreach (\common\models\PurchaseOrdersProducts::find()
            ->select('(products_quantity - (qty_cnld + qty_rcvd)) AS pending')
            ->where(['OR', ['uprid' => $uProductId], ['uprid' => (((int)$isStrict > 0) ? $uProductId : \common\helpers\Inventory::getInventoryId($uProductId))]])
            ->andWhere('(products_quantity - (qty_cnld + qty_rcvd)) > 0')
            ->asArray(true)->all() as $purchaseOrderProductRecord
        ) {
            $return += (int)$purchaseOrderProductRecord['pending'];
        }
        unset($purchaseOrderProductRecord);
        return $return;
    }
    ...
    public static function getWarehouseIdPriorityArray($uProductId = 0, $productQuantity = 1, $platformId = 0)
    {
        ...
        if ($isStockControl == false) {
            ...
            if ($extension = \common\helpers\Acl::checkExtension('WarehousePriority', 'getInstance')) {
                $rulesArray = [
    ...
    public static function getSupplierIdPriorityArray($uProductId = 0)
    {
        ...
        if ($productRecord instanceof \common\models\Products) {
            ...
            unset($supplierQuery);
            if ($extension = \common\helpers\Acl::checkExtension('SupplierPriority', 'getInstance')) {
    ...
    public static function getChildArray($productRecord = 0, $asArray = true, $recursionBreak = 0)
    {
        ...
        if ($productRecord instanceof \common\models\Products) {
            if ((int)$productRecord->is_bundle > 0) {
                ...
                if ($ext = \common\helpers\Acl::checkExtension('LinkedProducts', 'getChildrenProducts')) {
    ...
    {
        $return = false;
        $productRecord = self::getRecord($productRecord, false, $ignoreFreeze);
        if ($productRecord instanceof \common\models\FreezeProducts) {
            ...
            foreach (\common\models\FreezeOrdersProductsAllocate::find()->where(['prid' => $productId])->asArray(true)->all() as $productAllocatedRecord) {
                $allocated_stock_quantity += ($productAllocatedRecord['allocate_received'] - $productAllocatedRecord['allocate_dispatched']);
            }
            unset($productAllocatedRecord);
            $productRecord->allocated_stock_quantity = $allocated_stock_quantity;

            $temporary_stock_quantity = 0;
            foreach (\common\models\FreezeTemporaryStock::find()->where(['prid' => $productId])->asArray(true)->all() as $productAllocatedTemporaryRecord) {
                $temporary_stock_quantity += $productAllocatedTemporaryRecord['temporary_stock_quantity'];
            }
            unset($productAllocatedTemporaryRecord);
    ...
    {
        if (defined('ENABLE_FREEZE_STOCK') && ENABLE_FREEZE_STOCK == '1' && !$ignoreFreeze) {
            if (!($uProductId instanceof \common\models\FreezeProducts)) {
                $uProductId = \common\models\FreezeProducts::findOne(['products_id' => (int)$uProductId]);
            }
            if ((int)$doCache > 0 AND ($uProductId instanceof \common\models\FreezeProducts)) {
            ...
            if (!($uProductId instanceof \common\models\Products)) {
                $uProductId = \common\models\Products::findOne(['products_id' => (int)$uProductId]);
            }
    ...
    public static function hasAssets($products_id){
        if ($ext = \common\helpers\Acl::checkExtension('ProductAssets', 'hasAssets')){
    ...
    public static function getAssets($uprid, array $params = []){
        if ($ext = \common\helpers\Acl::checkExtension('ProductAssets', 'getAssets')){
    ...
    public static function getAsset($asset_id){
        if ($ext = \common\helpers\Acl::checkExtension('ProductAssets', 'getAsset')){
    ...
    public static function cleanupSearch($string, $checkLength = 2) {
      ...
      if ($checkLength ) {
        ...
        if ($checkLength == 2 && defined('MSEARCH_ENABLE') && MSEARCH_ENABLE == 'fulltext') {
          ...
          if ($ext = \common\helpers\Acl::checkExtension('PlainProductsDescription', 'getMinTokenLength')) {
 ...
 */
    public static function getAdminDetailsList($productsIds, $details = ['name', 'price', 'status', 'model'], $limit = false) {
      ...
      $pQ = \common\models\Products::find()->alias('p')
          ->joinWith('backendDescription pd')
          ->addSelect('pd.products_id, p.products_model, p.products_price, p.products_status')
