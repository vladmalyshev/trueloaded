 ...
 */
    public static function get_warehouses($include_inactive = false) {
        ...
        if ( !isset($cached[$include_inactive]) ) {
            $warehouses_query = tep_db_query("select warehouse_id, warehouse_name from " . TABLE_WAREHOUSES . " where 1 " . ($include_inactive ? '' : " and status = '1' ") . " order by sort_order, warehouse_name");
    ...
    public static function update_products_quantity($products_id, $warehouse_id, $qty, $qty_prefix, $suppliers_id = 0, $location_id = 0, $parameters = array())
    {
        ...
        if ($isError == false) {
            ...
            if ($ext = \common\helpers\Acl::checkExtension('Ebay', 'setUpdateProduct')) {
...
// }}
    ...
    public static function getLocationPath($location_id = 0, $warehouse_id, $blocksList = []) {
        $item = '';
        if ($location_id == 0) {
            return $item;
        }
