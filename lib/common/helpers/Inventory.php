...
class Inventory {
    ...
    public static function disabledOnProduct($productId)
    {
        ...
        return !!$products_arr['without_inventory'];
    }
...
/*
     ...
     */
    public static function getInventoryAttributeNameList($inventoryId = 0)
    {
        global $languages_id;
        $return = [];
     ...
     * Behaviour: depending on PRODUCTS_INVENTORY configuration value
     * @param mixed $inventoryId Product Inventory Id
    ...
    public static function isInventory($inventoryId = 0)
    {
        ...
        if (defined('PRODUCTS_INVENTORY') && PRODUCTS_INVENTORY == 'True') {
            ...
            if ($inventoryRecord instanceof \common\models\Inventory) {
                $productAttributeArray = \common\models\ProductsAttributes::find()->where(['products_id' => (int)$inventoryId])->asArray(true)->all();
