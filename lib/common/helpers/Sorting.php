 ...
 */
   public static function getPossibleSortOptions($inclGSO = false) {
     $ret = [];
     $ret[''] = TEXT_NO_SORTING . ($inclGSO?TEXT_FROM_CONFIGURATION:'');
     if ($inclGSO) {
       $ret['gso'] = TEXT_MARKETING_SORT_ORDER;
     }
    ...
    public static function getOrderByArray($shortKey){
        ...
        if (strlen($shortKey)==2) {
            ...
            foreach (\common\helpers\Sorting::SORT_OPTIONS as $so) {
                if ($so[1] == $f) {
                    return [$so[2] => ($d=='d' ? SORT_DESC : SORT_ASC)];
                }
            }
