...
class Properties {
    ...
    public static function generate_properties_tree($parent_id = '0', $properties_array = array(), $values_array = array(), $properties_tree_array = '', $throughoutID = '', $val_extra = array()) {
        ...
        $properties_query = tep_db_query("select p.properties_id, p.properties_type, p.decimals, p.parent_id, p.extra_values, pd.properties_name, pd.properties_image, pd.properties_color, pu.properties_units_title from " . TABLE_PROPERTIES . " p, " . TABLE_PROPERTIES_DESCRIPTION . " pd left join " . TABLE_PROPERTIES_UNITS . " pu on pu.properties_units_id = pd.properties_units_id where p.parent_id = '" . (int) $parent_id . "' and p.properties_id = pd.properties_id and pd.language_id = '" . (int) $languages_id . "' ".((is_array($only_requested_property_ids) && count($only_requested_property_ids)>0)?" AND p.properties_id IN('".implode("','",$only_requested_property_ids)."')":'')." order by (p.properties_type = 'category'), p.sort_order, pd.properties_name");
        $sort = 1;
        while ($properties = tep_db_fetch_array($properties_query)) {
