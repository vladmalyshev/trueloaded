...
class Gifts {
    ...
    public static function prepareAndSend($virtual_gift_card, $from_email = STORE_OWNER_EMAIL_ADDRESS, $from_name = STORE_OWNER){ //2do over helpers\Mail
        ...
        $contents = implode('', file(tep_href_link('catalog/gift', 'page_name=' . $virtual_gift_card['gift_card_design'], 'NONSSL', false)));
 ...
 */
    ...
    public static function getGiveAwaysArray($products_id = 0){
        ...
        $details = [];
        foreach (\frontend\design\Info::getListProductsDetails(array_unique(\yii\helpers\ArrayHelper::getColumn($products, 'products_id')), ['listing_type' => 'gwa' . $response['cart_total'], 0 => ['show_attributes' => 0]])
            as $detail) {
