    ...
    public function createUrl($manager, $route, $params)
    {
        ...
        if ( $settings['search_engine_friendly_urls'] && $settings['search_engine_unhide'] ) {
            ...
            if ($route === 'catalog/product' && tep_not_null($params['products_id']) && !strstr($params['products_id'], '{') && !strstr($params['products_id'], '}')) {
                $products_seo_page_name = \common\helpers\Product::getSeoName((int)$params['products_id'], (int)$_languages_id, (tep_not_null($params['platform_id'])?$params['platform_id']:\common\classes\platform::currentId()));
...
// }}
            ...
            if ( ($route === 'info' || $route === 'info/index') && $params['info_id'] > 0) {
                $information = tep_db_fetch_array(tep_db_query(
                  ...
                  "from " . TABLE_INFORMATION . " where information_id = '" . (int)$params['info_id']  . "' and languages_id = '" . (int)$_languages_id . "' ".
            ...
            if (preg_match('#(delivery-location)(/index)?#',$route, $routeMatch)){
                if (isset($params['id'])){
                    $id = $params['id'];
                    $path = [];
                    ...
                    do {
                        $get_data_r = tep_db_query(
                            ...
                            "WHERE fd.platform_id='" . (int)\common\classes\platform::currentId() . "' AND fdt.language_id='" . (int)$_languages_id . "' " .
                            " AND fd.id='" . (int)$id . "' " .
                            " AND fd.status=1 ".
                            ""
                        );
                        ...
                        if (tep_db_num_rows($get_data_r) > 0) {
                            ...
                            array_unshift($path,$get_data['seo_page_name']);
                        }else{
                            break;
                        }
                    }while($id>0);

                    //array_unshift($path, $routeMatch[1]);
                    if ( count($path)>0 ) {
                        unset($params['id']);
                        return implode('/', $path) . '/' . (count($params) > 0 ? '?' . http_build_query($params) : '');
                    }
                }
            }
                ...
                if (count($categories_array) == 0 && substr($seo_path_part, 0, 1) == '(' && substr($seo_path_part, -1) == ')') {
                    ...
                    foreach ($categories_seo_array as $categories_seo_page_name) {
                        ...
                        if ($category['categories_id'] > 0) {
                            ...
                            if ($category['categories_id'] == $categories_seo_page_name) {
                ...
                if (count($brands_array) == 0 && substr($seo_path_part, 0, 1) == '(' && substr($seo_path_part, -1) == ')') {
                    ...
                    foreach ($brands_seo_array as $brands_seo_name) {
                        ...
                        if ($manufacturer['manufacturers_id'] > 0) {
                            ...
                            if ($manufacturer['manufacturers_id'] == $brands_seo_name) {
...
// }}
            ...
            $information = tep_db_fetch_array(tep_db_query(
              "select information_id, languages_id from " . TABLE_INFORMATION . " where seo_page_name = '" . tep_db_input($seo_path) . "' ".
              " AND platform_id='".\common\classes\platform::currentId()."' ".
              "ORDER BY IF(languages_id='".(int)$languages_id."', 0, 1) ".
              "limit 1"
            ));
...
*/
          ...
          if ($ext = \common\helpers\Acl::checkExtension('SupportSystem', 'checkUrl')) {
            if (($response = $ext::checkUrl($seo_path)) !== false ){
                return $response;
            }
          }

          if ( !empty($seo_path) ) {
              $locationPath = explode('/', trim($seo_path,'/') );
              $id = \common\helpers\SeoDeliveryLocation::getIdBySeoName(\common\classes\platform::currentId(), $locationPath);
              if ( $id ) {
                  return ['delivery-location/',['id'=>$id]];
              }
          }
//          if( preg_match('/^delivery\-location\/?(.*)/',$seo_path, $location) ) {
//              $locationPath = explode('/', trim($location[1],'/') );
//              $id = \common\helpers\SeoDeliveryLocation::getIdBySeoName(\common\classes\platform::currentId(), $locationPath);
//              if ( $id ) {
//                  return ['delivery-location/',['id'=>$id]];
//              }
//          }
          $delivery_location = tep_db_fetch_array(tep_db_query(
             "select id from " . TABLE_SEO_DELIVERY_LOCATION . " ".
             "where old_seo_page_name = '" . tep_db_input($seo_path) . "' ".
             " AND platform_id='".\common\classes\platform::currentId()."' ".
             " AND status=1 ".
             "limit 1"
          ));
          if (is_array($delivery_location)) {
              $href = tep_href_link(FILENAME_DELIVERY_LOCATION, 'id=' . $delivery_location['id']);
          }
