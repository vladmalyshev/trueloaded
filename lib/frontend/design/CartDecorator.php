...
class CartDecorator
{
    ...
    private function decorateProducts(){
        ...
        for ($i=0, $n=sizeof($this->products); $i<$n; $i++) {
            ...
            $this->products[$i]['id_platform'] = $this->products[$i]['platform_id'];
...
// }}
            ...
            if (is_array($this->products[$i]['sub_products']) && count($this->products[$i]['sub_products']) > 0) { // if has sub-products
              // show full configurator pкice
            ...
            } elseif ($this->products[$i]['parent'] != '') { // if is sub-product
              // don't show pice
              ...
              if ($this->products[$i]['standard_price'] !== false){
                $this->products[$i]['standard_price'] = $currencies->display_price($this->products[$i]['standard_price'], \common\helpers\Tax::get_tax_rate($this->products[$i]['tax_class_id']), $this->products[$i]['quantity']);
              }
            }
            ...
            if ($this->products[$i]['parent'] == '') { // if not sub-product
            ...
            if (STOCK_CHECK == 'true'){
                if ( isset($this->products[$i]['stock_info']) ) {
                  if ( $this->bound_quantity_ordered==false ) {
                    $this->bound_quantity_ordered = $this->products[$i]['stock_info']['order_instock_bound'];
                  }
            ...
            if (isset($this->products[$i]['attributes']) && is_array($this->products[$i]['attributes'])){
                if ($this->products[$i]['virtual_gift_card'] && $this->products[$i]['attributes'][0] > 0) {
                    ...
                    foreach ($this->products[$i]['attributes'] as $option => $value) {
                        ...
                        $attributes_values = ProductsAttributes::find()
                                ...
                                ->where(['pa.products_id' => (int)($option_arr[1] > 0 ? $option_arr[1] : $this->products[$i]['id']),
