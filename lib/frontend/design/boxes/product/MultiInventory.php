    ...
    public function run() {
        ...
        if ($params['products_id']) {
            ...
            if (\common\helpers\Attributes::has_product_attributes($params['products_id'])) {
                ...
                if ($product && !$product['settings']->show_attributes_quantity && Yii::$app->controller instanceof \frontend\controllers\CatalogController) {
                ...
                if ($details['attributes_array']) {
                    ...
                    if (\Yii::$app->request->isAjax || $type=='listing' || $type=='productListing') {
                        ...
                        if ($type=='productListing'){
                            ...
                            $details['product_attributes'] = IncludeTpl::widget(['file' => 'boxes/product/multi-attributes.tpl', 'params' => ['attributes' => $details['attributes_array'], 'isAjax' => false, 'options_prefix' => $options_prefix]]);
                        } else {
                            $tmp = $details;
                            $lastRow = array_pop($tmp['attributes_array']);
                            $details['product_attributes'] = IncludeTpl::widget(['file' => 'boxes/product/attributes/mix.tpl', 'params' => ['item' => $lastRow, 'products_id' => $params['products_id'], 'isAjax' => true,]]);
                        }
                        return json_encode($details);
                    } else {
                        return IncludeTpl::widget(['file' => 'boxes/product/multi-attributes.tpl', 'params' => ['attributes' => $details['attributes_array'], 'isAjax' => false, 'options_prefix' => $options_prefix,]]);
                    }
                }
            }
        }
