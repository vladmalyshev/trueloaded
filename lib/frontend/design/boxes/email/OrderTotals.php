...
class OrderTotals extends Widget
{
    ...
    public function run()
    {
        ...
        $html = '<table class="totals-table"' . ($attributesText['.totals-table'] ? 'style="' . $attributesText['.totals-table'] . '"' : '') . ' cellpadding="0" cellspacing="0" border="0" width="100%">';
        foreach ($this->params['order_total_output'] as $item) {
            $html .= '
          <tr class="totals-row ' . $item['code'] . ($item['show_line'] ? ' totals-line' : '') . '" style="' .
                $attributesText['.totals-row'] .
                $attributesText['.'.$item['code']] .
                ($item['show_line'] ? $attributesText['.totals-line'] : ''). '">
