...
if (is_array($order->products)){
    foreach ($order->getOrderedProducts('invoice') as $product) {
      ...
      if ($ext = \common\helpers\Acl::checkExtensionAllowed('ProductAssets', 'allowed')){
        $html .= $ext::getOrderedProductsAssets($order, $product['id']);
      }
      $html .= '
            </td>
            ...
            <td style="text-align:right; border-top: 1px solid #ccc">' . $currencies->format($currencies->calculate_price_in_order($order->info, $product['final_price']), true, $order->info['currency'], $order->info['currency_value']) . '</td>
