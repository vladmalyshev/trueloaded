    ...
    public function run()
    {
        ...
        switch (true) {
            ...
            case preg_match('/^delivery-location\//', $full_action):
                $info = \common\helpers\SeoDeliveryLocation::getItem($params['id'],\common\classes\platform::currentId());
                $delivery_location_parents = \common\helpers\SeoDeliveryLocation::getParents(\common\classes\platform::currentId(),$info['parent_id'],true);
                ...
                $h1 = defined('HEAD_H1_TAG_DELIVERY_LOCATION') && tep_not_null(HEAD_H1_TAG_DELIVERY_LOCATION) ? HEAD_H1_TAG_DELIVERY_LOCATION : '';
                if (strstr($h1, '##LOCATION_NAME##')) {
                    $h1 = str_replace('##LOCATION_NAME##', $info['name'], $h1);
                }
                if ( strpos($h1,'##BREADCRUMB##')!==false ) {
                    if ( count($delivery_location_parents)==0 && Seo::getMetaDefaultBreadcrumb('delivery-location') ) {
                        $h1 = str_replace('##BREADCRUMB##', Seo::getMetaDefaultBreadcrumb('delivery-location'), $h1);
                    }else {
                        $h1 = str_replace('##BREADCRUMB##', implode(' / ',ArrayHelper::getColumn($delivery_location_parents,'location_name')), $h1);
                    }
                }
                if ( strpos($h1,'##LOCATION_TITLE_TAG##')!==false ) {
                    $h1 = str_replace('##LOCATION_TITLE_TAG##', $info['location_meta_title'], $h1);
                }
                if ( strpos($h1,'##LOCATION_DESCRIPTION_TAG##')!==false ) {
                    $h1 = str_replace('##LOCATION_DESCRIPTION_TAG##', $info['location_meta_description'], $h1);
                }

                $h2 = defined('HEAD_H2_TAG_DELIVERY_LOCATION') && tep_not_null(HEAD_H2_TAG_DELIVERY_LOCATION) ? HEAD_H2_TAG_DELIVERY_LOCATION : '';
                if (strstr($h2, '##LOCATION_NAME##')) {
                    $h2 = str_replace('##LOCATION_NAME##', $info['name'], $h2);
                }
                if ( strpos($h2,'##BREADCRUMB##')!==false ) {
                    if ( count($delivery_location_parents)==0 && Seo::getMetaDefaultBreadcrumb('delivery-location') ) {
                        $h2 = str_replace('##BREADCRUMB##', Seo::getMetaDefaultBreadcrumb('delivery-location'), $h2);
                    }else {
                        $h2 = str_replace('##BREADCRUMB##', implode(' / ',ArrayHelper::getColumn($delivery_location_parents,'location_name')), $h2);
                    }
                }
                if ( strpos($h2,'##LOCATION_TITLE_TAG##')!==false ) {
                    $h2 = str_replace('##LOCATION_TITLE_TAG##', $info['location_meta_title'], $h2);
                }
                if ( strpos($h2,'##LOCATION_DESCRIPTION_TAG##')!==false ) {
                    $h2 = str_replace('##LOCATION_DESCRIPTION_TAG##', $info['location_meta_description'], $h2);
                }
                $h2 = explode("\n", $h2);

                $h3 = defined('HEAD_H3_TAG_DELIVERY_LOCATION') && tep_not_null(HEAD_H3_TAG_DELIVERY_LOCATION) ? HEAD_H3_TAG_DELIVERY_LOCATION : '';
                if (strstr($h3, '##LOCATION_NAME##')) {
                    $h3 = str_replace('##LOCATION_NAME##', $info['name'], $h3);
                }
                if ( strpos($h3,'##BREADCRUMB##')!==false ) {
                    if ( count($delivery_location_parents)==0 && Seo::getMetaDefaultBreadcrumb('delivery-location') ) {
                        $h3 = str_replace('##BREADCRUMB##', Seo::getMetaDefaultBreadcrumb('delivery-location'), $h3);
                    }else {
                        $h3 = str_replace('##BREADCRUMB##', implode(' / ',ArrayHelper::getColumn($delivery_location_parents,'location_name')), $h3);
                    }
                }
                if ( strpos($h3,'##LOCATION_TITLE_TAG##')!==false ) {
                    $h3 = str_replace('##LOCATION_TITLE_TAG##', $info['location_meta_title'], $h3);
                }
                if ( strpos($h3,'##LOCATION_DESCRIPTION_TAG##')!==false ) {
                    $h3 = str_replace('##LOCATION_DESCRIPTION_TAG##', $info['location_meta_description'], $h3);
                }
                $h3 = explode("\n", $h3);
        ...
        if ($this->settings[0]['heading_item']) {
            ...
            if ($this->settings[0]['heading_type'] == 'h2') {
                if ($h2) {
                    ...
                    return '<h2>' . $h2[$this->settings[0]['heading_item'] - 1] . '</h2>';
                } else {
                    return '';
                }
                ...
                if ($h3) {
                    ...
                    return '<h3>' . $h3[$this->settings[0]['heading_item'] - 1] . '</h3>';
                } else {
                    return '';
                }
            }
