  ...
  public function run()
  {
    ...
    if ($current_category_id > 0) {
      ...
      // Get the category name and description
        $category = CategoriesDescriptionHelper::getCategoriesDescriptionList($current_category_id, $languages_id, (int) $_SESSION['affiliate_ref']);
      ...
      $manufacturer_query = tep_db_query("select m.manufacturers_name as categories_name, mi.manufacturers_h1_tag from " . TABLE_MANUFACTURERS . " m left join " . TABLE_MANUFACTURERS_INFO . " mi on (m.manufacturers_id = mi.manufacturers_id and mi.languages_id = '" . (int)$languages_id . "')  where m.manufacturers_id = '" . (int)$_GET['manufacturers_id'] . "'");
