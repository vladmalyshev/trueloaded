...
class ListByPage extends Widget
{
    ...
    public function run()
    {
        if (!$this->settings[0]['page_id']) {
            return $this->hideBox();
        }

        $idList = \common\models\SeoDeliveryLocation::find()
            ->select(['id'])
            ->where([
                'parent_id' => $this->settings[0]['page_id'],
        ...
        foreach ($idList as $data) {
            ...
            $item['href'] = \Yii::$app->urlManager->createUrl([FILENAME_DELIVERY_LOCATION,'id'=>$data['id']]);
