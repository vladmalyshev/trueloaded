                ...
                while ($downloads = tep_db_fetch_array($downloads_query)) {
                    ...
                    $download_expiry = date('Y-m-d H:i:s', $download_timestamp);
                      if ( ($downloads['download_count'] > 0) && (file_exists(DIR_FS_DOWNLOAD . $downloads['orders_products_filename'])) && ( ($downloads['download_maxdays'] == 0) || ($download_timestamp > time())) ) {
                        $text = '<a href="' . tep_href_link(FILENAME_DOWNLOAD, 'order=' . $this->params['orders_id'] . '&id=' . $downloads['orders_products_download_id']) . '">' . $downloads['products_name'] . '<br>' . '<button>' . TEXT_DOWNLOAD_FILE . '</button>' . '</a>';
                      } else {
                    ...
                    $downloadContents[] = [
                        $text,
                        TABLE_HEADING_DOWNLOAD_DATE . '<br>' . \common\helpers\Date::date_long($download_expiry),
