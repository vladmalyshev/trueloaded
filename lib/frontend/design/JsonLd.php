...
class JsonLd
{
    ...
    private static function organization()
    {
        ...
        if ($organization['@type'] == 'Store' || $organization['@type'] == 'LocalBusiness') {
            ...
            $minPrice = $currencies->display_price($productPrices['min_price'], \common\helpers\Tax::get_tax_rate($productPrices['products_tax_class_id']));
            $maxPrice = $currencies->display_price($productPrices['max_price'], \common\helpers\Tax::get_tax_rate($productPrices['products_tax_class_id']));
