      ...
      {foreach $banners as $bKey => $banner}
        ...
        {elseif $banner.banners_image && (!$banner.banner_display || $banner.banner_display == 3)}
            {if $banner.banners_url}
              ...
              <div class="single_banner"><a href="{$banner.banners_url}"{if $banner.target == 1} target="_blank"{else}{/if} class="imgBann" data-id="{$banner.banners_id}">{$banner.image}</a></div>
            {else}
    ...
    {elseif $banner_type == 'carousel'}
        <div class="carousel">
            {foreach $banners as $banner}
              {if $banner.is_banners_image_valid}
                <div class="item">
                  {if $banner.banner_display == '0' ||  $banner.banner_display == '2' ||  $banner.banner_display == '3'}
                    {if $banner.banners_url}
                      <script>collectBanners({json_encode($banner)});</script>
                      <a href="{$banner.banners_url}"{if $banner.target == 1} target="_blank"{else}{/if} class="imgBann" data-id="{$banner.banners_id}">
      ...
      <div class="slider-wrapper"><div id="slider" class="sliderItems">
          {foreach $banners as $banner}
              {if $banner.banner_display == '0' || $banner.banner_display == '2'}
                {if $banner.banners_url}
                    <a href="{$banner.banners_url}"{if $banner.target == 1} target="_blank"{else}{/if} class="imgBann" data-id="{$banner.banners_id}">{$banner.image}<script>collectBanners({json_encode($banner)});</script></a>
                {else}
                    {$banner.image}
                {/if}
              {else}
