...
{else}
  {$can_buy = false}
{/if}
{if ((!$product.product_has_attributes && !Yii::$app->user->isGuest) || $settings[0].list_demo) && !GROUPS_DISABLE_CHECKOUT}
  {$can_save = true}
{else}
  {$can_save = false}
{/if}
...
<div class="item"><div class="item-holder">
  ...
  {if !$settings[0].show_name}
    <div class="title name"><table class="wrapper"><tr><td>
                    <a href="{$product.link}">
  ...
  {/if}
    ...
    {if !$settings[0].show_description}
      <div class="description description-2"><table class="wrapper"><tr><td>{if $product.products_description_short}{$product.products_description_short|truncate:90:"...":false}{else}{$product.products_description|truncate:90:"...":true}{/if}</td></tr></table></div>
    {/if}
  ...
  {if (!$settings[0].show_wishlist_button && $can_save) || !$settings[0].show_view_button}
  <div class="buttons">
    {/if}
    {if !$settings[0].show_wishlist_button && $can_save && Info::pageBlock() != 'product'}
