...
<div class="item">
  ...
  {/if}
      ...
      {Html::beginForm($product.action_buy, 'post', ['class' => 'form-buy'])}
        <div class="qty-input"{if $product.product_in_cart} style="display: none"{/if}>
          ...
          <input type="text" name="qty" value="1" class="qty-inp"
                  ...
                  {/if}
                  {if \common\helpers\Acl::checkExtension('MinimumOrderQty', 'setLimit')}{\common\extensions\MinimumOrderQty\MinimumOrderQty::setLimit($product.order_quantity_data)}{/if}
                  {if \common\helpers\Acl::checkExtension('OrderQuantityStep', 'setLimit')}{\common\extensions\OrderQuantityStep\OrderQuantityStep::setLimit($product.order_quantity_data)}{/if} />
    ...
    <div class="buttons">
      {/if}
      {if !$settings[0].show_wishlist_button_rows}
