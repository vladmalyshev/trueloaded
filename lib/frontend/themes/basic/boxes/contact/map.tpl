      ...
      if (status === google.maps.GeocoderStatus.OK) {
        ...
        var panorama = new google.maps.StreetViewPanorama(
                document.getElementById('pano'), {
                  position: results[0].geometry.location,
                  pov: {
                    heading: {if $settings_street[0].street_heading}{$settings_street[0].street_heading}{else}0{/if},
                    pitch: {if $settings_street[0].street_pitch}{$settings_street[0].street_pitch}{else}0{/if}
                  }
                });
