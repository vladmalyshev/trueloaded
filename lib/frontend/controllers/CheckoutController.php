...
// redirect the customer to a friendly cookie-must-be-enabled page if cookies are disabled (or the session has not started)
        ...
        if (Yii::$app->request->isPost) {
            ...
            if ($ext = \common\helpers\Acl::checkExtension('Neighbour', 'saveNeighbourDetails')) {
...
// load selected payment module
        ...
        if ($this->manager->get('credit_covers') /* && $payment != 'paypal_express' */) {
...
//ICW ADDED FOR CREDIT CLASS SYSTEM
    ...
    public function actionRestart($order_id) {
        ...
        if (Yii::$app->user->isGuest) {
            ...
            $this->redirect(tep_href_link('account/login', '', 'SSL'));
        }
        ...
        if (\common\helpers\Acl::checkExtension('MultiCart', 'getLastCreatedKey')){
            $key = \common\extensions\MultiCart\MultiCart::getLastCreatedKey();
            if ($key){
        ...
        if ($cart->count_contents() > 0) {
            $this->redirect(tep_href_link('checkout/index', '', 'SSL'));
        } else {
            $this->redirect(tep_href_link('shopping-cart/index', '', 'SSL'));
        }
    }
