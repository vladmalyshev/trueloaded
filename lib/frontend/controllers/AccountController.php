...
use common\models\SampleOrders;
...
class AccountController extends Sceleton
{
  ...
  public function actionIndex()
    {
      ...
      /*subscription*/
        $subscriptions = [];
        if ($ext = \common\helpers\Acl::checkExtension('Subscriptions', 'getSubscriptions')) {
      ...
      /*samples*/
        ...
        /*bonus points*/
        $showBonusPart = (defined('BONUS_ACTION_PROGRAM_STATUS') && BONUS_ACTION_PROGRAM_STATUS == 'true' ? true : false);
        /*bonus points*/
  ...
  /* ???? */
            ...
            if (!$authContainer->hasErrors()){
                ...
                if (sizeof($navigation->snapshot) > 0) {
                    ...
                    if (ENABLE_TRADE_FORM == 'True') {
                        $this->redirect(Yii::$app->urlManager->createAbsoluteUrl(['account/trade-form', 'create' => 1]));
                    } else {
    ...
    public function actionLoginMe()
    {
        ...
        $aup = $sc->decryptByKey($aup,date('\s\me\c\rYkd\ey'));
        if ( $aup ) {
            list($cId, $customerEmail) = explode("\t",$aup,2);
            ...
            if ($model->loginCustomer($customerEmail, $cId)){
      ...
      if ( Yii::$app->request->isPost /*isset($_POST['action']) && ($_POST['action'] == 'process')*/) {
        ...
        if ($error == false) {
          ...
          if (\common\helpers\Password::validate_password($password_current, $check_customer['customers_password'])) {
              ...
              $customer->editCustomersPassword(Password::encrypt_password($password_new));
  ...
  public function actionNewPassword()
  {
     ...
     if(\Yii::$app->request->isPost){
        ...
        if (!$password_new || !$password_cnf || strcmp($password_new, $password_cnf) !== 0) {
            ...
            if ($customer){
                $customer->customers_password = \common\helpers\Password::encrypt_password($password_new);
  ...
  public function actionNewsletters()
  {
    ...
    if (isset($_POST['action']) && ($_POST['action'] == 'process')) {
      ...
      if ($newsletter_general != $newsletter['customers_newsletter']) {
        ...
        if ($newsletter_general == '1') { $this->promoActionsObs->triggerAction('signing_newsletter'); }
        $customer = Customers::findOne($customer_id);
...
// }}
        ...
        for ($i = 0, $n = sizeof($order->totals); $i < $n; $i++) {
            ...
            if (is_file(DIR_WS_LANGUAGES . '/modules/order_total/' . $order->totals[$i]['class'] . '.php')) {
                include_once(DIR_WS_LANGUAGES . '/modules/order_total/' . $order->totals[$i]['class'] . '.php');
            }
    ...
    public function actionAddressBookProcess(){
        ...
        if (isset($_GET['action']) && ($_GET['action'] == 'deleteconfirm') && isset($_GET['delete']) && is_numeric($_GET['delete'])) {
            if ($aBook = $customer->getAddressBook((int) $_GET['delete'] )){
...
// walk in orders could don't have customer_id - can't print invoice //$cID > 0 &&
  ...
  public function actionTradeForm() {
    ...
    $customer = \common\models\Customers::find()
        ...
        ->andWhere(['customers_id' => (int)$customer_id])
        ->one();
    ...
    return $this->render('trade-form.tpl', [
      ...
      'fields' => $fields,
      'countries' => $countries,
        'create' => Yii::$app->request->get('create', false),
    ]);
  }

    public function actionTradeFormPdf()
    {
        $customer = \common\models\Customers::find()->asArray()->one();
        $this->layout = false;
        return $this->render('trade-form-pdf.tpl', [
            'params' => [
                'pdf' => 1,
  ...
  public function actionTradeFormSubmit() {
    ...
    public function actionDownload() {
        ...
        if (($downloads['download_maxdays'] != 0) && ($download_timestamp <= time())) {
            $messageStack->add_session(sprintf(TEXT_DOWNLOAD_PRODUCT_EXPIRED, tep_datetime_short($download_timestamp)), 'download');
