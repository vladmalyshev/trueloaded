...
//      ),
  ...
  public function actionIndex()
  {
      ...
      foreach ( $this->registered_map as $registered_feed ) {
          ...
          if ($registered_feed['feedname'] == 'products' || $registered_feed['feedname']=='images' || $registered_feed['feedname']=='delivery-location' ) {
              $feed_count = 1;
    ...
    protected function do_create_delivery_location( $pageNumber=0 ){
        ...
        if ( count($this->urlParams)==0 ) {
            ...
            foreach ($this->urlParams as $urlParam){
                $params[] = '&'.http_build_query($urlParam);
            }
        }

        $on_page = 100000;
        if ( (int)$this->config['GSM_DELIVERY_LOCATION_PAGE_COUNT']>0 ) $on_page = (int)$this->config['GSM_DELIVERY_LOCATION_PAGE_COUNT'];

        $tree = SeoDeliveryLocation::getTree(\common\classes\platform::currentId());

        if ( $pageNumber==='count' ) {
            $pages_count = ceil(count($tree)/$on_page);
            return $pages_count;
        }
        if ( count($tree)>$on_page ) {
            $tree = array_slice($tree, ($pageNumber - 1) * $on_page, $on_page);
        }
        foreach( $tree as $treeItem ) {
            $locations = [];
            foreach( $params as $param ) {
                $location = tep_href_link(FILENAME_DELIVERY_LOCATION, 'id='.$treeItem['id'].$param, 'NONSSL',false);
                $locations[$location] = $location;
            }

            // $lastmod = date("Y-m-d", strtotime($treeItem['last_modified']));
            $lastmod = date("Y-m-d");
            foreach ($locations as $location) {
                self::_gsm_urlset(array(
                    ...
                    'priority' => number_format($this->config['GSM_DELIVERY_LOCATION_MIN_RATIO'], 1, '.', ''),
                ));
            }
        }
  ...
  protected function do_create_products( $pageNumber=0 ){
      ...
      $shop_products_sql = (
          ...
          (isset($_SESSION['affiliate_ref']) && $_SESSION['affiliate_ref']>0?" and p2a.affiliate_id is not null ":'').
          (
          (USE_MARKET_PRICES == 'True' || CUSTOMERS_GROUPS_ENABLE == 'True')?
  ...
  protected function do_create_pages( $pageNumber=0 ){
      ...
      $sql_query = tep_db_query(
          ...
          "WHERE i.visible='1' and i.hide_on_xml='0' and i.languages_id ='".$languages_id."' and i.affiliate_id = 0 ".(\common\classes\platform::activeId()?" AND i.platform_id='".\common\classes\platform::currentId()."' ":'')." ".
  ...
  protected function do_create_images( $pageNumber=0 ){
    ...
    $shop_products_sql = (
      ...
      "WHERE p.products_id = pd.products_id and pd.language_id = '" . (int)$languages_id . "' and pd.affiliate_id = '0' ".
  ...
  static protected function _get_max_ratio_param_in_categories( $ratio_param='viewed', $categories_ids=false ) {
    ...
    if ( isset( $_SESSION['affiliate_ref'] ) && (int)$_SESSION['affiliate_ref']>0 ) {
  ...
  static protected function _get_categories_map_tree(){
    ...
    if ( $categoriesMap===false ) {
      ...
      $categories_tree_sql =
        ...
        "left join " . TABLE_CATEGORIES_DESCRIPTION .  " cd1 on cd1.categories_id = cd.categories_id and cd1.affiliate_id = '" . (int)$_SESSION['affiliate_ref'] . "' and cd1.language_id = '" . (int)$languages_id . "' ".
