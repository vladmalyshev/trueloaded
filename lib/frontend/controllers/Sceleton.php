...
use common\models\promotions\PromotionsBonusObserver;
/**
...
class Sceleton extends Controller {
    ...
    function __construct($id,$module=null) {
        ...
        // {{ dev basic auth
        if ( strpos(\Yii::$app->id,'frontend')!==false && defined('DEVELOPMENT_MODE') && DEVELOPMENT_MODE=='True' ) {
            $requestRoute = $id.'/index';
            if ( is_object($module) && $module instanceof yii\web\Application) {
                $requestRoute = $module->requestedRoute;
            }
            if ( !Info::isAdmin() && !preg_match('#^(email-template|callback|multisafe|checkout|image/|account/order-barcode|account/order-qrcode|api\-)#',$requestRoute) ) {
        ...
        if ($_GET['max_items']){
            $_SESSION['max_items'] = intval($_GET['max_items']);
        }
        if ($ext = \common\helpers\Acl::checkExtension('ShowInactive', 'changeStatus')) {
            $ext::changeStatus();
        }
        ...
        $this->promoActionsObs = PromotionsBonusObserver::getInstance();
        ...
        switch (true) {
            ...
            case ( $full_action=='catalog/product' &&  isset($params['products_id']) && $params['products_id']>0 ):
                ...
                if (!is_array($the_product_info) || empty($the_product_info['products_head_title_tag']) || !$the_product_info['overwrite_head_title_tag']) {
                    ...
                    if (strstr($the_title, '##BRAND_NAME##')) {
                        $the_title = str_replace('##BRAND_NAME##', \common\helpers\Manufacturers::get_manufacturer_info('manufacturers_name', $the_product_info['manufacturers_id']), $the_title);
                    }
                    if (strstr($the_title, '##CATEGORY_NAME##') || strstr($the_title, '##BREADCRUMB##')) {
                        $product_cPath = \common\helpers\Product::get_product_path($the_product_info['products_id']);
                    ...
                    if (strstr($the_title, '##PRODUCT_TITLE_TAG##')) {
                        $the_title = str_replace('##PRODUCT_TITLE_TAG##', $the_product_info['products_head_title_tag'], $the_title);
                    }
                    if (strstr($the_title, '##PRODUCT_NAME##')) {
                        $the_title = str_replace('##PRODUCT_NAME##', $the_product_info['products_name'], $the_title);
                    } else {
                        $the_title = $the_title . ' ' . $the_product_info['products_name'];
                    }
                } else {
                ...
                if (!is_array($the_product_info) || empty($the_product_info['products_head_keywords_tag'])) {
                    ...
                    if (strstr($the_key_words, '##BRAND_NAME##')) {
                        $the_key_words = str_replace('##BRAND_NAME##', \common\helpers\Manufacturers::get_manufacturer_info('manufacturers_name', $the_product_info['manufacturers_id']), $the_key_words);
                    }
                    ...
                    if (strstr($the_key_words, '##PRODUCT_NAME##')) {
                        $the_key_words = str_replace('##PRODUCT_NAME##', $the_product_info['products_name'], $the_key_words);
                    } else {
                        $the_key_words = $the_product_info['products_name'] . ', ' . $the_key_words;
                    }
                } else {
                    $the_key_words = $the_product_info['products_head_keywords_tag'];
                }

                if (!is_array($the_product_info) || empty($the_product_info['products_head_desc_tag']) || !$the_product_info['overwrite_head_desc_tag']) {
                    ...
                    if (strstr($the_desc, '##BRAND_NAME##')) {
                        $the_desc = str_replace('##BRAND_NAME##', \common\helpers\Manufacturers::get_manufacturer_info('manufacturers_name', $the_product_info['manufacturers_id']), $the_desc);
                    }
                    if (strstr($the_desc, '##CATEGORY_NAME##') || strstr($the_desc, '##BREADCRUMB##')) {
                        ...
                        if ( count($breadcrumb_array)==0 && Seo::getMetaDefaultBreadcrumb($full_action) ) $breadcrumb_array[] = Seo::getMetaDefaultBreadcrumb($full_action);
                        $the_desc = str_replace('##BREADCRUMB##', implode(' / ', $breadcrumb_array), $the_desc);
                        $the_desc = str_replace('##CATEGORY_NAME##', $breadcrumb_array[count($breadcrumb_array)-1], $the_desc);
                    }
                    if (strstr($the_desc, '##PRODUCT_DESCRIPTION_TAG##')) {
                        $the_desc = str_replace('##PRODUCT_DESCRIPTION_TAG##', $the_product_info['products_head_desc_tag'], $the_desc);
                    }
                    if (strstr($the_desc, '##PRODUCT_NAME##')) {
                        $the_desc = str_replace('##PRODUCT_NAME##', $the_product_info['products_name'], $the_desc);
                    } else {
                        $the_desc = $the_product_info['products_name'] . ' ' . $the_desc;
                    }
                } else {
          ...
          "WHERE visible='1' AND information_id='".(int)$params['info_id']."' AND languages_id = '" . $languages_id . "'"
                ...
                */
                  $query_infromation_page = tep_db_query(
                    ...
                    "i.information_id ".
                    "from " . TABLE_INFORMATION . " i LEFT JOIN " . TABLE_INFORMATION . " i1 on i.information_id = i1.information_id  and i1.languages_id = '" . (int)$languages_id . "' and i1.affiliate_id = '" . (isset($_SESSION['affiliate_ref'])?(int)$_SESSION['affiliate_ref']:0) . "' ".(platform::activeId()?" AND i1.platform_id='".platform::currentId()."' ":'')." where i.information_id = '" . (int)$params['info_id'] . "' and i.languages_id = '" . (int)$languages_id . "' and i.visible = 1 and i.affiliate_id = 0 ".(platform::activeId()?" AND i.platform_id='".platform::currentId()."' ":'')." ".
                    "limit 1"
                  );
...
// ALL OTHER PAGES NOT DEFINED ABOVE
            case preg_match('/^delivery-location\//',$full_action):
                $info = \common\helpers\SeoDeliveryLocation::getItem($params['id'],platform::currentId());
                $delivery_location_parents = \common\helpers\SeoDeliveryLocation::getParents(platform::currentId(),$info['parent_id'],true);
                $level = ($info['parent_id']>0?count($delivery_location_parents)+1:1);
                $info = \common\helpers\SeoDeliveryLocation::applyTemplate(
                    platform::currentId(),
                    $languages_id,
                    $level,
                    $info['parent_id'],
                ...
                if ( $info['overwrite_head_title_tag'] ) {
                    $the_title = $info['location_meta_title'];
                }elseif (defined('HEAD_TITLE_TAG_DELIVERY_LOCATION') && tep_not_null(HEAD_TITLE_TAG_DELIVERY_LOCATION) ){
                    $the_title = HEAD_TITLE_TAG_DELIVERY_LOCATION;
                    $the_title = str_replace('##LOCATION_NAME##', $info['name'], $the_title);
                    if ( strpos($the_title,'##BREADCRUMB##')!==false ) {
                        if ( count($delivery_location_parents)==0 && Seo::getMetaDefaultBreadcrumb('delivery-location') ) {
                            $the_title = str_replace('##BREADCRUMB##', Seo::getMetaDefaultBreadcrumb('delivery-location'), $the_title);
                        }else{
                            $the_title = str_replace('##BREADCRUMB##', implode(' / ', ArrayHelper::getColumn($delivery_location_parents, 'location_name')), $the_title);
                        }
                    }
                    if ( strpos($the_title,'##LOCATION_TITLE_TAG##')!==false ) {
                        $the_title = str_replace('##LOCATION_TITLE_TAG##', $info['location_meta_title'], $the_title);
                    }
                    if ( strpos($the_title,'##LOCATION_DESCRIPTION_TAG##')!==false ) {
                        $the_title = str_replace('##LOCATION_DESCRIPTION_TAG##', $info['location_meta_description'], $the_title);
                    }
                }else{
                    $the_title = empty($info['meta_title'])?($info['location_name'].' '.$HEAD_TITLE_TAG_ALL):$info['meta_title'];
                    //$the_title = $HEAD_TITLE_TAG_ALL;
                }

                if ( $info['overwrite_head_desc_tag'] ) {
                    $the_desc = $info['location_meta_description'];
                }elseif (defined('HEAD_DESC_TAG_DELIVERY_LOCATION') && tep_not_null(HEAD_DESC_TAG_DELIVERY_LOCATION)){
                    $the_desc = HEAD_DESC_TAG_DELIVERY_LOCATION;
                    $the_desc = str_replace('##LOCATION_NAME##', $info['name'], $the_desc);
                    if ( strpos($the_desc,'##BREADCRUMB##')!==false ) {
                        if ( count($delivery_location_parents)==0 && Seo::getMetaDefaultBreadcrumb('delivery-location') ) {
                            $the_desc = str_replace('##BREADCRUMB##', Seo::getMetaDefaultBreadcrumb('delivery-location'), $the_desc);
                        }else{
                            $the_desc = str_replace('##BREADCRUMB##', implode(' / ', ArrayHelper::getColumn($delivery_location_parents, 'location_name')), $the_desc);
                        }
                    }
                    if ( strpos($the_desc,'##LOCATION_TITLE_TAG##')!==false ) {
                        $the_desc = str_replace('##LOCATION_TITLE_TAG##', $info['location_meta_title'], $the_desc);
                    }
                    if ( strpos($the_desc,'##LOCATION_DESCRIPTION_TAG##')!==false ) {
                        $the_desc = str_replace('##LOCATION_DESCRIPTION_TAG##', $info['location_meta_description'], $the_desc);
                    }
                }else{
                    $the_desc = empty($info['meta_description'])?($info['location_name'].' '.$HEAD_DESC_TAG_ALL):$info['meta_description'];
                    //$the_desc = $HEAD_DESC_TAG_ALL;
                };
                $the_title = trim(preg_replace('/\s{2,}/',' ',$the_title));
                $the_desc = trim(preg_replace('/\s{2,}/',' ',$the_desc));

//                $the_title = empty($info['meta_title'])?($info['location_name'] . ' ' . (defined('HEAD_TITLE_TAG_DELIVERY_LOCATION') && tep_not_null(HEAD_TITLE_TAG_DELIVERY_LOCATION) ? HEAD_TITLE_TAG_DELIVERY_LOCATION : $HEAD_TITLE_TAG_ALL)):$info['meta_title'];
                $the_key_words = empty($info['meta_keyword'])?($info['location_name'] . ', ' . (defined('HEAD_KEY_TAG_DELIVERY_LOCATION') && tep_not_null(HEAD_KEY_TAG_DELIVERY_LOCATION) ? HEAD_KEY_TAG_DELIVERY_LOCATION : $HEAD_KEY_TAG_ALL)):$info['meta_keyword'];
//                $the_desc = empty($info['meta_description'])?($info['location_name'] . ' ' . (defined('HEAD_DESC_TAG_DELIVERY_LOCATION') && tep_not_null(HEAD_DESC_TAG_DELIVERY_LOCATION) ? HEAD_DESC_TAG_DELIVERY_LOCATION : $HEAD_DESC_TAG_ALL)):$info['meta_description'];
                break;
    default:
...
*/
    ...
    public function bindActionParams($action, $params)
    {
        ...
        if ($ext = \common\helpers\Acl::checkExtension('Maintenance', 'bindParams')) {
