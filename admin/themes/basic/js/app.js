...
var App = function() {
	...
	var handleResponsive = function() {
      ...
      if ($('#container').hasClass('sidebar-closed')){
        $('.top_header').css('padding-right', '');
        $.cookie('s_hide', 0);
      } else {
        $('.top_header').css('padding-right', '22px');
        $.cookie('s_hide', 1);
      }
...
function updateClock ( )
{
	...
	$("#clock-1").html(currentTimeString);
}
function updateClock2 ( ){
  var clock = $('#clock-2');
  var st = clock.html();
  var pos = st.indexOf(":");
  var hour = st.substring(0, pos);
  var minute = st.substring(pos + 1)
  minute++;
  if (minute > 59){
    minute = 0;
    hour++;
...
$(document).ready(function()
{
	...
	setInterval(updateClock2, 60000);
...
$(window).scroll(function() {
	...
	if($('.order-wrap').length > 0){
		if(($('.order-wrap .table-responsive').offset().top - $('.order-wrap').offset().top) < 101){
			var extra_pad = 101-($('.order-wrap .table-responsive').offset().top - $('.order-wrap').offset().top);
		}else{
			var extra_pad = 0;
		}
